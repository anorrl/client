...
void StarterPlayerScripts::InitializeDefaultScripts()
{
	...
	if (!controlScript) {
		ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets("avatar/scripts/controlScript.arlmx"), 
	...
	if (!cameraScript) {
		ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets("avatar/scripts/cameraScript.arlmx"), 
			ContentProvider::PRIORITY_SCRIPT, boost::bind(&PlayerScriptsLoadHelper, weak_from(this), _1, _2), AsyncHttpQueue::AsyncWrite);
	}
