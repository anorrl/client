...
{
	ARLASSERT(dt == 0.0f);
	ARLASSERT(freeFallBodyIndex == -1);
	ARLASSERT(realTimeBodyIndex == -1);
	ARLASSERT(jointBodyIndex == -1);
	ARLASSERT(contactBodyIndex == -1);
}
...
void SimBody::update()
{
	...
	ARLASSERT_VERY_FAST(Math::longestVector3Component(force) < 1e20f);
	ARLASSERT_VERY_FAST(constantForceY < 1e20f);
	ARLASSERT_VERY_FAST(body->cofmIsClean());
	ARLASSERT_FISHING(!Math::isNanInfDenormVector3(qOrientation.imag()));

	ARLASSERT(dirty);		// concurrency check
...
{
	ARLASSERT(!dirty);
	ARLASSERT_SLOW(Math::longestVector3Component(force) < 1e19);
	ARLASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
	ARLASSERT_SLOW(Math::longestVector3Component(pv.velocity.linear) < 1e6);
	ARLASSERT_SLOW(Math::longestVector3Component(force) < 1e19);
	ARLASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
	ARLASSERT_SLOW(Math::longestVector3Component(pv.velocity.linear) < 1e6);
	...
	ARLASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.position.translation));
	ARLASSERT_FISHING(!Math::isNanInfDenormVector3(qOrientation.imag()));
	ARLASSERT_VERY_FAST(!Math::isNanInfDenormMatrix3(pv.position.rotation));
	ARLASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.velocity.linear));
	ARLASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.velocity.rotational));
...
void SimBody::stepPosition()
{
	...
	ARLASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
	ARLASSERT(!hasExternalForceOrImpulse());
