...
namespace ARL
{
	...
	static long long decodeInt64(long long value)
	{
		return (static_cast<unsigned long long>(value) >> 1) ^ (-(value & 1));
	}
...
#pragma warning( disable : 4293 )
	static void readInt64Vector(MemoryInputStream& stream, std::vector<long long>& values, size_t count)
	{
		values.clear();
		values.reserve(count);

		if (stream.offset + count * 8 > stream.datasize)
			throw ARL::runtime_error("Read offset is out of bounds while reading %d bytes", (int)count * 8);
		...
		for (size_t i = 0; i < count; ++i)
		{
			...
			unsigned char v2 = stream.data[stream.offset + count * 2 + i];
			unsigned char v3 = stream.data[stream.offset + count * 3 + i];
			unsigned char v4 = stream.data[stream.offset + count * 4 + i];
			unsigned char v5 = stream.data[stream.offset + count * 5 + i];
			unsigned char v6 = stream.data[stream.offset + count * 6 + i];
			unsigned char v7 = stream.data[stream.offset + count * 7 + i];
		...
		stream.offset += count * 8;
	}

	static void readUInt8Vector(MemoryInputStream& stream, std::vector<unsigned int>& values, size_t count)
	{
		values.clear();
		values.reserve(count);

		if (stream.offset + (count * 3) > stream.datasize)
			throw ARL::runtime_error("Read offset is out of bounds while reading %d bytes", (int)(count * 3));
		...
		for (size_t i = 0; i < count; ++i)
		{
			...
			values.push_back((v0 << 16) | (v1 << 8) | v2);
		}
	...
	static void writeUInt8Vector(MemoryOutputStream& stream, const std::vector<unsigned int>& values)
	{
		for (size_t i = 0; i < values.size(); ++i)
		{
			writeRaw(stream, (unsigned char)(values[i] >> 16));
	...
	void readPropertyValues(MemoryInputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<Instance*>& instances, const std::vector<shared_ptr<Instance> >& idMap)
	{
		...
		else if (desc.type == Reflection::Type::singleton<int>())
		{
			...
			else
			{
				if (format != bpfInt)
				{
					throw ARL::runtime_error("Unexpected format %d (expected %d)", format, bpfInt);
				}
		...
		else if (desc.type == Reflection::Type::singleton<G3D::Color3uint8>())
		{
			...
			readUInt8Vector(stream, values, instances.size());

			for (size_t i = 0; i < instances.size(); ++i)
				setPropertyValue(desc, instances[i], G3D::Color3uint8::fromARGB(values[i]));
		}
	...
	void writePropertyValues(MemoryOutputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<const Instance*>& instances, const std::map<const Instance*, int>& idMap, unsigned int flags)
	{
		...
		else if (desc.type == Reflection::Type::singleton<G3D::Color3uint8>())
		{
			...
			for (size_t i = 0; i < instances.size(); ++i)
			{
				...
				values.push_back(value.asUInt32());
			}

			writeRaw(stream, (char)bpfColor3uint8);
			writeUInt8Vector(stream, values);
		}
		else if (desc.type == Reflection::Type::singleton<G3D::Rect2D>())
		{
			std::vector<float> x0;
