...
local QUOTES = {
	"It's gnome spotting season. Get your binoculars, it's time.",
	"YOUR TOAST HAS BEEN BURNT AND NO AMOUNT OF SCRAPING WILL REMOVE THE BLACK PART",
	"Very much loading... Give me a bit...",
	"My name is Michael J Caboose, and I ... HATE ... TAXES!",
	"Bow Chicka Honk Honk",
	"Do you ever wonder why we're here?",
	"HAAAAAAAAAAAAAAAAANNNKKKKKK",
	"Looking snazzy! Or not...",
	"Hey have you- Never mind you don't look fit for that.",
	"Oh what's this game you're joining? Seems like ass.",
	"Are you seriously thinking about using DARK MODE. ON THIS??",
	"Do you like how I walk?",
	"Do you like how I talk?",
	"Do you like how my face disintegrates into chalk?",
	"You better be entering this game with only a spoonful!",
	"You musn't be afraid to dream a little bigger, darling!",
	"slendytubbiesscream.mp3",
	"Did you know that the background music here is from 'GU-L'?",
	"Screaming_chicken_on_tree_meme.mp4",
	"If it compiles, we ship it",
	"that looks like some anime filler",
	"what in the sweet onion on teriyaki is that",
	"im gonna box you like a fish",
	"can we recharge our HEV suits together haha",
	"im gonna boopity and boppity right in your bippity",
	"i'm gonna go star grind in the bathroom while this loads",
	"you can go kill yourself and die when im on Xbox live",
	"KILL YOURSELF, DONT BE YOURSELF! - kendrick lamar",
	"go look at my instagram bio you republican pig",
	"I'm just a guy that loves adventure",
	"Oh hi. So, how are you holding up? BECAUSE I'M A POTATO",
	"We only hire godlike coders from inner depths of Romania",
	"uh, uhm, i uh uh",
	"We sell data to the CCP!",
	"Rice cakes, never had one but somebody told me this is a really good way to start a diet",
}
...
if isDarkModeEnabled then
	IMAGES.GAME_BACKGROUND = "arlasset://textures/loading/darkLoadingTexture.png"
end
...
local function randomNumber()
	wait(0.25)
	return math.floor(math.random(10, 100000))
end
...
function MainGui:GenerateMain()
	...
	local mainBackgroundContainer = create 'Frame' {
		...
		create 'ImageButton' {
			Name = 'CloseButton',
			Image = 'arlasset://textures/ui/CloseButton.png',
			BackgroundTransparency = 1,
	...
	local thumbnailContainer = create 'Frame' {
		...
		create 'Frame' {
			...
			create 'TextLabel' {
				...
				Text = 'There seems to be a missing quote here...',
				TextColor3 = COLORS.LIGHTMED,
				TextStrokeColor3 = COLORS.DARKMED2,
				TextStrokeTransparency = 0,
				TextScaled = true,
...
renderSteppedConnection = Game:GetService("RunService").RenderStepped:connect(function()
	...
	if currScreenGui:FindFirstChild("ThumbnailContainer") and currScreenGui.ThumbnailContainer:FindFirstChild("LoadingInfoContainer") and currScreenGui.ThumbnailContainer.LoadingInfoContainer:FindFirstChild("Quote") then
		currScreenGui.ThumbnailContainer.LoadingInfoContainer.Quote.Text = currentQuote
	end
...
sound.Looped = true
sound.SoundId = "arlasset://sounds/loading/gu-l_menu.mp3"
sound.Volume = 1
wait(0.5)
sound:Play()
...
function removeLoadingScreen()
	...
	if currScreenGui then
		...
		--currScreenGui = nil
		currScreenGui.MainBackgroundContainer.BackgroundTransparency = 1
		currScreenGui.IgnoreGuiInset = false
		
		for _, v in pairs(currScreenGui:GetDescendants()) do
			if v.Name ~= "MainBackgroundContainer" and v.Name ~= "ErrorFrame" and v.Name ~= "ErrorText" and v.Parent and v.Parent.Name ~= "ErrorFrame"  then
				v:Destroy()
			end
		end
		
		currScreenGui.MainBackgroundContainer.Active = false
	end
