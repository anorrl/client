...
{
	int crash = 0;
	if (ARL::RegistryUtil::read32bitNumber("HKEY_CURRENT_USER\\Software\\kuro.remover\\ANORRL\\InferredCrash", crash)) 
	{
	...
	ARL::RegistryUtil::write32bitNumber("HKEY_CURRENT_USER\\Software\\kuro.remover\\ANORRL\\InferredCrash", 1);
}
...
bool Application::LoadAppSettings(HINSTANCE hInstance)
{
	try {
		...
		CRegKey key;
		bool validRegKey = (key.Open(HKEY_CURRENT_USER, "Software\\kuro.remover\\ANORRL", KEY_READ) == ERROR_SUCCESS);
		if (validRegKey)
		{
			DWORD value = TRUE;
...
void Application::Shutdown()
{
	...
	ARL::RegistryUtil::write32bitNumber("HKEY_CURRENT_USER\\Software\\kuro.remover\\ANORRL\\InferredCrash", 0);
