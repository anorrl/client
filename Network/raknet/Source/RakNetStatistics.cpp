...
void RAK_DLL_EXPORT RakNet::StatisticsToString(const RakNetStatistics *s, char *buffer, int verbosityLevel )
{
	...
	{
		sprintf(buffer,
			"Bytes per second sent     %u\n"
			"Bytes per second received %u\n"
	...
	{
		sprintf(buffer,
			"Actual bytes per second sent       %u\n"
			"Actual bytes per second received   %u\n"
			"Message bytes per second pushed    %u\n"
			"Total actual bytes sent            %u\n"
			"Total actual bytes received        %u\n"
			"Total message bytes pushed         %u\n"
			"Current packetloss                 %.1f%%\n"
			"Average packetloss                 %.1f%%\n"
			"Elapsed connection time in seconds %u\n",
			(long long unsigned int) s->valueOverLastSecond[ACTUAL_BYTES_SENT],
			(long long unsigned int) s->valueOverLastSecond[ACTUAL_BYTES_RECEIVED],
		...
		if (s->BPSLimitByCongestionControl!=0)
		{
			char buff2[128];
			sprintf(buff2,
				"Send capacity                    %u bytes per second (%.0f%%)\n",
	...
	else
	{
		sprintf(buffer,
			"Actual bytes per second sent         %u\n"
			"Actual bytes per second received     %u\n"
			"Message bytes per second sent        %u\n"
			"Message bytes per second resent      %u\n"
			"Message bytes per second pushed      %u\n"
			"Message bytes per second processed   %u\n"
			"Message bytes per second ignored     %u\n"
			"Total bytes sent                     %u\n"
			"Total bytes received                 %u\n"
			"Total message bytes sent             %u\n"
			"Total message bytes resent           %u\n"
			"Total message bytes pushed           %u\n"
			"Total message bytes received         %u\n"
			"Total message bytes ignored          %u\n"
			"Messages in send buffer, by priority %i,%i,%i,%i\n"
			"Bytes in send buffer, by priority    %i,%i,%i,%i\n"
			"Messages in resend buffer            %i\n"
			"Bytes in resend buffer               %u\n"
			"Current packetloss                   %.1f%%\n"
			"Average packetloss                   %.1f%%\n"
			"Elapsed connection time in seconds   %u\n",
			(long long unsigned int) s->valueOverLastSecond[ACTUAL_BYTES_SENT],
