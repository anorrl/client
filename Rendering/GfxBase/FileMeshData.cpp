...
namespace ARL
{
    ...
    static shared_ptr<FileMeshData> readMeshFromBinary(const std::string& data, size_t offset, unsigned int formatMask)
    {
        shared_ptr<FileMeshData> mesh(new FileMeshData());

        unsigned int vertcount;

        if (formatMask & MESH_V4_00)
        {
            FileMeshHeaderV4 header;
            readData(data, offset, &header.cbSize, sizeof(header.cbSize));

            if ((formatMask & MESH_V4_00) && !(formatMask & MESH_V5_00))
            {
                if (header.cbSize != (sizeof(FileMeshHeaderV4) - sizeof(unsigned int) * 2))
                    throw std::runtime_error("Error reading mesh data: incompatible stride");

                offset -= sizeof(header.cbSize);
                readData(data, offset, &header, (sizeof(header) - sizeof(unsigned int) * 2));
            }
            else if (formatMask & MESH_V5_00)
            {
                if (header.cbSize != sizeof(FileMeshHeaderV4))
                    throw std::runtime_error("Error reading mesh data: incompatible stride");
            ...
            vertcount = header.num_vertices;

            mesh->vnts.resize(header.num_vertices);
            readData(data, offset, &mesh->vnts[0], header.num_vertices * sizeof(FileMeshVertexNormalTexture3d));

            if (header.num_joints > 0)
            {
                mesh->envelopes.resize(header.num_vertices);
                readData(data, offset, &mesh->envelopes[0], header.num_vertices * sizeof(FileMeshEnvelope));
            }

            mesh->faces.resize(header.num_faces);
            readData(data, offset, &mesh->faces[0], header.num_faces * sizeof(FileMeshFace));

            mesh->LODOffsets.resize(header.num_LODOffsets);
            // TODO: Implement LODs.
            readData(data, offset, &mesh->LODOffsets[0], header.num_LODOffsets * sizeof(unsigned int));
            mesh->faces.resize(mesh->LODOffsets[1]);

            mesh->bones.resize(header.num_joints);
            readData(data, offset, &mesh->bones[0], header.num_joints * sizeof(FileMeshBone));

            mesh->boneNames.resize(header.num_jointNameChars);
            readData(data, offset, &mesh->boneNames[0], header.num_jointNameChars * sizeof(char));

            mesh->subsets.resize(header.num_skinningSubsets);
            readData(data, offset, &mesh->subsets[0], header.num_skinningSubsets * sizeof(FileMeshSubset));

            if (formatMask & MESH_V5_00)
            {
                mesh->facsData.resize(header.num_facsData);
                readData(data, offset, &mesh->facsData[0], header.num_facsData);
            }
        }
        else
        {   // Legacy Mesh
            FileMeshHeaderLOD header;
            readData(data, offset, &header.cbSize, sizeof(header.cbSize));

            if ((formatMask & MESH_V2_00) && !(formatMask & MESH_V3_00))
            {
                if (header.cbSize != (sizeof(FileMeshHeaderLOD) - sizeof(header.cbLODStride) - sizeof(header.num_LODOffsets)))
                    throw std::runtime_error("Error reading mesh data: incompatible stride");

                readData(data, offset, &header.cbVerticesStride, sizeof(header.cbVerticesStride));
                readData(data, offset, &header.cbFaceStride, sizeof(header.cbFaceStride));
                header.cbLODStride = header.num_LODOffsets = 0;
                readData(data, offset, &header.num_vertices, sizeof(header.num_vertices));
                readData(data, offset, &header.num_faces, sizeof(header.num_faces));
            }
            else if (formatMask & MESH_V3_00)
            {
                if (header.cbSize != sizeof(FileMeshHeaderLOD))
                    throw std::runtime_error("Error reading mesh data: incompatible stride");
            ...
            if (header.cbFaceStride != sizeof(FileMeshFace))
                throw std::runtime_error("Error reading mesh data: incompatible stride");
            ...
            {
                readData(data, offset, &mesh->vnts[i], header.cbVerticesStride);

                if (header.cbVerticesStride != 40)
                    mesh->vnts[i].r = mesh->vnts[i].g = mesh->vnts[i].b = mesh->vnts[i].a = (char)(255);
            }

            mesh->faces.resize(header.num_faces);
            readData(data, offset, &mesh->faces[0], header.num_faces * header.cbFaceStride);

            if (formatMask & MESH_V3_00)
            {
                mesh->LODOffsets.resize(header.num_LODOffsets);
                // TODO: Implement LODs.
                readData(data, offset, &mesh->LODOffsets[0], header.num_LODOffsets * header.cbLODStride);
                mesh->faces.resize(mesh->LODOffsets[1]);
            }
            else
            {
                mesh->LODOffsets.resize(2);
                mesh->LODOffsets.push_back(0);
                mesh->LODOffsets.push_back(header.num_faces);
            }
        }
        ...
        for (auto& face : mesh->faces)
            if (face.a >= vertcount || face.b >= vertcount || face.c >= vertcount)
...
namespace ARL
{
    shared_ptr<FileMeshData> ReadFileMesh(const std::string& data)
    {
        ...
        if (data.compare(0, 12, "version 2.00") == 0)
            meshFormat |= MESH_V2_00;
        if (data.compare(0, 12, "version 3.00") == 0 || data.compare(0, 12, "version 3.01") == 0) // 3.01 has zero change, seriously...
            meshFormat |= MESH_V2_00 | MESH_V3_00;
        if (data.compare(0, 12, "version 4.00") == 0 || data.compare(0, 12, "version 4.01") == 0) // Same deal as 3.01 but the unused padding at the end of the header changes.
            meshFormat |= MESH_V2_00 | MESH_V3_00 | MESH_V4_00;
        if (data.compare(0, 12, "version 5.00") == 0)
            meshFormat |= MESH_V2_00 | MESH_V3_00 | MESH_V4_00 | MESH_V5_00;

        if (!result && meshFormat == 0)
            throw std::runtime_error("Error reading mesh data: unknown version");

        if (meshFormat & MESH_V2_00)
        {
            result = readMeshFromBinary(data, versionEnd + 1, meshFormat);
        }
        else if (!result)
        {
            throw std::runtime_error("Error reading mesh data: unknown version");
    ...
    {
        f << "version 3.01" << std::endl;

        FileMeshHeaderLOD header;
        header.num_faces = data.faces.size();
        header.num_vertices = data.vnts.size();
        header.num_LODOffsets = data.LODOffsets.size();
        header.cbLODStride = sizeof(data.LODOffsets[0]);
        ...
        f.write(reinterpret_cast<const char*>(&data.LODOffsets[0]), sizeof(data.LODOffsets[0]) * data.LODOffsets.size());
    }
}
