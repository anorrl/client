...
namespace arl
{
	...
	*/
	...
	/*
		...
		to use boost::intrusive_ptr and arl::intrusive_weak_ptr. If you don't need
...
namespace boost
{
	...
	template<class T, typename Count, Count maxRefs>
	void intrusive_ptr_add_ref(const arl::quick_intrusive_ptr_target<T, Count, maxRefs> * p);
	template<class T, typename Count, Count maxRefs>
    void intrusive_ptr_release(const arl::quick_intrusive_ptr_target<T, Count, maxRefs> * p);

	/// Template Specialization for boost intrusive ptrs for intrusive_ptr_target
	template<class T, typename Count, Count maxStrong, Count maxWeak>
    void intrusive_ptr_add_ref(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak> * p);
	template<class T, typename Count, Count maxStrong, Count maxWeak>
    void intrusive_ptr_release(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak> * p);
	template<class T, typename Count, Count maxStrong, Count maxWeak>
    bool intrusive_ptr_expired(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak> * p);
	template<class T, typename Count, Count maxStrong, Count maxWeak>
    bool intrusive_ptr_try_lock(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak> * p);
	template<class T, typename Count, Count maxStrong, Count maxWeak>
    void intrusive_ptr_add_weak_ref(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak> * p);
	template<class T, typename Count, Count maxStrong, Count maxWeak>
    void intrusive_ptr_weak_release(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak> * p);
}
...
#pragma pack(8)	// Packing is useful if Count is short or byte
		struct counts
		{
			arl::atomic<Count> strong;   // #shared
			arl::atomic<Count> weak;     // #weak + (#shared != 0)
			counts()
			{
				strong = 0;
				weak = 1;
...
namespace boost
{
	...
	void intrusive_ptr_add_ref(const arl::quick_intrusive_ptr_target<T, Count, maxRefs>* p)
	{
		if (maxRefs > 0 && p->refs >= maxRefs)
			throw arl::too_many_refs();

		const_cast<arl::quick_intrusive_ptr_target<T, Count, maxRefs>*>(p)->refs++;
	}
	...
	{
		ARLASSERT(p->refs > 0);
		if (--(const_cast<arl::quick_intrusive_ptr_target<T, Count, maxRefs>*>(p)->refs) == 0)
	...
	void intrusive_ptr_add_ref(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak>* p)
	{
		typename arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak>::counts* c = arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak>::fetch(static_cast<const T*>(p));
	...
	void intrusive_ptr_release(const arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak>* p)
	{
        ...
        typedef typename arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak>::counts Counts;
		Counts* c = arl::intrusive_ptr_target<T, Count, maxStrong, maxWeak>::fetch(t);
