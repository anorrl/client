...
ARL_SIMD_INLINE v4f load3( const float* s );
...
ARL_SIMD_INLINE typename VectorType::elem_t extractSlow( const VectorType& v, uint32_t i );
...
// r = {v[a],v[b],v[c],v[d]}
template< uint32_t a, uint32_t b, uint32_t c, uint32_t d, class VectorType >
ARL_SIMD_INLINE VectorType permute( const VectorType& v );
...
ARL_SIMD_INLINE VectorType rotateLeft( VectorType& v );
...
#if defined( ARL_SIMD_USE_SSE )
#include "simd/simd_sse.inl"
#elif defined( ARL_SIMD_USE_NEON )
