...
QStringList AutoSaveDialog::getFileList()
{
    ...
    const QStringList entries = dir.entryList(QStringList() << "*.arl");
    if ( entries.empty() )
        return list;
...
void AutoSaveDialog::onOpen()
{
    ...
    if ( !fileName.isEmpty() )
    {
        ...
        bool openResult = UpdateUIManager::Instance().getMainWindow().handleFileOpen(fileName,IANORRLDoc::IDE);
        if ( openResult )
        {
			FASTLOG(FLog::AutoSave, "File opened successfully");
            ANORRLDocManager::Instance().getPlayDoc()->setAutoSaveLoad();
        }
        else
        {
