...
static const char* updatedLayoutXmlConfigFile = "ANORRLStudioRibbon.xml";
...
void ANORRLRibbonMainWindow::setupRibbonBar()
{
	if (m_bInitialized)
		throw ARL::runtime_error("Ribbon bar already initialized.");

	//create ribbonbar
	QString xmlFilePath(ANORRLSettings::getResourcesFolder());
	xmlFilePath.append("/");
    xmlFilePath.append(updatedLayoutXmlConfigFile);

	QFile file(xmlFilePath);
	if (!file.open(QIODevice::ReadOnly))
		throw ARL::runtime_error("Failed to open configuration XML file.");

	QDomDocument doc("appRibbonBar");
	if (!doc.setContent(&file)) 
		throw ARL::runtime_error("Failed to parse configuration XML file.");
...
RibbonPage* ANORRLRibbonMainWindow::createRibbonPage(const QString& objectName, const QString& pageTitle, int indexPosition)
{
...
RibbonGroup* ANORRLRibbonMainWindow::createRibbonGroup(RibbonPage* pParent, const QString& objectName, const QString& groupTitle)
{
	if (!pParent)
		return NULL;
	...
	ARLASSERT(pRibbonGroup);
	pRibbonGroup->setObjectName(objectName);

	return pRibbonGroup;
}

void ANORRLRibbonMainWindow::parseAndCreateQuickAccessBar(const QDomElement &docElem)
{
	...
	bool configLoaded = ANORRLQuickAccessConfig::singleton().loadQuickAccessConfig();
	
	ANORRLSettings settings;
	// parse data from XML 
	ANORRLQuickAccessConfig::QuickAccessActionCollection defaultQuickAccessCollection;
		...
		if (!actionName.isEmpty())
		{
			...
			if (pChildAction)
			{
				...
				if (!configLoaded)
				{
					...
					ANORRLQuickAccessConfig::singleton().addToQuickAccessBar(pChildAction, actVisible);
				}

				// make a list of actions defined in XML config for restore
				defaultQuickAccessCollection.append(ANORRLQuickAccessConfig::ActionData(pChildAction, actVisible, true));
			}
		}
	...
	ANORRLQuickAccessConfig::singleton().storeDefaults(defaultQuickAccessCollection);
	connect(customizeQuickAccessAction, SIGNAL(triggered()), this, SLOT(customizeQuickAccess()));
}

void ANORRLRibbonMainWindow::updateInternalWidgetsState(QAction* pAction, bool enabledState,bool checkedState)
{
...
QStringList ANORRLRibbonMainWindow::getRecentFiles() const
{	return ANORRLSettings().value(settingRecentFiles).toStringList(); }
 ...
 */
void ANORRLRibbonMainWindow::setCurrentDirectory(const QString& path)
{
    ...
    // save into settings
    ANORRLSettings().setValue(settingLastDirectory, m_LastDirectory);
}

void ANORRLRibbonMainWindow::updateRecentFile(const QString& fileName)
{
	//update the recent files
	if(!fileName.isEmpty()
       && (fileName.endsWith(".arl", Qt::CaseInsensitive) || fileName.endsWith(".arlx", Qt::CaseInsensitive))
       && !(fileName.endsWith("visit.arl", Qt::CaseSensitive))
       && !(fileName.endsWith("server.arl", Qt::CaseSensitive)))
	{
		ANORRLSettings settings;
...
void ANORRLRibbonMainWindow::actionEvent(QActionEvent* evt)
{
...
void ANORRLRibbonMainWindow::parseAndCreateSystemButton(const QDomElement &docElem)
{
...
void ANORRLRibbonMainWindow::parseAndCreateRibbonBar(const QDomElement &docElem)
{
...
void ANORRLRibbonMainWindow::parseAndCreateChildren(const QDomElement& tabPageElement, QObject* pParent)
{
	...
	{		
		...
		if (!flagName.isEmpty())
		{
			try
			{
				...
				bool keepItem = ARL::GlobalAdvancedSettings::singleton()->getFFlag(flagName.toStdString());
				if (negate)
					keepItem = !keepItem;
		...
		else if (domElementTagName == elemAddAction)
		{
			...
			{
				throw ARL::runtime_error("Invalid action type: %s", actionType.toStdString().c_str());
			}
		}
...
RibbonPage* ANORRLRibbonMainWindow::parseAndCreateRibbonPage(const QDomElement& domElement)
{
	// See if it exists first
	QString objectName = getName(domElement);
	ARLASSERT(!objectName.isEmpty());
...
Qtitan::RibbonGroup* ANORRLRibbonMainWindow::parseAndCreateRibbonGroup(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonPage* pParentPage = dynamic_cast<Qtitan::RibbonPage*>(pParent);
	if (!pParentPage)
		throw ARL::runtime_error("Group parent must be a RibbonPage!");
...
QWidget* ANORRLRibbonMainWindow::parseAndCreateColorPicker(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("Menu buttons must have a ribbon group parent element!");

	QString text = getText(domElement);
	QIcon icon   = getIcon(domElement);
	if (icon.isNull() && text.isEmpty())
		throw ARL::runtime_error("Colorpicker must have an icon or text!");
    ...
    if (!pAction)
        throw ARL::runtime_error("Failed to create action: %s", getName(domElement).toStdString().c_str());
...
QMenu* ANORRLRibbonMainWindow::parseAndCreateMenuPopupButton(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("Menu buttons must have a ribbon group parent element!");
	...
	QIcon icon = getIcon(domElement);
	if (icon.isNull() && text.isEmpty())
		throw ARL::runtime_error("Drop downs must have an icon or text!");
    ...
    {
        value = ANORRLSettings().value(settingName, value).toString();
    }
         ...
         iter++)
    {
        if ((*iter)->objectName() == value)
        {
            ...
            if (!settingName.isEmpty())
            {
                ANORRLSettings().setValue(settingName, value);
            }
        }
    }
...
QMenu* ANORRLRibbonMainWindow::parseAndCreateMenu(const QDomElement& domElement, QObject* pParent)
{	
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("Menu buttons must have a ribbon group parent element!");
...
Qtitan::RibbonGallery* ANORRLRibbonMainWindow::parseAndCreateRibbonGallery(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("Gallery parent must be a ribbon group!");
	...
	ARLASSERT(pGalleryGroup->getItemCount());
...
Qtitan::RibbonGalleryItem* ANORRLRibbonMainWindow::parseAndCreateGalleryItem(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGalleryGroup* pGalleryGroup = qobject_cast<Qtitan::RibbonGalleryGroup*>(pParent);
	if (!pGalleryGroup)
		throw ARL::runtime_error("galleryitem must be beneath a gallery group");
...
QActionGroup* ANORRLRibbonMainWindow::parseAndCreateActionGroup(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	QMenu* pMenu                = qobject_cast<QMenu*>(pParent);
	if (!pGroup && !pMenu)
		throw ARL::runtime_error("actiongroup elements must be beneath a group or menu");
...
QButtonGroup* ANORRLRibbonMainWindow::parseAnCreateButtonGroup(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("buttongroup must have a ribbon group parent element!");
...
QAction* ANORRLRibbonMainWindow::parseAndCreateSplitButton(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("splitbutton must have a ribbon group parent element!");
...
QAction* ANORRLRibbonMainWindow::parseAndCreateSeparator(const QDomElement& domElement, QObject* pParent)
{
	...
	throw ARL::runtime_error("addseparator elements must be beneath a menu or group!");
}

QRadioButton* ANORRLRibbonMainWindow::parseAndCreateRadioButton(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	QButtonGroup*  pButtonGroup       = qobject_cast<QButtonGroup*>(pParent);
	if (!pRibbonGroup && !pButtonGroup)
		throw ARL::runtime_error("radio buttons must be beneath a button/ribbon group");

	QString objectName = getName(domElement);
	if (objectName.isEmpty())
		throw ARL::runtime_error("radio button name can not be empty!");
...
QCheckBox* ANORRLRibbonMainWindow::parseAndCreateCheckBox(const QDomElement& domElement, QObject* pParent, bool actionMode)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("checkbox must be beneath a ribbon group");

	QString objectName = getName(domElement);
	if (objectName.isEmpty())
		throw ARL::runtime_error("checkbox name element can not be empty!");
...
QComboBox* ANORRLRibbonMainWindow::parseAndCreateComboBox(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("checkbox must be beneath a ribbon group");
...
QAction* ANORRLRibbonMainWindow::parseAndCreateRibbonAction(const QDomElement& domElement, QObject* pParent)
{
	if (!pParent)
		throw ARL::runtime_error("Failed to create action: %s.  No parent element provided!", getName(domElement).toStdString().c_str());
	...
	// instead of marking them as "iscommonslot=true" in the ANORRLStudioRibbon.xml file.
...
QMenu* ANORRLRibbonMainWindow::parseAndCreateGroupOptionsMenu(const QDomElement& domElement, QObject* pParent)
{
	Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
	if (!pRibbonGroup)
		throw ARL::runtime_error("optionsMenu must be beneath a ribbon group");

	QAction* optionButtonAction = pRibbonGroup->getOptionButtonAction();
	if (!optionButtonAction)
		throw ARL::runtime_error("optionsMenu must has an action to associate");
...
QMenu* ANORRLRibbonMainWindow::parseAndCreateSubMenu(const QDomElement& domElement, QObject* parent)
{
	QMenu* menuParent = qobject_cast<QMenu*>(parent);
	if (!menuParent)
		throw ARL::runtime_error("submenu must be beneath a QMenu");
...
void ANORRLRibbonMainWindow::showGroupOptionsMenu()
{
...
void ANORRLRibbonMainWindow::updateProxyAction()
{
...
///// Utility functions /////
QString ANORRLRibbonMainWindow::getName(const QDomElement& domElement) const
{	return domElement.attribute(attribName); }

QString ANORRLRibbonMainWindow::getToolTip(const QDomElement& domElement) const
{	
...
QString ANORRLRibbonMainWindow::getText(const QDomElement& domElement) const
{	
	if (!domElement.attribute(attribText).isEmpty())
		return domElement.attribute(attribText); 
...
QSize ANORRLRibbonMainWindow::getSize(const QDomElement& domElement, const QSize& defaultSize) const
{
...
QIcon ANORRLRibbonMainWindow::getIcon(const QDomElement& domElement, const QSize& defaultSize) const
{
...
Qt::ToolButtonStyle ANORRLRibbonMainWindow::getStyle( const QDomElement& domElement ) const
{
...
enum QToolButton::ToolButtonPopupMode ANORRLRibbonMainWindow::getPopupStyle(const QDomElement& domElement) const
{
...
QAction* ANORRLRibbonMainWindow::getAction(const QDomElement& domElement)
{
	QString objectName = getName(domElement);
	if (objectName.isEmpty())
		return NULL;
	...
	if (domElement.attribute(attribType) == "plugin")
	{
		ANORRLQuickAccessConfig& quickAccessConfig = ANORRLQuickAccessConfig::singleton();
...
void ANORRLRibbonMainWindow::addWidgetToRibbonGroup(Qtitan::RibbonGroup* pRibbonGroup, QWidget* pWidget)
{
...
void ANORRLRibbonMainWindow::addButtonToButtonGroup(QButtonGroup* pButtonGroup, QAbstractButton* pButton)
{	
	// add button the ribbon group
	pButtonGroup->addButton(pButton);		
}

void ANORRLRibbonMainWindow::addActionToParent(QAction* pAction, QObject* pParent, const QDomElement& domElement)
{
	if ( Qtitan::RibbonGallery* pRibbonGallery = qobject_cast<Qtitan::RibbonGallery*>(pParent) )
	{
		...
		if ( !pRibbonGalleryGroup )
			throw ARL::runtime_error("Failed to create action: %s. RibbonGallery must have RibbonGalleryGroup!", qPrintable(pAction->objectName()));
...
///// Slots /////
void ANORRLRibbonMainWindow::handleActionGroupSelected(QAction *pAction)
{
	...
	if (ANORRLDocManager::Instance().getCurrentDoc())
	{
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pActionGroup->objectName(), pAction->isChecked());
	}
}

void ANORRLRibbonMainWindow::handleButtonGroupSelected(QAbstractButton* pButton)
{
	...
	{		
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pButtonGroup->objectName(), pButton->isChecked());
	}
}

void ANORRLRibbonMainWindow::handleActionSelected(QAction* pAction)
{
    ...
    {
        ANORRLSettings().setValue(pMenu->settingName, pAction->objectName());
    }
	...
	if (ANORRLDocManager::Instance().getCurrentDoc())
	{
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pMenu->objectName(), pAction->isChecked());
		UpdateUIManager::Instance().updateToolBars();
	}
}

void ANORRLRibbonMainWindow::handleMenuActionTrigger(QAction* pAction)
{
	...
	{		
		...
		{
			ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pAction->objectName(), pAction->isChecked());
			UpdateUIManager::Instance().updateToolBars();
		}
		else
		{
...
void ANORRLRibbonMainWindow::handleBasicObjectsGalleryItemClicked(RibbonGalleryItem* item)
{
	...
	{		
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pGallery->objectName(), false);
	}
}

void ANORRLRibbonMainWindow::handleCheckBoxClicked(bool state)
{
	...
	if (ANORRLDocManager::Instance().getCurrentDoc())
	{
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pCheckBox->objectName(), false);
	}
}

void ANORRLRibbonMainWindow::handleComboBoxActivated(int index)
{
...
void ANORRLRibbonMainWindow::handleItemSelected(const QString& selectedItem)
{
	...
	if (ANORRLDocManager::Instance().getCurrentDoc())
	{
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(item->objectName(), false);
		UpdateUIManager::Instance().updateToolBars();
	}
}

void ANORRLRibbonMainWindow::handleToolButtonClicked()
{
	QToolButton* item = qobject_cast<QToolButton*>(sender());
...
void ANORRLRibbonMainWindow::handleColorPickMenu()
{
	...
	if (selectedColor != -1 && ANORRLDocManager::Instance().getCurrentDoc())
	{
		...
		ANORRLDocManager::Instance().getCurrentDoc()->handleAction(pButton->objectName(), false);
		UpdateUIManager::Instance().updateToolBars();
	}
...
void ANORRLRibbonMainWindow::handleRibbonMinimizationChanged(bool minimized)
{
	...
	ANORRLSettings().setValue(settingRibbonMinimized, minimized);	
}

void ANORRLRibbonMainWindow::toggleRibbonMinimize()
{	
	ribbonBar()->setMinimized(!ribbonBar()->isMinimized()); 
}

void ANORRLRibbonMainWindow::handleStyleChange(QAction* pAction)
{
    if (!m_pRibbonStyle)
		return;
...
void ANORRLRibbonMainWindow::initialize()
{
	cleanupToolBars();
	...
	ARLASSERT(!loadedFontFamilies.empty());
...
void ANORRLRibbonMainWindow::cleanupToolBars()
{
...
void ANORRLRibbonMainWindow::updateFonts(const QFont& font)
{
...
void ANORRLRibbonMainWindow::customizeQuickAccess()
{
...
void ANORRLRibbonMainWindow::createThemeOptions()
{
...
QAction* ANORRLRibbonMainWindow::findChildAction(const QString& actionName) const
{	return findChild<QAction*>(actionName); }

void ANORRLRibbonMainWindow::populateNameValueStore(const QString& storeID, const QObject *pObject)
{
...
QString ANORRLRibbonMainWindow::getFullName(QObject* pObject)
{
	QString fullName;
