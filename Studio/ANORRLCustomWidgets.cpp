...
, m_pModelInstance(instance)
{
	...
	ARL::Workspace* workspace = ANORRLDocManager::Instance().getPlayDoc()->getDataModel()->getWorkspace();
	...
	ANORRLTreeWidget* pTreeWidget = UpdateUIManager::Instance().getViewWidget<ANORRLExplorerWidget>(eDW_OBJECT_EXPLORER).getTreeWidget();
	if (pTreeWidget)
...
{
	if ((evt->type() == QEvent::ActionChanged) && (ANORRLDocManager::Instance().getPlayDoc()))
	{
...
void PrimaryPartSelectionEditor::onPartSelected(ARL::PartInstance* pPart)
{
	...
	QApplication::postEvent(this, new ANORRLCustomEvent(PRIMARY_PART_SELECTED));
}
...
bool PrimaryPartSelectionEditor::setPrimaryPart(ARL::PartInstance* pPart)
{
	if (pPart && pPart->isDescendantOf(m_pModelInstance.get()))
	{
		...
		ARLASSERT(ANORRLDocManager::Instance().getPlayDoc() && ANORRLDocManager::Instance().getPlayDoc()->getDataModel());
		ARL::DataModel::LegacyLock lock(ANORRLDocManager::Instance().getPlayDoc()->getDataModel(), ARL::DataModelJob::Write);
