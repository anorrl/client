...
ANORRLIDEDoc::ANORRLIDEDoc(ANORRLMainWindow* pMainWindow)
...
ANORRLIDEDoc::~ANORRLIDEDoc()
{
    clearPlayModifiedScriptList();
}

void ANORRLIDEDoc::applyScriptChanges()
{
	// Get all scripts and apply changes
	const tScriptDocs& openScriptDocs = ANORRLDocManager::Instance().getOpenScriptDocs();
	tScriptDocs::const_iterator iter = openScriptDocs.begin();
	for (; iter != openScriptDocs.end() ; ++iter)
		(*iter)->applyEditChanges();
}

void ANORRLIDEDoc::InitializeDefaultsFromSettings()
{
...
void ANORRLIDEDoc::createOptionalDMServices()
{
...
void ANORRLIDEDoc::displayWorkspaceMessage()
{
	...
	if (DFInt::StudioWorkspaceNotificationLevel && DFFlag::MaterialPropertiesEnabled )
	{
		...
		if ( messageEnabled)
		{
			// Should only display if ANORRL user hasn't set his play to Legacy Mode
...
void ANORRLIDEDoc::initializeNewPlace()
{
	...
	if (DFFlag::MaterialPropertiesEnabled)
	{
		...
		m_workspacePropertyChangedConnection = getEditDataModel()->getWorkspace()->propertyChangedSignal.connect(boost::bind(&ANORRLIDEDoc::onWorkspacePropertyChanged, this, _1));
	}
	...
	if (FFlag::CSGPhysicsLevelOfDetailEnabled)
	{
		...
		m_dataModelItemChangedConnection = getEditDataModel()->itemChangedSignal.connect(boost::bind(&ANORRLIDEDoc::onDMItemChanged, this, _1, _2));
	}
}

void ANORRLIDEDoc::initializeDataModeHash()
{
...
void ANORRLIDEDoc::initializeANORRLView()
{
    try
    {
        ARL::DataModel::LegacyLock lock(m_CurrentGame->m_Game->getDataModel(), ARL::DataModelJob::Write);

        m_GameView = ANORRLView::createGameView(m_pQOgreWidget);
        m_CurrentGame->m_View = new ANORRLView(m_pQOgreWidget,m_CurrentGame->m_Game, m_GameView);
...
bool ANORRLIDEDoc::open(ANORRLMainWindow *pMainWindow, const QString &fileName)
{
    m_pMainWindow = pMainWindow;
    return openFile(fileName, false);
}

bool ANORRLIDEDoc::openFile(const QString& fileName, bool asNew)
{
    if (!fileName.isEmpty())
    {
...
shared_ptr<EntityProperties> ANORRLIDEDoc::cloudEditDetectionAndPlaceLaunch()
{
	boost::optional<int> placeId;
	if ((ANORRLUser::singleton().getUserId() > 0) && StudioUtilities::containsEditScript(m_initializationScript))
	{
		...
		{
			EntityProperties p;
			std::string url = ANORRLSettings::getBaseURL().toStdString() +
...
bool ANORRLIDEDoc::openStream(const QString& fileName, std::istream* stream, bool asNew)
{
    ANORRLMainWindow *pMainWindow = m_pMainWindow;
	clearOutputWidget();
    ...
    ARL::Log::current()->writeEntry(ARL::Log::Information,"ANORRLIDEDoc::open - start");
	...
	try 
	{
		...
		{
			m_pDiagViewWidget = new ANORRLDiagnosticsView(true);
		...
		m_EditGame.m_TreeWidget = new ANORRLTreeWidget(m_EditGame.m_Game->getDataModel());

		UpdateUIManager::Instance().getViewWidget<ANORRLExplorerWidget>(eDW_OBJECT_EXPLORER).setCurrentWidget(m_EditGame.m_TreeWidget);
				...
				else if (ARL::ContentProvider::isUrl(m_initializationScript.toStdString()))
                {
					if (StudioUtilities::containsEditScript(m_initializationScript))
                    {
                        ...
                        m_contentDataLoadedConnection = m_EditGame.m_Game->getDataModel()->workspaceLoadedSignal.connect(boost::bind(
                                                                                                                                     &ANORRLIDEDoc::onContentDataLoaded,
                                                                                                                                     this));
						...
						if (StudioUtilities::isTestMode())
						{
							if (ARL::Network::Players* players = ARL::ServiceProvider::create<ARL::Network::Players>(getDataModel()))
							{
								m_localPlayerConnection = players->onDemandWrite()->childAddedSignal.connect( 
									                                                boost::bind(&ANORRLIDEDoc::onLocalPlayerAdded, this, _1) );
							}
						}
		...
		ANORRLPluginHost::loadPlugins(m_EditGame.m_Game->getDataModel().get());
	...
	if (m_initializationScript.isEmpty())
    {
        UpdateUIManager::Instance().getViewWidget<ANORRLGameExplorer>(eDW_GAME_EXPLORER).nonGameLoaded();
    }
    ...
    ARL::Log::current()->writeEntry(ARL::Log::Information,"ANORRLIDEDoc::open - end");
	return success;
}

void ANORRLIDEDoc::handleFileAction(FW::WatchID watchid, const FW::String& dir, const FW::String& filename, FW::Action action)
{
...
void ANORRLIDEDoc::exportSelection(std::string filePath, ExporterSaveType exportType)
{
	ARL::ViewBase* pView = (&m_CurrentGame->m_View->getViewBase());
...
void ANORRLIDEDoc::onDMItemChanged(shared_ptr<ARL::Instance> item, const ARL::Reflection::PropertyDescriptor* descriptor)
{
...
void ANORRLIDEDoc::enableUndo(bool enable)
{
    //undo redo manager.
    ...
    if (enable)
        m_CurrentGame->m_ChangeHistoryConnection = chs->waypointChangedSignal.connect(boost::bind(&ANORRLIDEDoc::onWaypointChanged,this));
    else
        m_CurrentGame->m_ChangeHistoryConnection.disconnect();
}

IANORRLDoc::ARLCloseRequest ANORRLIDEDoc::requestClose()
{
	...
	switch (shutdownRequestResult)
	{
		case ARL::DataModel::CLOSE_REQUEST_HANDLED:
			return IANORRLDoc::REQUEST_HANDLED;
		case ARL::DataModel::CLOSE_NOT_HANDLED:
			break;
		case ARL::DataModel::CLOSE_NO_SAVE_NEEDED:
            if (!m_TestModeEnabled)
                return IANORRLDoc::LOCAL_SAVE_NEEDED;
			return IANORRLDoc::NO_SAVE_NEEDED;
	...
	case KeepSavedChangesDialog::Fail:
		{                
			...
			msgBox.exec();
			return IANORRLDoc::CLOSE_CANCELED;
		}
...
bool ANORRLIDEDoc::doClose()
{
	...
	if (m_pQOgreWidget)
	{
	    m_pQOgreWidget->setANORRLView(NULL);
	...
	UpdateUIManager::Instance().getViewWidget<ANORRLExplorerWidget>(eDW_OBJECT_EXPLORER).setCurrentWidget(NULL);
	...
	UpdateUIManager::Instance().getViewWidget<ANORRLGameExplorer>(eDW_GAME_EXPLORER).onCloseIdeDoc();
...
void ANORRLIDEDoc::cleanupGameState(GameState& gameState)
{
...
bool ANORRLIDEDoc::loadFromStream(std::istream* stream)
{
    if (!stream)
        return false;
...
void ANORRLIDEDoc::preFetchResources()
{
    ARL::Verb* verb = getVerb(m_EditGame,"publishToANORRLAsAction");
	if(verb) {
		PublishToANORRLAsVerb* publishVerb = static_cast<PublishToANORRLAsVerb*>(verb);
		publishVerb->initDialog();
	}
}

QString ANORRLIDEDoc::displayName() const
{
...
QString ANORRLIDEDoc::windowTitle() const 
{
...
QWidget* ANORRLIDEDoc::getViewer()
{
 ...
 */
bool ANORRLIDEDoc::isSimulating()
{
...
void ANORRLIDEDoc::createScriptMapping(shared_ptr<ARL::Instance> instance)
{
...
void ANORRLIDEDoc::restoreEditModelScripts()
{
    ARL::DataModel::LegacyLock lock(m_EditGame.m_Game->getDataModel(), ARL::DataModelJob::Write);
	...
	for (tScriptDocs::const_iterator iter = openScriptDocs.begin(); iter != openScriptDocs.end() ; ++iter)
	{
		...
		{
			// use ANORRLDocManager::removeDoc for proper cleanup of documents
			if ((*iter) == ANORRLDocManager::Instance().getCurrentDoc())
			{
				// if script doc is current then make sure we ignore IDE doc activate while removing script doc
				// IDE doc will be activated in ANORRLIDEDoc::closePlayDataModel
				ARL::ScopedAssign<bool> ignoreDocActivate(m_bActive, true);
				ANORRLDocManager::Instance().removeDoc(**iter);
			}
			else
			{
				ANORRLDocManager::Instance().removeDoc(**iter);
...
bool ANORRLIDEDoc::saveScriptsChangedWhileTesting()
{
...
bool ANORRLIDEDoc::saveAs(const QString& fileName)
{
    QMutexLocker locker(&m_IOMutex);
	...
	if (result)
	{
		...
		if (fileName != m_fileName && fileName != m_AutoSaveName && !isCloudEditSession())
        {
            ...
            m_fileName = fileName;
            ANORRLDocManager::Instance().setDocTitle(*this,displayName(),titleTooltip(),titleIcon());
        }
...
void ANORRLIDEDoc::activate()
{
	...
	UpdateUIManager::Instance().getViewWidget<ANORRLExplorerWidget>(eDW_OBJECT_EXPLORER).setCurrentWidget(m_CurrentGame->m_TreeWidget);	
...
void ANORRLIDEDoc::setDockViewsEnabled(bool state)
{
	// update tree widget
    ...
    ANORRLPropertyWidget& property_view = uiManager.getViewWidget<ANORRLPropertyWidget>(eDW_PROPERTIES);
	property_view.setDataModel(dataModel);

    ANORRLDiagnosticsView& diagnostics_view = uiManager.getViewWidget<ANORRLDiagnosticsView>(eDW_DIAGNOSTICS);
	diagnostics_view.setDataModel(dataModel);

    ANORRLScriptReview& script_view = uiManager.getViewWidget<ANORRLScriptReview>(eDW_SCRIPT_REVIEW);
	script_view.setDataModel(dataModel);

    ANORRLToolBox& toolbox_view = uiManager.getViewWidget<ANORRLToolBox>(eDW_TOOLBOX);
	...
	if (!state)
	{
		ANORRLFindWidget& find_view = uiManager.getViewWidget<ANORRLFindWidget>(eDW_FIND);
		find_view.dataModelClosing();
	}
}

void ANORRLIDEDoc::onWaypointChanged()
{
    QMetaObject::invokeMethod(this,"updateUndoRedo",Qt::QueuedConnection);
}

void ANORRLIDEDoc::updateUndoRedo()
{
...
void ANORRLIDEDoc::mapActionIDWithVerb(GameState& gameState,const QString &actionID, const ARL::Name &verbName)
{
	gameState.m_VerbMap[actionID] = &verbName;
}

void ANORRLIDEDoc::mapActionIDWithVerb(GameState& gameState,const QString &actionID, const ARL::Verb *pVerb)
{
	if (pVerb)
		mapActionIDWithVerb(gameState, actionID, pVerb->getName());
}

void ANORRLIDEDoc::mapActionIDWithVerb(GameState& gameState,const QString &actionID, const char *verbName)
{
	if (verbName)
		mapActionIDWithVerb(gameState, actionID, ARL::Name::lookup(verbName));
}

bool ANORRLIDEDoc::handleDrop(const QString &fileName)
{
	if (fileName.isEmpty())
		return false;
...
bool ANORRLIDEDoc::doHandleAction(const QString& actionID, bool isChecked)
{
	...
	{
		if (ANORRLRibbonMainWindow* mainWindow = static_cast<ANORRLRibbonMainWindow*>(&UpdateUIManager::Instance().getMainWindow()))
		{
	...
	try
	{
		...
		static QString noLockVerbs(
            ...
            "startServerAction startPlayerAction publishToANORRLAction publishToANORRLAsAction "
            "publishSelectionToANORRLAction saveToANORRLAction undoAction redoAction "
...
void ANORRLIDEDoc::preProcessVerb(const QStringList &tokens)
{
...
bool ANORRLIDEDoc::handlePluginAction(void *pNotifier, void *pAction) 
{
...
ARL::Reflection::Variant ANORRLIDEDoc::evaluateCommandBarItem(const char* itemToEvaluate, shared_ptr<ARL::LuaSourceContainer> script)
{
    shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel();
	ARL::DataModel::LegacyLock lock(dataModel,ARL::DataModelJob::Write);
...
void ANORRLIDEDoc::handleScriptCommand(const QString &execCommand)
{
...
bool ANORRLIDEDoc::actionState(const QString &actionID, bool &enableState, bool &checkedState)
{		
	if (isCloudEditSession())
	{
		// disable save and publish related actions
		static QString cloudEditDeactiveActions(
			"publishToANORRLAsAction fileSaveAction fileSaveAsAction publishToANORRLAction ");
	...
	static QString defaultActiveActions(
		...
		"executeScriptAction publishToANORRLAsAction testClearStatsAction ");
	...
	if (m_pMainWindow->isRibbonStyle())
	{
		...
		if (clientAndServerActions.contains(actionID))
		{
			if (!m_CurrentGame || !m_CurrentGame->m_Game || !m_CurrentGame->m_Game->getDataModel() || (
				!ANORRLIDEDoc::isEditMode(m_CurrentGame->m_Game->getDataModel().get()) && !isCloudEditSession()))
			{
				enableState = false;
				return true;
			}
			...
			{
				enableState = !ANORRLApplicationManager::instance().hasChildProcesses();
				return true;
			}
...
void ANORRLIDEDoc::createActionIDVerbMap(GameState& gameState)
{
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
	...
	verbs.append(new ARL::BoundVerb(dataModel.get(), "renameObjectVerb",
		boost::bind(&ANORRLIDEDoc::onObjectRename, this), boost::bind(&ANORRLIDEDoc::isObjectRenameEnabled, this)));
...
void ANORRLIDEDoc::mapFilesMenu(GameState& gameState)
{
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;

	verbs.append(new PublishToANORRLVerb(dataModel.get(), m_pMainWindow));
	mapActionIDWithVerb(gameState,"publishToANORRLAction",verbs.back());

	verbs.append(new PublishToANORRLAsVerb(dataModel.get(), m_pMainWindow));
	mapActionIDWithVerb(gameState,"publishToANORRLAsAction",verbs.back());

	verbs.append(new PublishSelectionToANORRLVerb(dataModel.get(), m_pMainWindow));
	mapActionIDWithVerb(gameState,"publishSelectionToANORRLAction",verbs.back());
	
	/// This goes into the Tree View
	mapActionIDWithVerb(gameState,"saveToANORRLAction",verbs.back());
...
void ANORRLIDEDoc::mapEditMenu(GameState& gameState)
{
...
void ANORRLIDEDoc::mapViewMenu(GameState& gameState)
{
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
void ANORRLIDEDoc::mapFormatMenu(GameState& gameState)
{
    boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
void ANORRLIDEDoc::mapToolMenu(GameState& gameState)
{
    boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
void ANORRLIDEDoc::mapInsertMenu(GameState& gameState)
{
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
void ANORRLIDEDoc::mapAdvToolsToolbar(GameState& gameState)
{
	boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
	shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
	QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
void ANORRLIDEDoc::mapRunToolbar(GameState& gameState)
{
    boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
	...
	verbs.append(new ARL::BoundVerb(
        dataModel.get(), 
        "IDE Run",
		boost::bind(&ANORRLIDEDoc::onIdeRun, this, false), 
        boost::bind(&ANORRLIDEDoc::isIdeRunEnabled, this, false)));
	mapActionIDWithVerb(gameState,"simulationRunAction", verbs.back());

	verbs.append(new ARL::BoundVerb(
		dataModel.get(), 
		"IDE Play",
		boost::bind(&ANORRLIDEDoc::onIdeRun, this, true), 
		boost::bind(&ANORRLIDEDoc::isIdeRunEnabled, this, true)));
	mapActionIDWithVerb(gameState,"simulationPlayAction", verbs.back());

	verbs.append(new ARL::BoundVerb(
		dataModel.get(), 
		"IDE Pause",
		boost::bind(&ANORRLIDEDoc::onIdePause, this), 
		boost::bind(&ANORRLIDEDoc::isIdePauseEnabled, this)));  
	mapActionIDWithVerb(gameState,"simulationStopAction", verbs.back());
	
	verbs.append(new ARL::BoundVerb(
        dataModel.get(), 
        "IDE Reset",
		boost::bind(&ANORRLIDEDoc::onIdeReset, this),
        boost::bind(&ANORRLIDEDoc::isIdeResetEnabled, this)));
	mapActionIDWithVerb(gameState,"simulationResetAction", verbs.back());
}

void ANORRLIDEDoc::mapCameraToolbar(GameState& gameState)
{
...
void ANORRLIDEDoc::mapStandardToolbar(GameState& gameState)
{
    boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
...
void ANORRLIDEDoc::mapAdvanceBuildToolbar(GameState& gameState)
{
    boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
bool ANORRLIDEDoc::isIdePauseEnabled()
{
	if (m_PlayGame.m_Game)
	{
		shared_ptr<ARL::DataModel> playDataModel = m_PlayGame.m_Game->getDataModel();
...
void ANORRLIDEDoc::teleportToURL(QString url, bool play)
{
    if (StudioUtilities::isTestMode())
    {
        QtUtilities::ARLMessageBox msgBox;
...
void ANORRLIDEDoc::loadPlayDataModel(QString url, bool play, bool cloneDataModel, bool isTeleport)
{
    shared_ptr<ARL::DataModel> dataModel = m_EditGame.m_Game->getDataModel();
    ...
    if (!m_PlayGame.m_Game)
    {
        ...
        m_PlayGame.m_View = new ANORRLView(m_pQOgreWidget,m_PlayGame.m_Game, m_GameView, /* vr = */ true);
    ...
    m_pQOgreWidget->setANORRLView(m_PlayGame.m_View);
    ...
    shared_ptr<ARL::DataModel> playDataModel = m_PlayGame.m_Game->getDataModel();
    {
		...
		if (cloneDataModel)
		{
			playDataModel->setPlaceID(placeId, false /*not a anorrl place*/);
		...
		ANORRLPluginHost::loadPlugins(playDataModel.get());
	...
	ANORRLDocManager::Instance().setCurrentDoc(this);

    UpdateUIManager::Instance().updateToolBars();
}

void ANORRLIDEDoc::closePlayDataModel()
{
    ...
    if (FFlag::LuaDebugger)
    {
        ...
        if (ANORRLDocManager::Instance().getCurrentDoc() != this)
            ANORRLDocManager::Instance().getCurrentDoc()->activate();
    }

	m_pMainWindow->updateDiscordRPC();
}

void ANORRLIDEDoc::onIdeRun(bool play)
{
	shared_ptr<ARL::DataModel> dataModel = m_EditGame.m_Game->getDataModel();
    ...
    if (FFlag::UseBuildGenericGameUrl)
    {
        QString pathStr = QString("game/visit.slua?IsPlaySolo=1&UserID=%1&PlaceID=%2&universeId=%3")
            .arg(ANORRLUser::singleton().getUserId())
            .arg(dataModel->getPlaceID())
            .arg(dataModel->getUniverseId());
    
        scriptStr = QString("loadfile(\"%1\")()\n")
            .arg(QString::fromStdString(BuildGenericGameUrl(ANORRLSettings::getBaseURL().toStdString(),pathStr.toStdString())));
    }
    else
    {
...
bool ANORRLIDEDoc::isIdeRunEnabled(bool play)
{
	if (m_PlayGame.m_Game) 
	{
...
bool ANORRLIDEDoc::getIsCloudEditSession()
{
...
bool ANORRLIDEDoc::isObjectRenameEnabled()
{
...
bool ANORRLIDEDoc::autoSave(bool force)
{
    ...
    try
    {
        ...
        if (m_AutoSaveName.isEmpty())
        {
            ...
            QString fullFileName = QString("%1/%2_AutoSave_%3.arl").
                arg(autoSavePath).
                arg(fileName);
 ...
 */
void ANORRLIDEDoc::setAutoSaveLoad()
{
...
void ANORRLIDEDoc::resetDirty(ARL::DataModel* dataModel)
{
	if (!dataModel)
		return;
...
const char* ANORRLIDEDoc::getOpenFileFilters()
{
	return "All ANORRL Place Files (*arl *arlx);;ANORRL Place Files (*.arl);;ANORRL XML Place Files (*.arlx)";
}
 ...
 */
void ANORRLIDEDoc::toggleStats(const QString& stats)
{
 ...
 */
void ANORRLIDEDoc::setStatsEnabled(const QString& stats, bool enabled, QAction* verbAction)
{
    ARL::Verb* verb = m_CurrentGame->m_Game->getDataModel()->getVerb(stats.toStdString());
...
bool ANORRLIDEDoc::displayAskConvertPlaceToNewMaterialsIfInsertNewModel()
{
	bool conversionDialogDisabled = ANORRLSettings().value(kMaterialPropertiesConvertDialogDisabled, false).toBool();
	if (!conversionDialogDisabled)
	{
		...
		{
			ANORRLSettings().setValue(kMaterialPropertiesConvertDialogDisabled, true);
			ANORRLSettings().setValue(kMaterialPropertiesConvertByDefault, convertResponse);
		}

		return convertResponse;
	}

	return ANORRLSettings().value(kMaterialPropertiesConvertByDefault, false).toBool();
}

void ANORRLIDEDoc::onWorkspacePropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
...
void ANORRLIDEDoc::initializeDebuggerData(const QString &debugFile, const shared_ptr<ARL::DataModel> dm, const tDebuggersMap& debuggersMap)
{
 ...
 */
void ANORRLIDEDoc::setDraggerGridMode(ARL::DRAG::DraggerGridMode gridMode)
{
 ...
 */
void ANORRLIDEDoc::setGridSize(int gridSize)
{
	if ( gridSize == 2 )
...
static void placeNameLoadedHelperSuccess(QPointer<ANORRLIDEDoc> doc, std::string json)
{
...
static void placeNameLoadedHelperError(QPointer<ANORRLIDEDoc> doc, std::string error)
{
	// didn't get name, just do nothing for now
}

void ANORRLIDEDoc::placeNameLoaded(QString json)
{
...
void ANORRLIDEDoc::searchToolboxByDefaultSearchString()
{
	...
	if (FFlag::SearchToolboxByDataModelSearchString)
		if (Instance* defaultToolboxSearchInstance = m_EditGame.m_Game->getDataModel()->findFirstChildByName("DefaultToolboxSearch"))
			...
			{
				QMetaObject::invokeMethod(&UpdateUIManager::Instance().getViewWidget<ANORRLToolBox>(eDW_TOOLBOX),
					"loadUrl", Qt::QueuedConnection, Q_ARG(QString, 
					QString("%1/%2%3").arg(ANORRLSettings::getBaseURL()).arg("ide/clienttoolbox?searchset=").arg(stringValue->getValue().c_str())));
			}
}
 ...
 */
void ANORRLIDEDoc::onContentDataLoaded()
{
 ...
 */
void ANORRLIDEDoc::onContentDataSaved()
{
	// save debugger data
 ...
 */
void ANORRLIDEDoc::onDelayedInitializeDebuggerData()
{
	shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel();
	...
	dataModel->saveFinishedSignal.connect(boost::bind(&ANORRLIDEDoc::onContentDataSaved, this));
}

/**
 * Gets upload URL for current datamodel
 */
QString ANORRLIDEDoc::getUploadURL()
{
 ...
 */
QString ANORRLIDEDoc::getDebugInfoFile(bool isAutoSaveFile, const QString& debuggerFileExt)
{
	QString fileName;
...
void ANORRLIDEDoc::notifyScriptEdited(shared_ptr<ARL::Instance> script)
{
	if (script)
	{
		...
		item.ancestryChangedConnection = script->ancestryChangedSignal.connect(
			boost::bind(&ANORRLIDEDoc::onPlayModifiedAncestryChanged, this, script, _2));
	}
}

void ANORRLIDEDoc::onPlayModifiedAncestryChanged(shared_ptr<ARL::Instance> script, shared_ptr<ARL::Instance> newParent)
{
	if (newParent==NULL)
	{
...
void ANORRLIDEDoc::clearPlayModifiedScriptList()
{
    for (tPlayModifiedScriptList::iterator iter = m_playModifiedScriptList.begin(); iter != m_playModifiedScriptList.end(); ++iter)
    {
        delete (*iter);
...
void ANORRLIDEDoc::addScriptMapping(shared_ptr<ARL::Instance> script, shared_ptr<ARL::DataModel> dataModel)
{
	if (!script)
		return;
...
std::string ANORRLIDEDoc::getScriptIndexHierarchy(shared_ptr<ARL::Instance> script,  std::vector<int>& indexHierarchy)
{
	if (!script)
		return "";
...
shared_ptr<ARL::Instance> ANORRLIDEDoc::getScriptByIndexHierarchy(shared_ptr<ARL::DataModel> dataModel, const std::vector<int>& indexHierarchy, const std::string& serviceName)
{
    if (serviceName == "")
...
boost::shared_ptr<ARL::Instance> ANORRLIDEDoc::getEditScriptByPlayInstance(ARL::Instance* instance)
{
...
void ANORRLIDEDoc::saveEditModelDebuggerData()
{
	...
	{
		ARL::DataModel* dm = m_EditGame.m_Game->getDataModel().get();
		dm->visitDescendants(boost::bind(&ANORRLIDEDoc::addToDebuggersMap, this, _1, boost::ref(m_EditDebuggersMap)));
	}
}

void ANORRLIDEDoc::updateEditModelDebuggerData()
{
	if (m_PlayGame.m_Game && m_PlayGame.m_Game->getDataModel())
	{
		ARL::DataModel* dm = m_PlayGame.m_Game->getDataModel().get();
		dm->visitDescendants(boost::bind(&ANORRLIDEDoc::updateDebuggersMap, this, _1, boost::ref(m_EditDebuggersMap)));
	}
}

void ANORRLIDEDoc::updateDebuggerManager(const tDebuggersMap& debuggers)
{
...
void ANORRLIDEDoc::addToDebuggersMap(shared_ptr<ARL::Instance> instance, tDebuggersMap& debuggersMap)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) || ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
	{
		ARL::Scripting::DebuggerManager& debuggerManager = ARL::Scripting::DebuggerManager::singleton();
...
void ANORRLIDEDoc::updateDebuggersMap(shared_ptr<ARL::Instance> instance, tDebuggersMap& debuggersMap)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) || ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
	{
...
ANORRLIDEDoc::DebuggerData ANORRLIDEDoc::getDebuggerData(ARL::Scripting::ScriptDebugger* debugger)
{
	DebuggerData data;
	...
	data.breakpoints.sort(ANORRLIDEDoc::DebuggerData::BreakpointLineNumberOrder());
...
bool ANORRLIDEDoc::isEditMode(const ARL::Instance* context)
{
...
void ANORRLIDEDoc::updateDisplayName(const QString& newDisplayName)
{
...
void ANORRLIDEDoc::refreshDisplayName()
{
	ANORRLDocManager::Instance().setDocTitle(*this, displayName(), titleTooltip(), titleIcon());
}

void ANORRLIDEDoc::forceReloadImages(const QStringList& contentIds)
{
...
const QIcon& ANORRLIDEDoc::titleIcon() const
{
    static QIcon icon;
    if (icon.isNull())
...
void ANORRLIDEDoc::updateFromPlaceID(int placeId)
{
	if (ARL::HttpRbxApiService* apiService = ARL::ServiceProvider::find<ARL::HttpRbxApiService>(getDataModel()))
	{
		...
		apiService->getAsync( ARL::format("marketplace/productinfo?assetId=%d", placeId),false, ARL::PRIORITY_DEFAULT,
			boost::bind(&placeNameLoadedHelperSuccess, QPointer<ANORRLIDEDoc>(this), _1),
			boost::bind(&placeNameLoadedHelperError, QPointer<ANORRLIDEDoc>(this), _1));
	}
}

const QString& ANORRLIDEDoc::titleTooltip() const
{
    if (!m_fileName.isEmpty())
        return m_fileName;
...
void ANORRLIDEDoc::resetMouseCommand(const char* mouseCommandToReset)
{
	shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel();
	if (dataModel)
	{
...
void ANORRLIDEDoc::setMuteAllSounds(bool mute)
{
	if (!m_CurrentGame)
	{
		return;
...
void ANORRLIDEDoc::initServerAudioBehavior()
{
...
void ANORRLIDEDoc::promptForExistingAssetId(const std::string& assetType, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
{	
...
void ANORRLIDEDoc::showImportAssetDialog(QVariant resumeFunctionVar, QVariant errorFunctionVar)
{
	...
	QString importUrl(QString("%1/studio/animations/import").arg(ANORRLSettings::getBaseURL()));	
...
QString ANORRLIDEDoc::getRecentSaveFile()
{
	QString fileToSave;
	...
	if (QDir().mkpath(recentSavesPath))
	{
		...
		if (m_bIsLocalDocument)
		{
			fileToSave = recentSavesPath+"/"+baseName+"_"+ QString::number(ARL::Hash::hash(QFileInfo(fileName()).absoluteFilePath().toStdString()))+".arl";	
		}
		else
		{
			fileToSave = recentSavesPath+"/"+baseName+"_"+QString::number(getDataModel()->getPlaceID())+".arl";
		}
	}
	return fileToSave;
}

void ANORRLIDEDoc::clearOutputWidget()
{
	if (AuthoringSettings::singleton().clearOutputOnStart)
		m_pMainWindow->getOutputWidget()->clear();
}

void ANORRLIDEDoc::onLocalPlayerAdded(shared_ptr<ARL::Instance> newPlayer)
{	
	m_localPlayerConnection = newPlayer->propertyChangedSignal.connect(boost::bind(&ANORRLIDEDoc::onLocalPlayerNameChanged, this, _1));	
}

void ANORRLIDEDoc::onLocalPlayerNameChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
...
void ANORRLIDEDoc::syncPlayerName()
{
...
void ANORRLIDEDoc::appendTimeStampToDisplayName()
{
...
void ANORRLIDEDoc::removeTimeStampFromDisplayName()
{
	QString updatedDisplayName = displayName();
...
void ANORRLIDEDoc::updateOnSimulationStateChange(bool run)
{
	if (isCloudEditSession())
		run ? appendTimeStampToDisplayName() : removeTimeStampFromDisplayName();
}

void ANORRLIDEDoc::setTeamCreateWidgetEnabled(bool enable)
{
	...
	ANORRLTeamCreateWidget& pTeamCreateWidget = UpdateUIManager::Instance().getViewWidget<ANORRLTeamCreateWidget>(eDW_TEAM_CREATE);
...
void ANORRLIDEDoc::initCloudEditObjects()
{
	if (!isCloudEditSession())
	{
		ARLASSERT(false);
	...
	if (pChatWidgetStack)
	{
		m_ChatWidget.reset(new ANORRLChatWidget(pChatWidgetStack, dataModel, playersDataManager));
...
QString ANORRLIDEDoc::downloadPlaceCopy()
{
	ANORRLSettings settings;
	QString rbxmLastDir = settings.value("arlm_last_directory").toString();
...
void ANORRLIDEDoc::enableDownloadPlaceCopy(bool state)
{
	...
	UpdateUIManager::Instance().getMainWindow().publishToANORRLAsAction->setVisible(!state);
	...
	UpdateUIManager::Instance().getMainWindow().publishToANORRLAction->setVisible(!state);
}
...
WorkspaceAnnouncementTooltip::WorkspaceAnnouncementTooltip(QWidget* parent)
{
	...
	setLastClosedMessage(ANORRLSettings().value(kWorkspaceNotificationMessageAlreadyClosed, "").toString());
...
{
	setVisible(false);
	ANORRLSettings().setValue(kWorkspaceNotificationMessageAlreadyClosed, getNotificationId().c_str());
