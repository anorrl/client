...
int ANORRLScriptDoc::sScriptCount = 1;
int ANORRLScriptDoc::slastActivatedRibbonPage = -1;
...
void ANORRLScriptDoc::init(ANORRLMainWindow& mainWindow)
{	
	...
	qRegisterMetaType<ARL::ScriptAnalyzer::Result>("ARL::ScriptAnalyzer::Result");
}

QString ANORRLScriptDoc::fileName() const
{
    if ( m_pParentDoc )
...
bool ANORRLScriptDoc::open(ANORRLMainWindow *pMainWindow, const QString &fileName)
{
	bool success = true;
...
static void receiveStateAndSetEvent(bool value, shared_ptr<ARL::LuaSourceContainer> lsc, bool* state,
	std::string* ownerOfLock, ARL::CEvent* event)
{
	*state = value;
	if (ARL::Instance* currentEditor = lsc->getCurrentEditor())
	{
...
static void disconnected(bool* state, ARL::CEvent* event)
{
	*state = false;
	event->Set();
}

static void attemptToLockScript(shared_ptr<ARL::DataModel> dm, LuaSourceBuffer lsb,
...
void ANORRLScriptDoc::editingContents()
{
	ANORRLIDEDoc* ideDoc = ANORRLDocManager::Instance().getPlayDoc();
	ARLASSERT(ideDoc && ideDoc->isCloudEditSession());
	...
	{
		QtUtilities::ARLMessageBox warningBox;
	...
	{
		ARL::ScopedAssign<bool> sa(m_currentlyAttemptingToGetScriptLock, true);
		...
		if (!m_haveCloudEditScriptLock)
		{
			...
			// notify user
			QtUtilities::ARLMessageBox messageBox;
...
IANORRLDoc::ARLCloseRequest ANORRLScriptDoc::requestClose()
{
	return IANORRLDoc::NO_SAVE_NEEDED;
}

bool ANORRLScriptDoc::doClose()
{
	deActivate();
	...
	setScript(shared_ptr<ARL::DataModel>(), LuaSourceBuffer());
	
	//self deletion (maybe we must use reference counting!)
	deleteLater();

    if ( m_pParentDoc )
        ANORRLDocManager::Instance().setCurrentDoc(m_pParentDoc);

	return true;
}

bool ANORRLScriptDoc::isModified()
{
	if (m_pParentDoc)
		return false;
...
const QIcon& ANORRLScriptDoc::titleIcon() const
{
	static QIcon empty;
	static QIcon cloud;
...
bool ANORRLScriptDoc::saveAs(const QString &fileName)
{
	if (fileName.isEmpty())
		return false;
	...
	try
	{
		...
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Successfully saved at %s", qPrintable(_fileName));
	}	
	catch (...)
	{
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Error in saving document at %s", qPrintable(_fileName));
		return false;
	}
	return true;
}

QString ANORRLScriptDoc::saveFileFilters()
{ 
	if (m_pParentDoc)
...
QWidget* ANORRLScriptDoc::getViewer() 
{ 
    return static_cast<QWidget*>(m_pTextEdit); 
}

void ANORRLScriptDoc::activate()
{
	if (m_bActive)
		return;
...
void ANORRLScriptDoc::deActivate()
{
	if (!m_bActive)
		return;
...
void ANORRLScriptDoc::updateUI(bool state)
{
	setMenuVisibility(state);
}

bool ANORRLScriptDoc::handleDrop(const QString &fileName)
{
	if(fileName.isEmpty())
		return false;
...
bool ANORRLScriptDoc::doHandleAction(const QString& actionID, bool isChecked)
{
	bool handled = false;
    ...
    // All Actions that we do not want to be handled by the Parent ANORRLIDEDoc when we are in Script Doc 
    ...
    else if ( actionID == "actionCustomContextMenuShow" )
	{
		if ( m_pMainWindow->treeWidgetStack().currentWidget()->hasFocus() )
		{
			...
			if ( pMimeData && pMimeData->hasFormat("application/x-roblox-studio") ) // compatibility maybe
				m_pMainWindow->pasteIntoAction->setEnabled(true);
			if (pMimeData && pMimeData->hasFormat("application/x-anorrl-studio"))
				m_pMainWindow->pasteIntoAction->setEnabled(true);
		}
    ...
    {
        // Delegate all script enabled actions to parent ANORRLIDEDoc & give parent the chance to handle
...
void ANORRLScriptDoc::handleScriptCommand(const QString &execCommand)
{
	if(m_pParentDoc)
	{
...
bool ANORRLScriptDoc::actionState(const QString &actionID, bool &enableState, bool &checkedState)
{
    ...
    if (actionID == "reloadScriptAction")
    {
        if ((ANORRLDocManager::Instance().getPlayDoc() && ANORRLDocManager::Instance().getPlayDoc()->isSimulating()) ||
	...
	/// Anything that needs to be handled by ANORRLIDEDoc goes in here, the enabled/checked state is all delegated to parent.
	static QString delegateToParentActions(
        "saveToANORRLAction insertIntoFileAction selectionSaveToFileAction playSoloAction "
...
void ANORRLScriptDoc::setupScriptConnection()
{
...
void ANORRLScriptDoc::setScript(shared_ptr<ARL::DataModel> dm, LuaSourceBuffer script)
{
	if (m_script == script)
		return;
	...
	if (!m_script.empty())
	{
		...
		if (m_haveCloudEditScriptLock && m_dataModel && script.empty())
		{
			...
			{
				ARL::DataModel::LegacyLock lock(m_dataModel, ARL::DataModelJob::Write);
				if (shared_ptr<ARL::LuaSourceContainer> lsc = m_script.toInstance())
				{
					ARL::Network::Player* localPlayer = ARL::Network::Players::findLocalPlayer(m_dataModel.get());
        ...
        ANORRLDocManager::Instance().deregisterScriptDoc(*this);
	}

	if (!script.empty()) // adding an associated script instance
	{
		m_script = script;
		m_pParentDoc = ANORRLDocManager::Instance().getPlayDoc();
		m_dataModel = dm;
		...
		if (shared_ptr<ARL::Instance> inst = script.toInstance())
		{
			m_ancestryChangedConnection = inst->ancestryChangedSignal.connect(boost::bind(&ANORRLScriptDoc::onAncestryChanged, this, _2));
			m_propertyChangedConnection = inst->propertyChangedSignal.connect(boost::bind(&ANORRLScriptDoc::onPropertyChanged, this, _1));
		}

		onScriptNamePropertyChanged(buildTabTitle());
		onScriptSourcePropertyChanged(QString::fromStdString(m_script.getScriptText()));

        ANORRLDocManager::Instance().registerOpenedScriptDoc(*this,m_script);  
...
void ANORRLScriptDoc::onAncestryChanged(shared_ptr<ARL::Instance> newParent)
{
	if (newParent==NULL)
	{
...
void ANORRLScriptDoc::requestScriptDeletion()
{	
	if (!m_script.empty())
		ANORRLDocManager::Instance().closeDoc(m_script);

	// Set script to NULL because it is no longer valid
	setScript(shared_ptr<ARL::DataModel>(), LuaSourceBuffer());
}

void ANORRLScriptDoc::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* desc)
{
	ARLASSERT(!m_dataModel || m_dataModel->currentThreadHasWriteLock());
	...
	if (*desc==ARL::Instance::desc_Name || desc->name == "CurrentEditor")
	{
		QMetaObject::invokeMethod(this, "onScriptNamePropertyChanged", Qt::QueuedConnection,
			Q_ARG(QString, buildTabTitle()));
	}
	else if ((*desc==ARL::Script::prop_EmbeddedSourceCode) || (*desc==ARL::ModuleScript::prop_Source))
	{
...
void ANORRLScriptDoc::onScriptNamePropertyChanged(const QString& newName)
{
	m_displayName = newName;
	ANORRLDocManager::Instance().setDocTitle(*this, displayName(), titleTooltip(), titleIcon());
}

void ANORRLScriptDoc::onScriptSourcePropertyChanged(const QString& newSource)
{
	ARL::ScopedAssign<bool> sa(m_updatingText, true);
	maybeUpdateText(newSource);
}

void ANORRLScriptDoc::maybeUpdateText(const QString& code)
{
	if (m_script.empty() || !m_pTextEdit)
		return;
...
void ANORRLScriptDoc::refreshEditorFromSourceBuffer()
{
	QString source;
	{
		ARL::DataModel::LegacyLock l(m_dataModel, ARL::DataModelJob::Write);
		source = QString::fromStdString(m_script.getScriptText());
	}
	onScriptSourcePropertyChanged(source);
}

void ANORRLScriptDoc::applyEditChanges()
{
	if (m_script.empty() || !m_dataModel)
		return;

	if (ANORRLIDEDoc::getIsCloudEditSession() && !m_haveCloudEditScriptLock)
	{
		return;
	}
	...
	if (m_pTextEdit->document()->isModified() && !m_updatingText)
	{
        ...
        if (ANORRLDocManager::Instance().getPlayDoc() &&
            ANORRLDocManager::Instance().getPlayDoc()->isSimulating())
        {
            ANORRLDocManager::Instance().getPlayDoc()->notifyScriptEdited(getCurrentScript().toInstance());
        }
...
LuaSourceBuffer ANORRLScriptDoc::getCurrentScript()
{
	return m_script;
}

void ANORRLScriptDoc::disconnectScriptConnection()
{
...
void ANORRLScriptDoc::deactivateConnection()
{
...
void ANORRLScriptDoc::activateConnection()
{
	//connect the edit tools for script editor with QTextEdit public slots.
	if (!FFlag::StudioSeparateActionByActivationMethod)
	{
...
QString ANORRLScriptDoc::buildTabTitle()
{
	QString tabTitle = QString::fromStdString(m_script.getScriptName());
	if (shared_ptr<ARL::LuaSourceContainer> lsc = m_script.toInstance())
	{
		if (ARL::Instance* editor = lsc->getCurrentEditor())
		{
...
void ANORRLScriptDoc::setMenuVisibility(bool visible)
{
	if (!visible)
	{
		FindReplaceProvider::instance().setFindListener(NULL);
	...
	if (scriptMenu)
	{
		if (mainWindow.isRibbonStyle())
		{
			...
			ANORRLRibbonMainWindow& ribbonMainWindow = static_cast<ANORRLRibbonMainWindow&>(mainWindow);
			if (visible)
			{
...
void ANORRLScriptDoc::onSelectionChanged()
{
    // update actions modified on selection change
    UpdateUIManager&  updateUIManager = UpdateUIManager::Instance();
    ANORRLMainWindow& rbxMainWindow = updateUIManager.getMainWindow();
...
void ANORRLScriptDoc::reloadLiveScript()
{
...
void ANORRLScriptDoc::explainBreakpointsDisabled()
{
	QMessageBox mb;
...
void ANORRLScriptDoc::removeScriptLock()
{
	ANORRLIDEDoc* ideDoc = ANORRLDocManager::Instance().getPlayDoc();
	ARLASSERT(ideDoc && ideDoc->isCloudEditSession());

	boost::shared_ptr<ARL::DataModel> dataModel = ideDoc->getEditDataModel();
	// if edit mode use the current script  else for play solo get the corresponding script in Edit datamodel 
	boost::shared_ptr<ARL::LuaSourceContainer> lsc = ideDoc->isPlaySolo() ? ARL::Instance::fastSharedDynamicCast<ARL::LuaSourceContainer>(ideDoc->getEditScriptByPlayInstance(m_script.toInstance().get()))
																		  : m_script.toInstance();
	if (lsc)
	{
		ARL::DataModel::LegacyLock lock(dataModel, ARL::DataModelJob::Write);
		ARL::Network::Player* localPlayer = ARL::Network::Players::findLocalPlayer(dataModel.get());
		if (localPlayer && lsc->getCurrentEditor() == localPlayer)
		{
			lsc->setCurrentEditor(NULL);
			lsc->fireSourceChanged();
