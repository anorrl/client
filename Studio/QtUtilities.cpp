...
namespace QtUtilities
{
	...
	QString getMacOSXVersion()
	{
		...
		switch (macVersion) 
		{
			...
			case QSysInfo::MV_SNOWLEOPARD:
				...
				break;
			default:
				// ANORRL Studio is not allowed to Run on 10.4 or below
    ...
    QByteArray getResourceFileBytes(const QString& resourceName)
    {
        ...
        ARLASSERT(resource.data());
        ARLASSERT(resource.size() > 0);
	...
	ARLMessageBox::ARLMessageBox(QWidget* parent)
		:QMessageBox(parent ? parent : &UpdateUIManager::Instance().getMainWindow())
	{
		updateWindowTitle(this);
	}

	ARLMessageBox::ARLMessageBox(QMessageBox::Icon icon, const QString & title, const QString & text, 
    ...
    // ARLMessageBox
    /*****************************************************************************/
	ARLConfirmationMessageBox::ARLConfirmationMessageBox(QString question, QString yesButtonName, QString noButtonName,  QWidget* parent)
	...
	void ARLConfirmationMessageBox::markYes()
	{
		clickedYes = true;
	}

	void ARLConfirmationMessageBox::markNo()
	{
		clickedYes = false;
	}
