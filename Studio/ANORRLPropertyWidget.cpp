...
CategoryItem* PropertyTreeWidget::getCategoryItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
	...
	pItem->setExpanded(m_bStoreCollapasedState ? (!collapsedCategoryItems.contains(categoryName) && ANORRLSettings().value(getRegistryKey(pItem), true).toBool()) : true);
...
void PropertyTreeWidget::onItemExpanded(QTreeWidgetItem * pItem)
{
	if (m_bStoreCollapasedState)
	{
		...
		if (pCategoryItem)
			ANORRLSettings().remove(getRegistryKey(pCategoryItem));
	}
}
...
void PropertyTreeWidget::onItemCollapsed(QTreeWidgetItem * pItem)
{
	if (m_bStoreCollapasedState)
	{
		CategoryItem *pCategoryItem = dynamic_cast<CategoryItem *>(pItem);
		if (pCategoryItem)
			ANORRLSettings().setValue(getRegistryKey(pCategoryItem), false);
	}
}
...
//--------------------------------------------------------------------------------------------
// ANORRLPropertyWidget -- main widget for showing properties of selected items
//--------------------------------------------------------------------------------------------
ANORRLPropertyWidget::ANORRLPropertyWidget(QWidget *pParent)
: QWidget(pParent)
, m_bUpdateRequested(false)
{
	setObjectName("ANORRLPropertyWidget");
...
ANORRLPropertyWidget::~ANORRLPropertyWidget()
{
}

QSize ANORRLPropertyWidget::sizeHint() const 
{
	return QSize(295, UpdateUIManager::Instance().getMainWindow().size().height() / 2);
}

void ANORRLPropertyWidget::setDataModel(boost::shared_ptr<ARL::DataModel> pDataModel)
{
	if ( m_pDataModel == pDataModel )
       return;
	...
	if (m_pDataModel)
	{
		...
		if (pSelection)
		{
			std::for_each(pSelection->begin(), pSelection->end(), boost::bind(&ANORRLPropertyWidget::addInstance, this, _1));
			m_cInstanceSelectionChanged = pSelection->selectionChanged.connect(boost::bind(&ANORRLPropertyWidget::onInstanceSelectionChanged, this, _1));
		}
	}
...
void ANORRLPropertyWidget::addInstance(boost::shared_ptr<ARL::Instance> pInstance)
{
	if (!pInstance)
		return;
...
void ANORRLPropertyWidget::removeInstance(boost::shared_ptr<ARL::Instance> pInstance)
{
	if (!pInstance)
		return;
...
void ANORRLPropertyWidget::onInstanceSelectionChanged(const ARL::SelectionChanged& evt)
{
...
void ANORRLPropertyWidget::requestInfoLabelUpdate()
{
	if (m_bUpdateRequested)
		return;
	//qt will take ownership of the event so no need to delete
	QApplication::postEvent(this, new ANORRLCustomEvent(PROPERTY_LABEL_UPDATE));
	m_bUpdateRequested = true;
}

bool ANORRLPropertyWidget::event(QEvent * evt)
{
...
void ANORRLPropertyWidget::updateSelectionInfoLabel()
{
