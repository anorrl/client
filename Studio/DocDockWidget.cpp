...
{
    ANORRLDocManager::Instance().attachDoc(m_Doc);
}
...
void DocDockWidget::moveEvent(QMoveEvent* event)
{
    ...
    else
    {
        ...
        if ( moveDelta.manhattanLength() > QApplication::startDragDistance() )
            ANORRLDocManager::Instance().setDockHoverOverPos(QCursor::pos());
    }
...
bool DocDockWidget::stopDragging()
{
    ...
    if ( m_IsMoving || m_IsSimulatingDrag )
    {
        ...
        attached = ANORRLDocManager::Instance().attemptAttach(m_Doc,clickPos);
        if ( !attached )
...
bool DocDockWidget::event(QEvent* event)
{
         ...
         event->type() == QEvent::MouseButtonRelease )
    {
        ...
        ARL::BaldPtr<QMouseEvent> mouseEvent = static_cast<QMouseEvent*>(event);
