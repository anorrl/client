...
typedef QMap<QString,const ARL::Name*> tActionIDVerbMap;

struct GameState
{
    shared_ptr<ARL::Game>           m_Game;
    QVector<ARL::Verb*>             m_Verbs;
    tActionIDVerbMap                m_VerbMap;
    ARL::BaldPtr<ANORRLView>        m_View;
    ARL::BaldPtr<ANORRLTreeWidget>  m_TreeWidget;
...
class ANORRLIDEDoc: public QObject, public ANORRLBasicDoc, FW::FileWatchListener
{
    Q_OBJECT

public:
	ANORRLIDEDoc(ANORRLMainWindow* pMainWindow);
	virtual ~ANORRLIDEDoc();
    ...
    void initializeANORRLView();
	
	IANORRLDoc::ARLCloseRequest requestClose();

	IANORRLDoc::ARLDocType docType() { return IANORRLDoc::IDE; }

    QString fileName() const        { return m_fileName; }
	QString displayName() const;
	QString windowTitle() const;
    QString keyName() const         { return "ANORRLIDEDoc"; }
    ...
    ARL::DataModel* getDataModel() const
	{
		if (m_CurrentGame && m_CurrentGame->m_Game && m_CurrentGame->m_Game->getDataModel())
			return m_CurrentGame->m_Game->getDataModel().get();
		return NULL;
	}
	shared_ptr<ARL::DataModel> getEditDataModel() const
	{
		return m_EditGame.m_Game ? m_EditGame.m_Game->getDataModel() : shared_ptr<ARL::DataModel>();
	}
	...
	void resetDirty(ARL::DataModel* dataModel);
	...
	void onWorkspacePropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor);
	...
	void onDMItemChanged(shared_ptr<ARL::Instance> item, const ARL::Reflection::PropertyDescriptor* descriptor);
    ...
    ARL::Reflection::Variant evaluateCommandBarItem(const char* itemToEvaluateshared, shared_ptr<ARL::LuaSourceContainer> script);

	ARL::Verb* getVerb(const QString& name) const;
    ...
    void notifyScriptEdited(shared_ptr<ARL::Instance> modifiedScript);

	static bool isEditMode(const ARL::Instance* context);
	...
	boost::shared_ptr<ARL::Instance> getEditScriptByPlayInstance(ARL::Instance* instance);

	QString downloadPlaceCopy();

public Q_SLOTS:
	void exportSelection(std::string filePath = "", ARL::ExporterSaveType exportType = ARL::ExporterSaveType_Selection);
	void exportPlace() { exportSelection("", ARL::ExporterSaveType_Everything); }
...
private:
	...
	void setDraggerGridMode(ARL::DRAG::DraggerGridMode gridMode);
	void setGridSize(int gridSize);

    void mapActionIDWithVerb(GameState& gameState,const QString &actionID, const ARL::Name &verbName);
	void mapActionIDWithVerb(GameState& gameState,const QString &actionID, const ARL::Verb *pVerb);
	void mapActionIDWithVerb(GameState& gameState,const QString &actionID, const char *verbName);
    ARL::Verb* getVerb(GameState& gameState,const QString& name) const;
    ...
    void createScriptMapping(shared_ptr<ARL::Instance> instance);
    ...
    typedef std::map<ARL::Instance*, boost::shared_ptr<ARL::Instance> > tScriptMap;

    void onPlayModifiedAncestryChanged(boost::shared_ptr<ARL::Instance> script, boost::shared_ptr<ARL::Instance> newParent);
    ...
    void addScriptMapping(shared_ptr<ARL::Instance> script, shared_ptr<ARL::DataModel> playDataModel);
    void clearScriptMap() { m_scriptMap.clear(); }    
    
    std::string getScriptIndexHierarchy(shared_ptr<ARL::Instance>,  std::vector<int>& indexHierarchy);
	...
	typedef std::map<boost::shared_ptr<ARL::Instance>, DebuggerData> tDebuggersMap;
	void saveEditModelDebuggerData();
	void updateEditModelDebuggerData();

	void initializeDebuggerData(const QString &fileName, const shared_ptr<ARL::DataModel> dm, const tDebuggersMap& debuggersMap = tDebuggersMap());
	void updateDebuggerManager(const tDebuggersMap& debuggers);

	void updateDebuggersMap(shared_ptr<ARL::Instance> instance, tDebuggersMap& debuggersMap);
	void addToDebuggersMap(shared_ptr<ARL::Instance> script, tDebuggersMap& debuggersMap);
	DebuggerData getDebuggerData(ARL::Scripting::ScriptDebugger* debugger);

	QString getRecentSaveFile();
	void clearOutputWidget();

	void searchToolboxByDefaultSearchString();
	void onLocalPlayerAdded(shared_ptr<ARL::Instance> newPlayer);
	void onLocalPlayerNameChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor);
	...
	shared_ptr<ANORRLChatWidget>      m_ChatWidget;
};
