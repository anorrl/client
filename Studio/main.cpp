...
class ANORRLApplication: public QApplication
{
public:
    ANORRLApplication(int &argc, char **argv): QApplication(argc, argv)
	{
	}
...
int main(int argc, char *argv[])
{
    ...
    ANORRLApplication app(argc,argv);
	...
	try
	{
		...
		if (UploadDmp)
		{
			...
			dumpErrorUploader->Upload(GetDmpUrl(ANORRLSettings::getBaseURL().toStdString(), true));
			return retVal;
		}
		...
		ANORRLApplicationManager::instance().registerApplication();
		ANORRLMainWindow rbxMainWin(argMap);
	...
	{
		FASTLOG1(FLog::ANORRLWndInit, "Application closed due to an exception: '%s'", exp.what());
	}

	ANORRLApplicationManager::instance().unregisterApplication();
	return retVal;
}
