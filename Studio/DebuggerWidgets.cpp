...
static bool populateValues(QTreeWidgetItem *pParentItem, const QString &key, const ARL::Reflection::Variant varValue);
...
: m_bBreakpointEncountered(false)
{
	...
	ANORRLMainWindow* rbxMainWindow = &UpdateUIManager::Instance().getMainWindow();
...
void DebuggerViewsManager::onDebuggerClientActivated(int currentBreakpoint, ARL::Scripting::ScriptDebugger::Stack currentStack)
{
...
void DebuggerViewsManager::onBreakpointAdded(shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{
	getViewWidget<BreakpointsWidget>(eDV_BREAKPOINTS).addBreakpoint(spBreakpoint);
}

void DebuggerViewsManager::onBreakpointRemoved(shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{
	getViewWidget<BreakpointsWidget>(eDV_BREAKPOINTS).removeBreakpoint(spBreakpoint);
}

void DebuggerViewsManager::onWatchAdded(shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
	getViewWidget<WatchWidget>(eDV_WATCH).addWatch(spWatch);
}

void DebuggerViewsManager::onWatchRemoved(shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
...
// StackDetailsItem
//--------------------------------------------------------------------------------------------
StackDetailsItem::StackDetailsItem(const ARL::Scripting::ScriptDebugger::FunctionInfo& functionInfo, bool isFirstItem)
...
void CallStackTreeWidget::setCallStack(const ARL::Scripting::ScriptDebugger::Stack& stack)
{
	m_pPreviousItem = NULL;
	clear();

	for(ARL::Scripting::ScriptDebugger::Stack::const_iterator iter = stack.begin(); iter != stack.end(); ++iter)
...
void CallStackWidget::populateDebuggers(DebuggerClient* pDebuggerClient)
{
	...
	for (unsigned int ii=0; ii < debuggerClients.size(); ++ii)
	{
		...
		if (!currentDebuggerClient || !ARL::Scripting::DebuggerManager::singleton().findDebugger(currentDebuggerClient->getScript().get()))
			continue;
		...
		const ARL::Scripting::ScriptDebugger::PausedThreads& pausedThreads = currentDebuggerClient->getPausedThreads();
		for (ARL::Scripting::ScriptDebugger::PausedThreads::const_iterator it=pausedThreads.begin(); it!=pausedThreads.end(); ++it)
		{
...
void CallStackWidget::setCallStack(const ARL::Scripting::ScriptDebugger::Stack& stack)
...
void CallStackWidget::onCurrentIndexChanged(int index)
{
	...
	boost::shared_ptr<ARL::Script> spScript = shared_from(static_cast<ARL::Script*>(var.value<void*>()));
	if (spScript)
	{
...
// BreakpointItem
//--------------------------------------------------------------------------------------------
BreakpointItem::BreakpointItem(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
...
, m_bIgnoreWaypointRequest(false)
{
	...
	ARL::Scripting::ScriptDebugger* spDebugger = ARL::Instance::fastDynamicCast<ARL::Scripting::ScriptDebugger>(spBreakpoint->getParent());
	if (spDebugger)
	{
...
bool BreakpointItem::hasBreakpoint(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{ return (spBreakpoint == m_spBreakpoint); }

void BreakpointItem::columnModified(int column)
{
	ARL::ScopedAssign<bool> ignoreBreakpointModificationEvent(m_bIgnoreBreakpointModification, true);

	if (column == 0)
	{
		ARL::DataModel::LegacyLock lock(DebuggerClientManager::Instance().getDataModel(), ARL::DataModelJob::Write);
		ARL::Scripting::DebuggerBreakpoint::prop_Enabled.setValue(m_spBreakpoint.get(), checkState(column) == Qt::Checked);
	...
	else if (column == 4)
	{
		...
		{
			ARL::DataModel::LegacyLock lock(DebuggerClientManager::Instance().getDataModel(), ARL::DataModelJob::Write);
			ARL::Scripting::DebuggerBreakpoint::prop_Condition.setValue(m_spBreakpoint.get(), modifiedCondition.toStdString());
		}
	}
}
...
void BreakpointItem::activate()
{
	...
	ARL::Scripting::ScriptDebugger* pDebugger = ARL::Instance::fastDynamicCast<ARL::Scripting::ScriptDebugger>(m_spBreakpoint->getParent());
	if (pDebugger)
	{
		ANORRLDocManager::Instance().openDoc(LuaSourceBuffer::fromInstance(shared_from(pDebugger->getScript())));
...
{
	ARL::ScopedAssign<bool> ignoreBreakpointModificationEvent(m_bIgnoreWaypointRequest, true);
...
void BreakpointItem::onBreakpointPropertyChanged(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
	...
	if (*pPropertyDescriptor == ARL::Scripting::DebuggerBreakpoint::prop_Enabled)
		setBreakpointEnabled(m_spBreakpoint->isEnabled());
}

void BreakpointItem::onScriptAncestryChanged(shared_ptr<ARL::Instance> newParent)
{
	// update breakpoint name if the script has change in hierarchy
	if (newParent)
	{
		ARL::Scripting::ScriptDebugger* spDebugger = ARL::Instance::fastDynamicCast<ARL::Scripting::ScriptDebugger>(m_spBreakpoint->getParent());
		if (spDebugger)
			setText(2, spDebugger->getScript()->getFullName().c_str());
	}
}
...
void BreakpointsTreeWidget::addBreakpoint(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{
	if(spBreakpoint && !findBreakpointItem(spBreakpoint))
		addTopLevelItem( new BreakpointItem(spBreakpoint) );
}

void BreakpointsTreeWidget::removeBreakpoint(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{
	if(spBreakpoint)
	{
...
BreakpointItem* BreakpointsTreeWidget::findBreakpointItem(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{
...
void BreakpointsWidget::addBreakpoint(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> breakpoint)
{ 
	if (!m_bIgnoreBreakpointModification)
...
void BreakpointsWidget::removeBreakpoint(boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> breakpoint)
{ 
	if (!m_bIgnoreBreakpointModification)
...
void BreakpointsWidget::onPushButtonClicked()
{
	...
	ARL::ScopedAssign<bool> ignorePropertyModifyEvent(m_bIgnoreBreakpointModification, true);
...
// WatchItem
//--------------------------------------------------------------------------------------------
WatchItem::WatchItem(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
: m_spWatch(spWatch)
, m_bIsInValid(true)
{
...
void WatchItem::updateWatchValue()
{
	...
	try
	{
		...
		ARL::Reflection::Variant Value = DebuggerClientManager::Instance().getWatchValue(m_spWatch);
		if (!Value.isVoid())
	...
	{
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Exception: %s", exp.what());
		result = false;
	}
...
bool WatchItem::hasWatch(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
...
void WatchItem::columnModified(int column)
{
	...
	ARL::ScopedAssign<bool> ignoreWatchModificationEvent(m_bIgnoreWatchModification, true);
	if (column == 0)
	{
		...
		if (watchExpression != m_spWatch->getExpression())
		{
			...
			ARL::DataModel::LegacyLock lock(DebuggerClientManager::Instance().getDataModel(), ARL::DataModelJob::Write);
			ARL::Scripting::DebuggerWatch::prop_Expression.setValue(m_spWatch.get(), watchExpression);
...
void WatchItem::onWatchPropertyChanged(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
	if (m_bIgnoreWatchModification || !m_spWatch)
		return;

	if (*pPropertyDescriptor == ARL::Scripting::DebuggerWatch::prop_Expression)
		setText(0, QString::fromStdString(ARL::Scripting::DebuggerWatch::prop_Expression.getValue(m_spWatch.get())));
}
...
void WatchItem::onScriptAncestryChanged(shared_ptr<ARL::Instance> newParent)
{
	// update script name if the script has change in hierarchy
	if (newParent)
	{
		QString scriptName;
		ARL::Scripting::ScriptDebugger* spDebugger = ARL::Instance::fastDynamicCast<ARL::Scripting::ScriptDebugger>(m_spWatch->getParent());
...
public:
	DummyWatchItem()
	: WatchItem(boost::shared_ptr<ARL::Scripting::DebuggerWatch>())
...
public:
	...
	{ 
		...
		if (dummyWatchItem)
		{
			...
			if (pDoc && (pDoc->docType() != IANORRLDoc::SCRIPT))
			{
				ARL::StandardOut::singleton()->print(ARL::MESSAGE_INFO, "Please open a script document to add a new watch expression");
				return NULL;
			}
		}
...
void WatchTreeWidget::addWatch(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
...
void WatchTreeWidget::removeWatch(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
...
{
	ARL::ScopedAssign<bool> ignoreWatchDeleteEvent(m_bIgnoreWatchModification, true);
	QList<QTreeWidgetItem*> selItems = selectedItems();
	if (selItems.length() > 0)
	{
...
void WatchTreeWidget::contextMenuEvent(QContextMenuEvent* evt)
{
	...
	IANORRLDoc *pCurrentDoc = ANORRLDocManager::Instance().getCurrentDoc();
	m_pAddWatchAction->setEnabled(pCurrentDoc && (pCurrentDoc->docType() == IANORRLDoc::SCRIPT));
...
WatchItem* WatchTreeWidget::findWatchItem(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
	int numChild = 	topLevelItemCount(), currentChild = 0;
	WatchItem* pWatchItem = NULL;
...
void WatchWidget::addWatch(shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{	m_pWatchTreeWidget->addWatch(spWatch); }

void WatchWidget::removeWatch(shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
...
void DebuggerToolTipWidget::showToolTip(const QPoint& pos, const QString &key, const ARL::Reflection::Variant &value)
{
...
bool populateValues(QTreeWidgetItem *pParentItem, const QString &key, const ARL::Reflection::Variant varValue)
{
	bool result = true;
	QString textToSet;
	...
	if(varValue.isType<boost::shared_ptr<ARL::Instance> >())
	{
		...
		for (ARL::Reflection::PropertyIterator iter = pInstance->properties_begin(); iter != pInstance->properties_end(); ++iter)
		{
			if (ARL::Reflection::RefPropertyDescriptor::isRefPropertyDescriptor((*iter).getDescriptor()))
			{
				const ARL::Reflection::RefPropertyDescriptor* desc = boost::polymorphic_downcast<const ARL::Reflection::RefPropertyDescriptor*>(&(*iter).getDescriptor());
				ARL::Reflection::DescribedBase* pBase = desc->getRefValue(pInstance.get());
				if (pBase)
				{
					ARL::Instance* instance = boost::polymorphic_downcast<ARL::Instance*>(pBase);
					if (instance)
					{
						if (!pInstance->contains(instance))
							populateValues(addChildren(pParentItem), desc->name.c_str(), ARL::shared_from(instance));
					}
					else
					{
	...
	{
		//get x, y
		textToSet = ARL::StringConverter<ARL::Vector2>::convertToString(varValue.get<ARL::Vector2>()).c_str();

		ARL::Vector2 vec2 = varValue.get<ARL::Vector2>();
	...
	{
		//get x, y
		textToSet = ARL::StringConverter<ARL::Vector2int16>::convertToString(varValue.get<ARL::Vector2int16>()).c_str();

		ARL::Vector2int16 vec2 = varValue.get<ARL::Vector2int16>();
	...
	{
		textToSet = ARL::StringConverter<ARL::Vector3>::convertToString(varValue.get<ARL::Vector3>()).c_str();

		ARL::Vector3 vec3 = varValue.get<ARL::Vector3>();
	...
	{
		textToSet = ARL::StringConverter<G3D::Vector3int16>::convertToString(varValue.get<G3D::Vector3int16>()).c_str();
	...
	{
		textToSet = ARL::StringConverter<G3D::CoordinateFrame>::convertToString(varValue.get<G3D::CoordinateFrame>()).c_str();

		G3D::CoordinateFrame coordinateFrame = varValue.get<G3D::CoordinateFrame>();
		populateValues(addChildren(pParentItem), "p", ARL::Reflection::Variant(coordinateFrame.translation));
		populateValues(addChildren(pParentItem), "lookVector", ARL::Reflection::Variant(coordinateFrame.lookVector()));
	}
	else if (varValue.isType<ARL::Region3>())
	{
		textToSet = ARL::StringConverter<ARL::Region3>::convertToString(varValue.get<ARL::Region3>()).c_str();

		ARL::Region3 region = varValue.get<ARL::Region3>();
		populateValues(addChildren(pParentItem), "CFrame", ARL::Reflection::Variant(region.getCFrame()));
		populateValues(addChildren(pParentItem), "Size", ARL::Reflection::Variant(region.getSize()));
	}
	else if (varValue.isType<ARL::Rect2D>())
	{
		textToSet = ARL::StringConverter<ARL::Rect2D>::convertToString(varValue.get<ARL::Rect2D>()).c_str();

		ARL::Rect2D rect2D = varValue.get<ARL::Rect2D>();
		populateValues(addChildren(pParentItem), "Min", ARL::Reflection::Variant(rect2D.x0y0()));
		populateValues(addChildren(pParentItem), "Max", ARL::Reflection::Variant(rect2D.x1y1()));
		populateValues(addChildren(pParentItem), "Width", ARL::Reflection::Variant(rect2D.width()));
		populateValues(addChildren(pParentItem), "Height", ARL::Reflection::Variant(rect2D.height()));
	}
	else if (varValue.isType<ARL::Region3int16>())
	{
		textToSet = ARL::StringConverter<ARL::Region3int16>::convertToString(varValue.get<ARL::Region3int16>()).c_str();

		ARL::Region3int16 region = varValue.get<ARL::Region3int16>();
		populateValues(addChildren(pParentItem), "Min", ARL::Reflection::Variant(region.getMinPos()));
		populateValues(addChildren(pParentItem), "Max", ARL::Reflection::Variant(region.getMaxPos()));
	}
	else if (varValue.isType<ARL::BrickColor>())
	{
		textToSet = ARL::StringConverter<ARL::BrickColor>::convertToString(varValue.get<ARL::BrickColor>()).c_str();

		ARL::BrickColor brickColor = varValue.get<ARL::BrickColor>();
		populateValues(addChildren(pParentItem), "Color", ARL::Reflection::Variant(brickColor.color3()));
		populateValues(addChildren(pParentItem), "Name", ARL::Reflection::Variant(std::string(brickColor.name())));
		populateValues(addChildren(pParentItem), "Number", ARL::Reflection::Variant((int)brickColor.number));
	}
	else if (varValue.isType<G3D::Color3>())
	{
		textToSet = ARL::StringConverter<G3D::Color3>::convertToString(varValue.get<G3D::Color3>()).c_str();

		G3D::Color3 color = varValue.get<G3D::Color3>();
		populateValues(addChildren(pParentItem), "r", ARL::Reflection::Variant(color.r));
		populateValues(addChildren(pParentItem), "g", ARL::Reflection::Variant(color.g));
		populateValues(addChildren(pParentItem), "b", ARL::Reflection::Variant(color.b));
	}
	else if (varValue.isType<ARL::RbxRay>())
	{
		textToSet = ARL::StringConverter<ARL::RbxRay>::convertToString(varValue.get<ARL::RbxRay>()).c_str();

		ARL::RbxRay ray = varValue.get<ARL::RbxRay>();
		populateValues(addChildren(pParentItem), "Origin", ARL::Reflection::Variant(ray.origin()));
		populateValues(addChildren(pParentItem), "Direction", ARL::Reflection::Variant(ray.direction()));
	}
	else if (varValue.isType<ARL::UDim>())
	{
		textToSet = ARL::StringConverter<ARL::UDim>::convertToString(varValue.get<ARL::UDim>()).c_str();

		ARL::UDim dim = varValue.get<ARL::UDim>();
		populateValues(addChildren(pParentItem), "Scale", ARL::Reflection::Variant(dim.scale));
		populateValues(addChildren(pParentItem), "Offset", ARL::Reflection::Variant(int(dim.offset)));
	}
	else if (varValue.isType<ARL::UDim2>())
	{
		textToSet = ARL::StringConverter<ARL::UDim2>::convertToString(varValue.get<ARL::UDim2>()).c_str();

		ARL::UDim2 dim2 = varValue.get<ARL::UDim2>();
		populateValues(addChildren(pParentItem), "X", ARL::Reflection::Variant(dim2.x));
		populateValues(addChildren(pParentItem), "Y", ARL::Reflection::Variant(dim2.y));
	}
	else if (varValue.isType<shared_ptr<const ARL::Reflection::ValueMap> >())
	{
		textToSet = "Map";
		shared_ptr<const ARL::Reflection::ValueMap> valMap = varValue.get<shared_ptr<const ARL::Reflection::ValueMap> >();

		for (ARL::Reflection::ValueMap::const_iterator iter = valMap->begin(); iter != valMap->end(); ++iter)
		{
	...
	{
		textToSet = "Array";
		shared_ptr<const ARL::Reflection::ValueArray> valMap = varValue.get<shared_ptr<const ARL::Reflection::ValueArray> >();
		ARL::Reflection::ValueArray::const_iterator iter = valMap->begin();
	...
	else if (varValue.isType<shared_ptr<const ARL::Instances> >())
	{
		textToSet = "Instances";
		shared_ptr<const ARL::Instances> valMap = varValue.get<shared_ptr<const ARL::Instances> >();
		ARL::Instances::const_iterator iter = valMap->begin();
	...
	else if (varValue.isType<shared_ptr<const ARL::Reflection::Tuple> >())
	{
		textToSet = "Tuple";
		shared_ptr<const ARL::Reflection::Tuple> valMap = varValue.get<shared_ptr<const ARL::Reflection::Tuple> >();
		ARL::Reflection::ValueArray::const_iterator iter = valMap->values.begin();
	...
	else if (varValue.isType<ARL::Lua::WeakFunctionRef>())
	{
		textToSet = "function";
