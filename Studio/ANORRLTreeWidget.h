...
class ANORRLTreeWidgetItem;
class ANORRLTreeWidget;
...
typedef std::set<boost::shared_ptr<ARL::Instance> > InstanceList_SPTR;
typedef std::set<ARL::Instance*>					InstanceList_PTR;
typedef std::set<ANORRLTreeWidgetItem*>             TreeWidgetItemList;
typedef std::set<ANORRLTreeWidgetItem*, DepthCompare> TreeWidgetItemSortedList;

typedef	std::pair<std::string, SearchParams>		StringParamPair;
typedef std::vector<StringParamPair>				ParamList;
typedef std::deque<shared_ptr<ARL::Instance> >		FilterDeque_SPTR;

typedef std::map<const ARL::Instance*, ANORRLTreeWidgetItem*> InstanceMap;

class InstanceSelectionHandler
{
public:
	virtual void onInstanceSelected(boost::shared_ptr<ARL::Instance> instance) = 0;
	virtual void onInstanceHovered(boost::shared_ptr<ARL::Instance> instance) = 0;
};
...
{
public:
	InstanceUpdateHandler(boost::shared_ptr<ARL::Instance> pInstance);
	virtual ~InstanceUpdateHandler();

	boost::shared_ptr<ARL::Instance> getInstance() { return m_pInstance; }

	void populateChildren(ANORRLTreeWidget* pTreeWidget);

	virtual ANORRLTreeWidget     *getTreeWidget() = 0;
	virtual ANORRLTreeWidgetItem *getItemParent() = 0;

	virtual bool onChildAdded(shared_ptr<ARL::Instance> pChild);
	virtual bool onChildRemoved(shared_ptr<ARL::Instance> pChild);
	virtual void onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor);
	...
	// these events.
	virtual void childAddedSignalHandler(shared_ptr<ARL::Instance> pChild) = 0;
	virtual void childRemovedSignalHandler(shared_ptr<ARL::Instance> pChild) = 0;
	virtual void propertyChangedSignalHandler(const ARL::Reflection::PropertyDescriptor* pDescriptor) = 0;
	
	void removeFromRemovalList(ANORRLTreeWidgetItem *pTreeWidgetItem);

	ANORRLTreeWidgetItem* processChildAdd(shared_ptr<ARL::Instance> pInstance);
	virtual bool processChildrenAdd();
	virtual void processChildRemove();
	virtual void processPropertyChange() {}

	void createWidgetItem(shared_ptr<ARL::Instance> pInstance);
...
class ANORRLTreeWidgetItem: public QObject, public QTreeWidgetItem, public InstanceUpdateHandler
{
    Q_OBJECT
public:
	ANORRLTreeWidgetItem(int index, ANORRLTreeWidget *pTreeWidget, boost::shared_ptr<ARL::Instance> pInstance);	
	ANORRLTreeWidgetItem(int index, ANORRLTreeWidgetItem *pParentWidgetItem, boost::shared_ptr<ARL::Instance> pInstance);
	...
	void aboutToDelete(ANORRLTreeWidget* pTreeWidget);

	static bool isExplorerItem(const shared_ptr<ARL::Instance>& pInstance);
	static int  getImageIndex(const shared_ptr<ARL::Instance>& pInstance);
	static int  getItemType(const shared_ptr<ARL::Instance>& pInstance);

	void processPropertyChange() override;

	bool handleOpen();
	int getIndexToInsertAt(shared_ptr<ARL::Instance> pInstance);
...
protected:
	...
	void childRemovedSignalHandler(shared_ptr<ARL::Instance> pchild) override
	{
		QMetaObject::invokeMethod(this, "onChildRemoved", Qt::QueuedConnection, Q_ARG(shared_ptr<ARL::Instance>, pchild));
	}
	void propertyChangedSignalHandler(const ARL::Reflection::PropertyDescriptor* pDescriptor) override
	{
		QMetaObject::invokeMethod(this, "onPropertyChanged", Qt::QueuedConnection, Q_ARG(const ARL::Reflection::PropertyDescriptor*, pDescriptor));
	}

private:
	void setData(int column, int role, const QVariant & value) override;

	Q_INVOKABLE bool onChildAdded(shared_ptr<ARL::Instance> pChild) override;
	Q_INVOKABLE bool onChildRemoved(shared_ptr<ARL::Instance> pChild) override;
	// override just to mark as Q_SLOT
	Q_INVOKABLE void onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor) override
	{
...
class ANORRLTreeWidget : public QTreeWidget, public InstanceUpdateHandler
{
	Q_OBJECT
public:
	ANORRLTreeWidget(boost::shared_ptr<ARL::DataModel> pDataModel);
	virtual ~ANORRLTreeWidget();
	...
	void eraseInstance(ARL::Instance *pInstance);
	ANORRLTreeWidgetItem* findItemFromInstance(const ARL::Instance *pInstance);

	void scrollToInstance(ARL::Instance *pInstance);
	...
	void addToUpdateList(ANORRLTreeWidgetItem *pTreeWidgetItem);
	void removeFromUpdateList(ANORRLTreeWidgetItem *pTreeWidgetItem);

	void requestDelete();

	void requestViewportUpdate();

	void updateSelectionState(ARL::Instance *pInstance);

	void processChildrenRemoval();
	void requestItemDelete(ANORRLTreeWidgetItem* item);

	void removeItemFromRemovalList(ANORRLTreeWidgetItem* item)
	{
		m_itemsToRemove.erase(item);
	}

	bool existsInRemovalList(ANORRLTreeWidgetItem* item)
	{
	...
	bool filterInstance(shared_ptr<ARL::Instance> instance);

	bool filterAncestors(shared_ptr<ARL::Instance> pInstance);

	bool filterWidgetRecursive(shared_ptr<ARL::Instance> instance, FilterDeque_SPTR& previousFilterStack, const ARL::Time& startTime);
	...
	bool isFoundItem(shared_ptr<ARL::Instance> instance);
	bool isAncestorOfFoundItem(shared_ptr<ARL::Instance> instance);

	void addFilterItemsToUpdateList();

	ANORRLTreeWidgetItem* lastSelectedItem()
	{
	...
	void addItemToSelection(ARL::Instance* item)
	{
	...
	void itemDeletionRequested(ANORRLTreeWidgetItem* pItem);
...
private Q_SLOTS:
	...
	bool onChildAdded(shared_ptr<ARL::Instance> pChild) override;
	bool onChildRemoved(shared_ptr<ARL::Instance> pChild) override;
	void onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor) override
	{
...
private:	
	void onDescendantAdded(shared_ptr<ARL::Instance> pDescendant);
	void onDescendantRemoved(shared_ptr<ARL::Instance> pDescendant);
	...
	void initTreeView(boost::shared_ptr<ARL::DataModel> pDataModel);
	void createTreeRoot(const boost::shared_ptr<ARL::Instance> instance);
	
	void onInstanceSelectionChanged(const ARL::SelectionChanged& evt);
	bool isValidSelection(ARL::Instance *pInstance);	
	...
	bool isSearchTimeUp(const ARL::Time& startTime, int duration);
	...
	std::set<shared_ptr<ARL::Instance> >	 m_foundItems;
	std::set<shared_ptr<ARL::Instance> >	 m_ancestorsOfFoundItems;
	...
	ANORRLTreeWidgetItem*					 m_lastSelectedItem;
	
	FilterDeque_SPTR						 m_currentFilterStack;

	ARL::Time								 m_filterStartTime;
...
class ANORRLExplorerWidget : public QWidget
{
	Q_OBJECT
public:
	ANORRLExplorerWidget(QWidget *pParent = 0);
	
	void setCurrentWidget(ANORRLTreeWidget* treeWidget);

	ANORRLTreeWidget* getTreeWidget() { return m_anorrlTreeWidget; }
...
private:
	...
	ANORRLTreeWidget*		m_anorrlTreeWidget;
