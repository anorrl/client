...
public:
	...
	void setFromValueTable(const boost::shared_ptr<const ARL::Reflection::ValueTable>& valueTable);
	...
	{
		boost::optional<ARL::Reflection::Variant> var = getVariant(fieldName);
	...
	boost::optional<ARL::Reflection::Variant> getVariant(const std::string& fieldName);
...
private:
	...
	ARL::Reflection::ValueTable data;
...
class ANORRLGameExplorer : public QTreeView, public ARL::AssetFetchMediator
{
    Q_OBJECT
public:
	ANORRLGameExplorer(QWidget* parent);

// Asset management functions
	virtual boost::optional<std::string> findCachedAssetOrEmpty(const ARL::ContentId& contentId, int universeId);
	void updateAsset(const ARL::ContentId& contentId, const std::string& newContents);
	void notifyIfUnpublishedChanges(int universeId);
	void publishNamedAssetsToCurrentSlot();
	static ARL::HttpFuture publishScriptAsset(const std::string& scriptText,
...
private:
    ...
    static void handleUniverseForPlaceResponse(QPointer<ANORRLGameExplorer> explorer,
        int originatingSessionId, int placeId, std::string* data, std::exception* error);
	static void doneLoadingDataForCategory(QPointer<ANORRLGameExplorer> explorer,
