...
TSJobItem* ArbiterItem::getOrCreateTSJobItem(boost::shared_ptr<const ARL::TaskScheduler::Job> pJob)
{
	int currentIndex = 0;
...
TSJobItem::TSJobItem(ArbiterItem* pParentItem, boost::shared_ptr<const ARL::TaskScheduler::Job> pJob)
: m_pJob(pJob)
{
...
{
	bool isSleeping = m_pJob->getSleepingTime() > ARL::Time::Interval(5);
	if (!isSleeping)
	{
		double error = m_pJob->averageError();
		setText(1, ARL::format("%.2f", error).c_str());

		double priority = m_pJob->getPriority();
		setText(2, ARL::format("%.1f", priority).c_str());

		double dutyCycle = m_pJob->averageDutyCycle();
		setText(3, ARL::format("%.1f%%", 100.0 * dutyCycle).c_str());

		double sps = m_pJob->averageStepsPerSecond();
		setText(4, ARL::format("%.1f/s", sps).c_str());

		double cv = m_pJob->getStepStats().stepInterval().coefficient_of_variation();
		setText(5, ARL::format("%.1f%%", 100.0 * cv).c_str());

		double step = m_pJob->averageStepTime() * 1000;
		setText(6, ARL::format("%.3fms", step).c_str());
	}
	else
	{
...
ANORRLTaskScheduler::ANORRLTaskScheduler()
...
void ANORRLTaskScheduler::updateValues()
{
	...
	Jobs jobs;
	ARL::TaskScheduler::singleton().getJobsInfo(jobs);
	...
	{
		boost::shared_ptr<const ARL::TaskScheduler::Job> pJob = (*iter);
		if(pJob)
		{
			boost::shared_ptr<ARL::TaskScheduler::Arbiter> pArbiter(pJob->getArbiter());
			if (!pArbiter)
				continue;

			pArbiterItem = getOrCreateArbiterItem(pArbiter);
			ARLASSERT(pArbiterItem);
...
void ANORRLTaskScheduler::setVisible(bool visible)
{
	if(visible)
	{
		m_pTimer->start();
...
ArbiterItem* ANORRLTaskScheduler::getOrCreateArbiterItem(boost::shared_ptr<ARL::TaskScheduler::Arbiter> pArbiter)
{
...
void ANORRLTaskScheduler::syncTSJobItems(const std::set<TSJobItem *> &currentTSJobItems)
{
