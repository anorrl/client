...
{	
    ...
    if (FFlag::UseBuildGenericGameUrl)
    {
        logoutUrl = QString::fromStdString(BuildGenericGameUrl(ANORRLSettings::getBaseURL().toStdString(),"game/logout.aspx"));
    }
    else
    {
...
{
	userAgentStr = ANORRLNetworkAccessManager::Instance().getUserAgent();
	if (FFlag::StudioInSyncWebKitAuthentication)
		qApp->installEventFilter(this);
    
	connect(&ANORRLNetworkAccessManager::Instance(), SIGNAL(sslErrors(QNetworkReply*, const QList<QSslError> & )),
...
{
	return ARL::MachineIdUploader::uploadMachineId(qPrintable(ANORRLSettings::getBaseURL()))
...
bool AuthenticationHelper::eventFilter(QObject * watched, QEvent * evt)
{
	...
	{
        // save webkit cookies
		savedCookieKey = ANORRLNetworkAccessManager::Instance().cookieJar()->getCookieValue(ANORRLSettings::getBaseURL(), "ANORRLSECURITY");
	}
	else if (eventType == QEvent::ApplicationActivate)
	{
		QString currentCookieKey = ANORRLNetworkAccessManager::Instance().cookieJar()->getCookieValue(ANORRLSettings::getBaseURL(), "ANORRLSECURITY");
		if (QString::compare(savedCookieKey, currentCookieKey, Qt::CaseSensitive) != 0)	
		{
			...
			{
				ANORRLNetworkAccessManager::Instance().cookieJar()->reloadCookies(true);
			}
			...
			ANORRLWebDoc* startPage = dynamic_cast<ANORRLWebDoc*>(ANORRLDocManager::Instance().getStartPageDoc());
			if (startPage)
...
void AuthenticationHelper::authenticateUserAsync(const QString& url, const QString& ticket)
{
	if (!url.isEmpty() && !ticket.isEmpty())
	{
        ...
        ANORRLNetworkAccessManager::Instance();
	...
	else
	{
		...
		QString secCookie = ANORRLNetworkAccessManager::Instance().cookieJar()->getCookieValue(ANORRLSettings::getBaseURL(), "ANORRLSECURITY");
...
bool AuthenticationHelper::authenticateHttpSession()
{
	...
	cookieJar->setCookiesFromUrl(ANORRLNetworkAccessManager::Instance().cookieJar()->cookiesForUrl(baseURL), baseURL);
...
bool AuthenticationHelper::authenticateQtWebkitSession(const QString& authenticationUrl)
{
	...
	if (rbxNetworkReply->waitForFinished(10000, 100))
	{
		if (rbxNetworkReply->statusCode() == 200)
		{
			...
			ANORRLNetworkAccessManager::Instance().cookieJar()->setCookiesFromUrl(accessManager->cookieJar()->cookiesForUrl(baseURL), baseURL);
			result = true;
		}
	}
	else
