	...
	, startPlaceOpenRequested(false)
{
	...
	CategoryUiSettings placeSettings =
	{
		...
		boost::bind(&ANORRLGameExplorer::placeDoubleClickCallback, this, _1),
		boost::bind(&ANORRLGameExplorer::placeContextMenuHandler, this, _1, _2),
		boost::bind(&ANORRLGameExplorer::placeGroupContextMenuHandler, this, _1),
		boost::bind(&ANORRLGameExplorer::checkRowForNameUpdate, this, ENTITY_CATEGORY_Places, _1),
	...
	CategoryUiSettings devProductSettings =
	{
		...
		boost::bind(&ANORRLGameExplorer::developerProductContextMenuHandler, this, _1, _2),
		boost::bind(&ANORRLGameExplorer::developerProductGroupContextMenuHandler, this, _1),
		boost::bind(&ANORRLGameExplorer::checkRowForNameUpdate, this, ENTITY_CATEGORY_DeveloperProducts, _1),
		CategoryUiSettings::FlatNames
	};
	...
	CategoryUiSettings namedAssetSettings =
	{
		...
		boost::bind(&ANORRLGameExplorer::namedAssetsDoubleClickCallback, this, _1),
		boost::bind(&ANORRLGameExplorer::namedAssetsContextMenuHandler, this, _1, _2),
		boost::bind(&ANORRLGameExplorer::namedAssetsGroupContextMenuHandler, this, _1),
		boost::bind(&ANORRLGameExplorer::checkRowForNameUpdate, this, ENTITY_CATEGORY_NamedAssets, _1),
...
boost::optional<std::string> ANORRLGameExplorer::findCachedAssetOrEmpty(const ARL::ContentId& contentId, int universeId)
{
...
void ANORRLGameExplorer::updateAsset(const ARL::ContentId& contentId, const std::string& newContents)
{
...
void ANORRLGameExplorer::notifyIfUnpublishedChanges(int universeId)
{
...
void ANORRLGameExplorer::publishNamedAssetsToCurrentSlot()
{
	ARLASSERT(currentGameId != kNoGameLoaded);
	...
	entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<boost::shared_future<void> >(
		boost::bind(&ANORRLGameExplorer::publishIfThereAreLocalModifications, this, _1), &publishFutures);
...
HttpFuture ANORRLGameExplorer::publishScriptAsset(const std::string& scriptText,
...
void ANORRLGameExplorer::getListOfImages(std::vector<QString>* out)
{
	if (currentGameId == kNoGameLoaded)
	{
		return;
...
void ANORRLGameExplorer::getListOfAnimations(std::vector<QString>* out)
{	
	if (currentGameId == kNoGameLoaded)
		return;
...
QString ANORRLGameExplorer::getAnimationsDataJson()
{
	if (currentGameId == kNoGameLoaded)
		return QString();
...
void ANORRLGameExplorer::openGameFromGameId(int gameId)
{
...
void ANORRLGameExplorer::doOpenGameFromGameId(int gameId)
{
	...
	UpdateUIManager::Instance().getMainWindow().publishGameAction->setEnabled(!ANORRLIDEDoc::getIsCloudEditSession());
...
void ANORRLGameExplorer::openGameFromPlaceId(int placeId)
{
    ...
    getUniverse.get(boost::bind(&ANORRLGameExplorer::handleUniverseForPlaceResponse, QPointer<ANORRLGameExplorer>(this),
        currentSessionId, placeId, _1, _2));
}

void ANORRLGameExplorer::onCloseIdeDoc()
{
	currentPlaceId = kNoGameLoaded;
	if (currentGameId == kNoGameLoaded)
	{
...
static void kickOffPublish()
{
	...
	Verb* v = ideDoc->getVerb("PublishToANORRLAsVerb");
	ARLASSERT(v);
	if (!v) return;
...
void ANORRLGameExplorer::nonGameLoaded()
{
    currentGameId = kNoGameLoaded;
    currentPlaceId = kNoGameLoaded;
...
std::pair<int, boost::function<std::string()> > ANORRLGameExplorer::buildPlaceContentPair(EntityProperties* properties)
{
...
ANORRLGameExplorer::NamedAssetCopyInfo ANORRLGameExplorer::buildNamedContentInfo(
...
void ANORRLGameExplorer::publishGameThread(boost::function<int()> newUniverseFuture, bool* succeeded, int* targetUniverseId)
{
	*targetUniverseId = -1;
	*succeeded = false;
	try
	{
		*targetUniverseId = newUniverseFuture();
		...
		entitySettings[ENTITY_CATEGORY_Places]->transform<std::pair<int, boost::function<std::string()> > >(
			boost::bind(&ANORRLGameExplorer::buildPlaceContentPair, this, _1), &placeContentFutures);
		...
		entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<NamedAssetCopyInfo>(
			boost::bind(&ANORRLGameExplorer::buildNamedContentInfo, this,
...
void ANORRLGameExplorer::publishGameToNewSlot()
{
...
void ANORRLGameExplorer::publishGameToNewGroupSlot(int groupId) {
...
void ANORRLGameExplorer::publishGameToExistingSlot(int gameId)
{
	publishInternal(boost::bind(&identity<int>, gameId));
}

void ANORRLGameExplorer::handleUniverseForPlaceResponse(QPointer<ANORRLGameExplorer> explorer,
...
void ANORRLGameExplorer::doneLoadingDataForCategory(QPointer<ANORRLGameExplorer> explorer, int originatingSessionId, EntityCategory category)
{
	if (explorer.isNull()) return;
	if (explorer->currentSessionId != originatingSessionId) return;
...
QStandardItemModel* ANORRLGameExplorer::getModel()
{
    return dynamic_cast<QStandardItemModel*>(model());
}

QStandardItem* ANORRLGameExplorer::findGroup(EntityCategory category)
{
...
QStandardItem* ANORRLGameExplorer::makeGroup(EntityCategory category)
{
...
void ANORRLGameExplorer::openAndFocusConfigureDoc(const std::string& url)
{
	ANORRLWebDoc* webDoc = UpdateUIManager::Instance().getMainWindow().getConfigureWebDoc();
	webDoc->open(&UpdateUIManager::Instance().getMainWindow(), QString::fromStdString(url));
}

void ANORRLGameExplorer::setGroupLoadingStatus(EntityCategory category)
{
...
void ANORRLGameExplorer::placeDoubleClickCallback(EntityProperties* placeInfo)
{
	openPlace(getEntityId(placeInfo, ENTITY_CATEGORY_Places).toInt());
}

void ANORRLGameExplorer::namedAssetsDoubleClickCallback(EntityProperties* properties)
{
...
void ANORRLGameExplorer::openPlace(int placeId)
{
	if (placeId != currentPlaceId)
	{
		UpdateUIManager::Instance().getMainWindow().handleFileOpen("", IANORRLDoc::IDE, QString::fromStdString(
			formatUrl(
	...
	else
	{
		...
		if (ANORRLIDEDoc* ideDoc = ANORRLDocManager::Instance().getPlayDoc())
		{
			ANORRLDocManager::Instance().setCurrentDoc(ideDoc);
		}
	}
}

void ANORRLGameExplorer::refreshOpenedPlaceIndicator()
{
...
void ANORRLGameExplorer::openStartPlaceIfRequestedAndNoPlaceOpenedAndDataReady()
{
...
void ANORRLGameExplorer::updateItemForOpenedPlaceState(QStandardItem* placeItem)
{
...
void ANORRLGameExplorer::publishInternal(boost::function<int()> newUniverseFuture)
{
	...
	UpdateUIManager::Instance().waitForLongProcess("Publishing Game...",
		boost::bind(&ANORRLGameExplorer::publishGameThread, this,
...
void ANORRLGameExplorer::failedToLoadSettings(int originatingSessionId)
{
...
void ANORRLGameExplorer::populateWithLoadedData(int originatingSessionId, int cat)
{
	...
	entitySettings[category]->transform(boost::function<QStandardItem*(EntityProperties*)>(
		boost::bind(&ANORRLGameExplorer::buildItem, boost::ref(settings), boost::ref(kWebSettings[cat]), group, _1)),
		&items);
...
static void invokeThumbnailLoadedForImage(ANORRLGameExplorer* explorer, int originatingSessionId, QModelIndex item, HttpFuture future)
{
	QVariant var;
...
void ANORRLGameExplorer::afterBadgesFinished(int originatingSessionId)
{
	if (originatingSessionId != currentSessionId) return;
...
void ANORRLGameExplorer::afterNamedAssetsFinished(int originatingSessionId)
{
	if (originatingSessionId != currentSessionId) return;
...
void ANORRLGameExplorer::afterNamedAssetsFinishedRecursive(QStandardItem* root, QStringList& imagesToReload)
{
...
void ANORRLGameExplorer::thumbnailLoadedForImage(int originatingSessionId, QModelIndex index, QVariant future)
{
	if (originatingSessionId != currentSessionId) return;
...
void ANORRLGameExplorer::addNewPlace()
{
...
void ANORRLGameExplorer::placeContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
	QMenu menu(this);
...
void ANORRLGameExplorer::developerProductContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
	QMenu menu(this);
...
void ANORRLGameExplorer::namedAssetsContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
	QMenu menu(this);
	...
	if (result == remove)
	{
		...
		{
			ANORRLDocManager::Instance().closeDoc(
	...
	else if (revertToLastPublished && result == revertToLastPublished)
	{
		...
		if (mb.exec() == QMessageBox::Yes)
		{
			...
			refreshNamedScriptIcons(currentSessionId);
			if (ANORRLScriptDoc* scriptDoc = ANORRLDocManager::Instance().findOpenScriptDoc(
				LuaSourceBuffer::fromContentId(ContentId::fromGameAssetName(name))))
			{
...
void ANORRLGameExplorer::placeGroupContextMenuHandler(const QPoint& point)
{
	QMenu menu(this);
...
void ANORRLGameExplorer::developerProductGroupContextMenuHandler(const QPoint& point)
{
	QMenu menu(this);
...
void ANORRLGameExplorer::handleRename(const QPoint& globalPoint)
{
...
void ANORRLGameExplorer::bulkAddNewImageNames()
{
	ANORRLSettings settings;
	QString imageLastDir = settings.value(kLastImageDirectoryKey).toString();
	if (imageLastDir.isEmpty())
...
void ANORRLGameExplorer::insertNamedImage(EntityProperties* imageInfo)
{
...
void ANORRLGameExplorer::doneEditingUniverseName()
{
...
void ANORRLGameExplorer::reloadDataFromWeb()
{
	currentSessionId++;
	...
	if (!gameSettings.currentUserHasAccess() && !ANORRLIDEDoc::getIsCloudEditSession())
	{
		nonGameLoaded();
		return;
	}
	...
	for (unsigned int i = 0; i < ENTITY_CATEGORY_MAX; ++i)
	{
		...
		entitySettings[(EntityCategory)i] = EntityPropertiesForCategory::loadFromWeb(
			...
			boost::bind(&doneLoadingDataForCategory, QPointer<ANORRLGameExplorer>(this),
			currentSessionId, (EntityCategory)i));
	}
}

void ANORRLGameExplorer::doubleClickHandler(const QModelIndex& modelIndex)
{
...
void ANORRLGameExplorer::contextMenuHandler(const QPoint& point)
{
...
void ANORRLGameExplorer::itemChangedHandler(QStandardItem* updatedItem)
{
...
void ANORRLGameExplorer::handleNameUpdates()
{
...
void ANORRLGameExplorer::checkRowForNameUpdate(EntityCategory category, QStandardItem* entityRow)
{
		...
		else
		{
			...
			if (category == ENTITY_CATEGORY_NamedAssets)
			{
				...
				if (assetTypeId == (int)ASSET_TYPE_ID_Script)
				{
					...
					if (ANORRLScriptDoc* scriptDoc = ANORRLDocManager::Instance().findOpenScriptDoc(oldLsb))
					{
...
bool ANORRLGameExplorer::namedAssetHasUnpublishedChanges(const std::string& name)
{
	return QFile(getUnpublishedAssetFilePath(name, currentGameId)).exists();
}

boost::shared_future<void> ANORRLGameExplorer::publishIfThereAreLocalModifications(EntityProperties* settings)
{
