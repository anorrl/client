...
public:
	ScriptTraversalThread(ScriptTextEditor * parent, ANORRLScriptDoc * document);
...
public:
    CheckSyntaxThread(ScriptTextEditor *parent, ANORRLScriptDoc *document)
...
public:
	...
	FindThread(ScriptTextEditor *parent, ANORRLScriptDoc *document)
		: ScriptTraversalThread(parent, document)
		, m_goToNextItem(true)
...
public:
	...
	ScriptTextEditor(ANORRLScriptDoc *pScriptDoc,ANORRLMainWindow *pParentWidget);
	~ScriptTextEditor();

	Q_INVOKABLE void setErrorLine(int errorLine, QString errorMessage);
	Q_INVOKABLE void setAnalysisResult(const ARL::ScriptAnalyzer::Result& analysisResults);
	...
	void selectText(const ARL::ScriptAnalyzer::Location& location, bool trySettingBlocksVisible = true);
...
private:
	...
	void populateValuesFromLocation(const ARL::ScriptAnalyzer::Location& location, int& startPos, int& endPos, bool& atEof);

	int getStartPosition(const ARL::ScriptAnalyzer::Location& location);
	int getEndPosition(const ARL::ScriptAnalyzer::Location& location);
	...
	ARL::BaldPtr<ANORRLScriptDoc>           m_pScriptDoc;
	ARL::BaldPtr<ScriptSyntaxHighlighter>   m_pSyntaxHighlighter;
	ARL::BaldPtr<ScriptSideWidget>          m_pSideWidget;
	ARL::BaldPtr<CheckSyntaxThread>         m_pCheckSyntaxThread;
	...
	boost::optional<std::vector<ARL::ScriptAnalyzer::IntellesenseResult> >			m_analysisResult;
	...
	typedef ARL::DenseHashMap<const char*, std::pair<int, std::string>, ARL::Reflection::StringHashPredicate, ARL::Reflection::StringEqualPredicate> WikiLookup;
