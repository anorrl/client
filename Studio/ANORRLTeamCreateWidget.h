...
private Q_SLOTS:
	void groupDataLoaded(ARL::HttpFuture future);
	void groupThumbnailLoaded(ARL::HttpFuture future);
...
private:
	...
	void doneLoading();
	Q_INVOKABLE void creatorsListLoaded(int pageLoaded, ARL::HttpFuture future);
...
private:
	...
	Q_INVOKABLE void friendsDataLoaded(int pageLoaded, ARL::HttpFuture future);
...
class CreatorsListWidget : public QTreeWidget
{
	Q_OBJECT
public:
	CreatorsListWidget(QWidget* parent, shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> dataManager);
...
protected:	
	void onPlayerAdded(boost::shared_ptr<ARL::Instance> player);
	void onPlayerRemoving(boost::shared_ptr<ARL::Instance> player);	
...
class UserPlaceCreatorsListWidget: public CreatorsListWidget
{
	Q_OBJECT
public:
	UserPlaceCreatorsListWidget(QWidget* parent, shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> dataManager);
...
class GroupPlaceCreatorsListWidget: public CreatorsListWidget
{
	Q_OBJECT
public:
	GroupPlaceCreatorsListWidget(QWidget* parent, shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> dataManager);
	void addOwnerDetails(const PlaceCreatorDetails& creatorDetails) override;
...
class ANORRLTeamCreateWidget : public QWidget, public Ui::TeamCreate
{
	Q_OBJECT
public:
	ANORRLTeamCreateWidget(QWidget* parent);
	void setDataModel(boost::shared_ptr<ARL::DataModel> dataModel, boost::shared_ptr<PlayersDataManager> pPlayersDataManager);
...
private:
	...
	void onPlaceIdChanged(const ARL::Reflection::PropertyDescriptor* desc);
	
	void enableFriendsInviteEdit(bool state);
	void loadFriendsList();

	void updatePlaceDetails(ARL::HttpFuture future);
	eTeamType getCreatorType();

	void updatePlaceManageStatus(ARL::HttpFuture future);
