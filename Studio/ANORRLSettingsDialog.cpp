...
static const char* GeometrySetting = "ANORRLSettingsDialog/Geometry";
static const char* SplitterStateSetting = "ANORRLSettingsDialog/SplitterState";

ANORRLSettingsDialog::ANORRLSettingsDialog(QWidget* parent)
...
ANORRLSettingsDialog::~ANORRLSettingsDialog()
{
    // save window state
    QSettings settings;
...
void ANORRLSettingsDialog::onReset()
{
    ...
    if ( !hasSeenWarning )
    {
        QMessageBox::information(
            this,
            "Reset Settings",
            "Resetting settings will not take effect until you restart ANORRL" );
        hasSeenWarning = true;
    }

	ARL::GlobalAdvancedSettings::singleton()->eraseSettingsStore();
}

bool ANORRLSettingsDialog::close()
{
    static bool hasSeenWarning = false;
...
void ANORRLSettingsDialog::populate()
{
	boost::shared_ptr<ARL::Instance> container = ARL::shared_from(ARL::GlobalAdvancedSettings::singleton().get());
	if(!container)
		return;
	...
	container->visitDescendants(boost::bind(&ANORRLSettingsDialog::onDescendantAdded, this, _1));
	m_descendantAddedConnection = container->getOrCreateDescendantAddedSignal()->connect(boost::bind(&ANORRLSettingsDialog::onDescendantAdded, this, _1));
	m_descendantRemovingConnection = container->getOrCreateDescendantRemovingSignal()->connect(boost::bind(&ANORRLSettingsDialog::onDescendantRemoving, this, _1));
	...
	ARL::Selection* sel = ARL::ServiceProvider::create<ARL::Selection>(ARL::GlobalAdvancedSettings::singleton().get());
...
void ANORRLSettingsDialog::onDescendantAdded(boost::shared_ptr<ARL::Instance> child)
{
	if(!child)
		return;

	const ARL::Reflection::ClassDescriptor* pClassDescriptor = &child->getDescriptor();
	if(pClassDescriptor)
	{
...
bool ANORRLSettingsDialog::hasProperties(const ARL::Reflection::ClassDescriptor* pClassDescriptor)
{
	if(!pClassDescriptor)
		return false;

	bool retVal = false;
	ARL::Reflection::ClassDescriptor::ClassDescriptors::const_iterator iter = pClassDescriptor->derivedClasses_begin();
	for(; iter != pClassDescriptor->derivedClasses_end(); iter++)
	{
		ARL::Reflection::ClassDescriptor* pDerived = *iter;
		if(pDerived)
	...
	if(!retVal)
	{
		ARL::Reflection::MemberDescriptorContainer<ARL::Reflection::PropertyDescriptor>::Collection::const_iterator iter = pClassDescriptor->begin<ARL::Reflection::PropertyDescriptor>();
		for(; iter != pClassDescriptor->end<ARL::Reflection::PropertyDescriptor>(); iter++)
		{
			const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor = *iter;
			if(pPropertyDescriptor)
			{
				if ( pPropertyDescriptor->owner != *pClassDescriptor )
					continue;

				if ( !pPropertyDescriptor->isPublic() || pPropertyDescriptor->security != ARL::Security::None )
				{
					continue;
                }

				ARL::Reflection::Metadata::Member* pMemeber = ARL::Reflection::Metadata::Reflection::singleton()->get(*pPropertyDescriptor);
				if(ARL::Reflection::Metadata::Item::isDeprecated(pMemeber, *pPropertyDescriptor))
					continue;
...
void ANORRLSettingsDialog::onDescendantRemoving(boost::shared_ptr<ARL::Instance> child)
{
	if(!child)
		return;
...
void ANORRLSettingsDialog::onCategorySelected(QTableWidgetItem* current, QTableWidgetItem* previous)
{
	if(!current)
		return;
	...
	ARL::Selection* sel = m_SettingsTree->getSelection();
	if(!sel)
		return;
...
void SelectionPropertyTree::addSelectionInstance(boost::shared_ptr<ARL::Instance> pInstance)
{
	if (!pInstance)
		return;
...
void SelectionPropertyTree::removeSelectionInstance(boost::shared_ptr<ARL::Instance> pInstance)
{
	if (!pInstance)
		return;
...
void SelectionPropertyTree::setSelection(ARL::Selection* value)
{
...
void SelectionPropertyTree::onSelectionChanged(const ARL::SelectionChanged& evt)
{
...
{
	QApplication::postEvent(this, new ANORRLCustomEvent(PROPERTY_WIDGET_UPDATE));
}

void SelectionPropertyTree::initProperty(const ARL::Reflection::ClassDescriptor* pClassDescriptor, 
										  const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
	...
	if (pPropertyDescriptor->owner != ARL::Instance::classDescriptor())
	{
		PropertyTreeWidget::initProperty(pClassDescriptor, pPropertyDescriptor);
	}
}

SettingsItem::SettingsItem(boost::shared_ptr<ARL::Instance> inst, QString name)
