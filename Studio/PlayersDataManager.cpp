...
// PlayersDataManager
//--------------------------------------------------------------------------------------------
PlayersDataManager::PlayersDataManager(QObject* parent, boost::shared_ptr<ARL::DataModel> dataModel)
: QObject(parent)
...
static void invokePlayerDataLoaded(QPointer<PlayersDataManager> playersDataManager, ARL::HttpFuture future)
{
	if (playersDataManager.isNull())
		return;
	QMetaObject::invokeMethod(playersDataManager.data(), "onPlayerNameLoaded", Qt::QueuedConnection, Q_ARG(ARL::HttpFuture, future));
...
void PlayersDataManager::getPlayerName(int playerId, QObject* resultListener, const char* callbackName)
{
	...
	{
		QString str = QString("%1/Users/%2").arg(ANORRLSettings::getApiBaseURL()).arg(playerId);
		ARL::HttpAsync::get(str.toStdString()).then(boost::bind(&invokePlayerDataLoaded, QPointer<PlayersDataManager>(this), _1));
...
shared_ptr<const ARL::Instances> PlayersDataManager::getLastKnownSelection(int playerId)
{
	...
	static shared_ptr<const ARL::Instances> emptyInstances(new ARL::Instances());
	return emptyInstances;
}

bool PlayersDataManager::isLocalPlayer(boost::shared_ptr<ARL::Network::Player> player)
{
	ARL::Network::Players* players = m_pDataModel->find<ARL::Network::Players>();
	return (players && players->getLocalPlayer() == player.get());
}

boost::shared_ptr<ARL::Network::Player> PlayersDataManager::getPlayer(const QString& playerName, Qt::CaseSensitivity cs)
{
	if (!playerName.isEmpty())
	{
		...
		if (players)
		{
			boost::shared_ptr<const ARL::Instances> playersList = players->getPlayers();
			for (ARL::Instances::const_iterator iter = playersList->begin(); iter != playersList->end(); ++iter)
			{
	...
	return shared_ptr<ARL::Network::Player>();
}
...
void PlayersDataManager::onPlayerAdded(shared_ptr<ARL::Instance> playerInstance)
{
	if (shared_ptr<ARL::Network::Player> player = ARL::Instance::fastSharedDynamicCast<ARL::Network::Player>(playerInstance))
	{
...
void PlayersDataManager::onPlayerRemoving(shared_ptr<ARL::Instance> playerInstance)
{
	if (shared_ptr<ARL::Network::Player> player = ARL::Instance::fastSharedDynamicCast<ARL::Network::Player>(playerInstance))
	{
...
void PlayersDataManager::onCloudEditSelectionChanged(int playerId, shared_ptr<const ARL::Reflection::ValueArray>& newSelection)
{
	shared_ptr<ARL::Instances> inst(new ARL::Instances());
	inst->reserve(newSelection->size());
	for (auto& var : *newSelection)
	{
		if (var.isType<shared_ptr<ARL::Instance> >())
		{
			if (shared_ptr<ARL::Instance> i = var.cast<shared_ptr<ARL::Instance> >())
			{
				inst->push_back(i);
			}
		}
...
{
	QUrl url(ANORRLSettings::getBaseURL());
...
void PlayersDataManager::onPlayerNameLoaded(ARL::HttpFuture future)
{
...
// AvatarLoader
//--------------------------------------------------------------------------------------------
static void invokeHeadshotThumbnailDataLoaded(QPointer<AvatarLoader> avatarLoader, int playerId, ARL::HttpFuture future)
{
	if (avatarLoader.isNull())
		return;
	QMetaObject::invokeMethod(avatarLoader.data(), "headshotThumbnailDataLoaded", Qt::QueuedConnection, 
		Q_ARG(int, playerId), Q_ARG(ARL::HttpFuture, future));
}

static void invokeHeadshotThumbnailLoaded(QPointer<AvatarLoader> avatarLoader, int playerId, ARL::HttpFuture future)
{
	if (avatarLoader.isNull())
		return;
...
, m_bFinishedLoading(false)
{
	...
	QString str = QString("%1/thumbnail/avatar-headshot?userid=%2").arg(ANORRLSettings::getBaseURL()).arg(playerId);
	ARL::HttpAsync::get(str.toStdString()).then(boost::bind(&invokeHeadshotThumbnailDataLoaded, QPointer<AvatarLoader>(this), playerId, _1));
}

void AvatarLoader::headshotThumbnailDataLoaded(int playerId, ARL::HttpFuture future)
{
	...
	try
	{
		...
		boost::shared_ptr<const ARL::Reflection::ValueTable> table;
		if (ARL::WebParser::parseJSONTable(future.get(), table) && table->size() > 0)
		{
			ARL::Reflection::ValueTable::const_iterator finalStrIter = table->find("Final");
			if (finalStrIter != table->end() && finalStrIter->second.isType<bool>() && finalStrIter->second.get<bool>())
			{
				ARL::Reflection::ValueTable::const_iterator thumnailUrlIter = table->find("Url");
				if (thumnailUrlIter != table->end() && thumnailUrlIter->second.isString())
					thumnailUri = thumnailUrlIter->second.get<std::string>();
			}
			else
			{
				ARL::Reflection::ValueTable::const_iterator RetryUrlIter = table->find("RetryUrl");
	...
	if (thumnailUri.is_initialized() && !thumnailUri.get().empty())
	{
		...
		ARL::HttpAsync::get(thumnailUri.get(), options).then(boost::bind(&invokeHeadshotThumbnailLoaded, QPointer<AvatarLoader>(this), playerId, _1));
	}
}

void AvatarLoader::headshotThumbnailLoaded(int playerId, ARL::HttpFuture future)
{
	...
	if (!imageContent.empty())
	{
		...
		if (image.loadFromData((uchar*)imageContent.c_str(), imageContent.size()))
		{
			...
			resultingImage.fill(QtUtilities::toQColor(ARL::BrickColor(m_BackgroundColor).color3()));
