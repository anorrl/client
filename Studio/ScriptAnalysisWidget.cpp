...
public:
	...
	void update(const ARL::ScriptAnalyzer::Warning& warning)
	{
	...
	void launchURL()
	{
        if (!m_bIsError && m_warningCode > 0)
        {
            ...
            QMetaObject::invokeMethod(&ANORRLContextualHelpService::singleton(), "onHelpTopicChanged", Q_ARG(QString, QString("Script_Analysis#%1").arg(getWarningCodeString(m_warningCode))));
        }
	}

	const ARL::ScriptAnalyzer::Location& getLocation() { return m_location; }
	bool isError() const { return m_bIsError; }

    static QString getWarningCodeString(int code) { return QString().sprintf("W%03d", code); }

private:
	ARL::ScriptAnalyzer::Location     m_location;
    int                               m_warningCode;
	bool                              m_bIsError;
};
...
void ScriptAnalysisTreeWidget::setDataModel(boost::shared_ptr<ARL::DataModel> dataModel)
{
	ARLASSERT(m_dataModel == NULL);
	m_dataModel = dataModel;
}

void ScriptAnalysisTreeWidget::updateResults(const LuaSourceBuffer& script, const ARL::ScriptAnalyzer::Result& result, bool showErrors, bool showWarnings)
{
	if (script.empty())
		return;
...
{
	m_dataModel = boost::shared_ptr<ARL::DataModel>();
	m_InstanceMap.clear();
...
void ScriptAnalysisTreeWidget::updateItemName(const QModelIndex& index)
{
	...
	if (pItem)
	{
		...
		if (!script.empty())
		{
			ARL::DataModel::LegacyLock lock(m_dataModel, ARL::DataModelJob::Read);
			pItem->setText(0, QString("<b>%1</b>").arg(script.getFullName().c_str()));
...
void ScriptAnalysisTreeWidget::onItemActivated(QTreeWidgetItem* pItem)
{
	...
	if(pBaseItem)
	{
		...
		if (!script.empty())
		{
			IANORRLDoc* pDocument = ANORRLDocManager::Instance().openDoc(script);
			if (pDocument)
			{
...
void ScriptAnalysisTreeWidget::instanceRemoving(boost::shared_ptr<ARL::Instance> script)
{
...
void ScriptAnalysisTreeWidget::categoryItemBecomingLinked(QTreeWidgetItem* item)
{
	...
	if (FFlag::StudioUpdateSAResultsInUIThread)
	{
		...
		ARL::ScriptAnalyzer::Result result;
		QMetaObject::invokeMethod(qobject_cast<ScriptAnalysisWidget*>(parent()), "updateResultsInternal", Qt::AutoConnection,
			Q_ARG(shared_ptr<ARL::Instance>, script.toInstance()), Q_ARG(ARL::ScriptAnalyzer::Result, result));
	}
	else
	{
...
//--------------------------------------------------------------------------------------------
LinkedSourceInstance::LinkedSourceInstance(boost::shared_ptr<ARL::LuaSourceContainer> instance)
...
void LinkedSourceInstance::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
...
, m_bDisplayCurrentScriptState(true)
{
	...
	qRegisterMetaType<ARL::HttpFuture>("ARL::HttpFuture");
	...
	m_pErrorsToolButton->setChecked(ANORRLSettings().value(kSAShowErrorsKey, true).toBool());
	...
	m_pWarningsToolButton->setChecked(ANORRLSettings().value(kSAShowWarningsKey, true).toBool());
	...
	connect(&ANORRLDocManager::Instance(), SIGNAL(currentDocChanged()),
			this,                          SLOT(onCurrentDocChanged()));

	updateButtonsTextAndState();
}

void ScriptAnalysisWidget::updateResults(const LuaSourceBuffer& script, const ARL::ScriptAnalyzer::Result& result)
{
	ARLASSERT(!FFlag::StudioUpdateSAResultsInUIThread || (QThread::currentThread() == qApp->thread()));
...
void ScriptAnalysisWidget::setDataModel(boost::shared_ptr<ARL::DataModel> dataModel)
{
    m_dataModel = boost::shared_ptr<ARL::DataModel>();
...
void ScriptAnalysisWidget::onInstanceRemoving(boost::shared_ptr<ARL::Instance> instance)
{
	...
	if (!script.empty())
	{
		...
		if (FFlag::StudioUpdateSAResultsInUIThread)
		{
			...
			QMetaObject::invokeMethod(this, "updateResultsInternal", Qt::AutoConnection,
				Q_ARG(shared_ptr<ARL::Instance>, instance), Q_ARG(ARL::ScriptAnalyzer::Result, result));
		}
		else
		{
...
{
	if (ANORRLScriptDoc* rsd = dynamic_cast<ANORRLScriptDoc*>(ANORRLDocManager::Instance().getCurrentDoc()))
	{
...
template <typename T> static const T* findFirstAncestorByType(const ARL::Instance* instance)
{
    while (instance)
	{
...
void ScriptAnalysisWidget::updateResultsInstance(const shared_ptr<ARL::Instance>& instance)
{
    if (!instance)
		return;
	
	if (!instance->isA<ARL::Script>() && !instance->isA<ARL::ModuleScript>())
        return;

	if (findFirstAncestorByType<ARL::CoreGuiService>(instance.get()))
        return;
    ...
    if (instance && (instance->isA<ARL::Script>() || instance->isA<ARL::ModuleScript>()))
    {
        ...
        {
            ARL::ScriptAnalyzer::Result result = ARL::ScriptAnalyzer::analyze(
	...
	if (iter != m_LinkedSources.end())
	{
		if (FFlag::StudioUpdateSAResultsInUIThread)
		{
			QMetaObject::invokeMethod(this, "analyzeAndUpdateResults", Qt::QueuedConnection, 
				                      Q_ARG(shared_ptr<ARL::Instance>, linkedSourceInstance->getInstance()));
		}
		else
		{
...
void ScriptAnalysisWidget::updateResultsGameScriptAssets()
{
	...
	UpdateUIManager::Instance().getViewWidget<ANORRLGameExplorer>(eDW_GAME_EXPLORER).getListOfScripts(&scriptNames);
...
static void invokeAnalyzeScriptAssetSource(ScriptAnalysisWidget* analysisWidget, const QString& scriptAssetName, ARL::HttpFuture future)
{
	QMetaObject::invokeMethod(analysisWidget, "analyzeScriptAssetSource", Qt::QueuedConnection,
		                      Q_ARG(QString, scriptAssetName), Q_ARG(ARL::HttpFuture, future));
}

void ScriptAnalysisWidget::requestAnalyzeScriptAssetSource(const QString& scriptAssetName)
{
	ANORRLGameExplorer& gameExplorer = UpdateUIManager::Instance().getViewWidget<ANORRLGameExplorer>(eDW_GAME_EXPLORER);
	ARL::ContentId scriptContentId = ARL::ContentId::fromGameAssetName(scriptAssetName.toStdString());

	ARL::HttpFuture future = LuaSourceBuffer::getScriptAssetSource(scriptContentId, gameExplorer.getCurrentGameId());
	if (future.is_ready())
	{
...
void ScriptAnalysisWidget::analyzeScriptAssetSource(const QString& scriptAssetName, ARL::HttpFuture future)
{
	ANORRLGameExplorer& gameExplorer = UpdateUIManager::Instance().getViewWidget<ANORRLGameExplorer>(eDW_GAME_EXPLORER);
	...
	shared_ptr<ARL::Instance> scriptInstance = ARL::Creatable<ARL::Instance>::create<ARL::Script>();

	ARL::DataModel::LegacyLock lock(m_dataModel.get(), ARL::DataModelJob::Write);
	ARL::ScriptAnalyzer::Result result = ARL::ScriptAnalyzer::analyze(m_dataModel.get(), scriptInstance, scriptCode);
	
	// update results in main thread
	updateResultsScriptAsset(scriptAssetName, result);
}

void ScriptAnalysisWidget::updateResultsScriptAsset(const QString& scriptAssetName, const ARL::ScriptAnalyzer::Result& scriptResults)
{		
	ARL::ContentId contentId = ARL::ContentId::fromGameAssetName(scriptAssetName.toStdString());
	if (!contentId.isNull())
		updateResults(LuaSourceBuffer::fromContentId(contentId), scriptResults);
}

void ScriptAnalysisWidget::updateResultsInternal(shared_ptr<ARL::Instance> script, const ARL::ScriptAnalyzer::Result& result)
{	
	ARL::DataModel::LegacyLock lock(m_dataModel.get(), ARL::DataModelJob::Write);
	updateResults(LuaSourceBuffer::fromInstance(script), result); 
}

void ScriptAnalysisWidget::analyzeAndUpdateResults(shared_ptr<ARL::Instance> script)
{
	ARL::DataModel::LegacyLock lock(m_dataModel.get(), ARL::DataModelJob::Write);
