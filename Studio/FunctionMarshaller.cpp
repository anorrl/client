...
{
	ARLASSERT(threadID == GetCurrentThreadId());

#ifdef _DEBUG
	{
		boost::recursive_mutex::scoped_lock lock(staticData().windowsCriticalSection);
		ARLASSERT (refCount==0);
		// Nobody is using this window
		ARLASSERT (staticData().windows.find(threadID) == staticData().windows.end());
	}
#endif
}
...
void FunctionMarshaller::handleAppEvent(void *pClosure)
{
	...
	ARL::CEvent *pWaitEvent = closure->waitEvent;
	try
	{
