...
void DebuggerClientManager::setDataModel(shared_ptr<ARL::DataModel> spDataModel)
{
	...
	if (m_spDataModel)
	{
		...
		if (FFlag::LuaDebuggerBreakOnError)
		{
			...
			debuggerManager.setBreakOnErrorMode((ARL::Scripting::BreakOnErrorMode)settings.value(kBreakOnErrorModeSetting, (int)ARL::Scripting::BreakOnErrorMode_Never).toInt());
		}

		ARL::RunService* pRunService = m_spDataModel->create<ARL::RunService>();
		if (pRunService)
...
shared_ptr<ARL::DataModel> DebuggerClientManager::getDataModel()
...
bool DebuggerClientManager::actionState(const QString &actionID, bool &enableState, bool &checkedState)
{
	if (FFlag::LuaDebuggerBreakOnError)
	{
		...
		{
			checkedState = ARL::Scripting::DebuggerManager::singleton().getBreakOnErrorMode() == ARL::Scripting::BreakOnErrorMode_Never;
			enableState = true;
			return true;
		}
		...
		{
			checkedState = ARL::Scripting::DebuggerManager::singleton().getBreakOnErrorMode() == ARL::Scripting::BreakOnErrorMode_AllExceptions;
			enableState = true;
			return true;
		}
		...
		{
			checkedState = ARL::Scripting::DebuggerManager::singleton().getBreakOnErrorMode() == ARL::Scripting::BreakOnErrorMode_UnhandledExceptions;
			enableState = true;
			return true;
		}
	}
...
bool DebuggerClientManager::handleAction(const QString &actionID, bool checkedState)
{
	if (FFlag::LuaDebuggerBreakOnError)
	{
		if (actionID == "neverBreakOnScriptErrorsAction")
		{
			ARL::Scripting::DebuggerManager::singleton().setBreakOnErrorMode(ARL::Scripting::BreakOnErrorMode_Never);
			ANORRLSettings().setValue(kBreakOnErrorModeSetting, (int)ARL::Scripting::BreakOnErrorMode_Never);
			return true;
		}

		if (actionID == "breakOnAllScriptErrorsAction")
		{
			ARL::Scripting::DebuggerManager::singleton().setBreakOnErrorMode(ARL::Scripting::BreakOnErrorMode_AllExceptions);
			ANORRLSettings().setValue(kBreakOnErrorModeSetting, (int)ARL::Scripting::BreakOnErrorMode_AllExceptions);
			return true;
		}

		if (actionID == "breakOnUnhandledScriptErrorsAction")
		{
			ARL::Scripting::DebuggerManager::singleton().setBreakOnErrorMode(ARL::Scripting::BreakOnErrorMode_UnhandledExceptions);
			ANORRLSettings().setValue(kBreakOnErrorModeSetting, (int)ARL::Scripting::BreakOnErrorMode_UnhandledExceptions);
			return true;
		}
	}
...
ARL::Reflection::Variant DebuggerClientManager::getWatchValue(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
	if (m_pActiveDebuggerClient)
...
void DebuggerClientManager::syncBreakpointState(shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint)
{
	if (!spBreakpoint || !spBreakpoint->getParent())
		return;
	
	ARL::Scripting::ScriptDebugger* pDebugger = ARL::Instance::fastDynamicCast<ARL::Scripting::ScriptDebugger>(spBreakpoint->getParent());
	if (pDebugger)
	{
...
DebuggerClient* DebuggerClientManager::getOrCreateDebuggerClient(shared_ptr<ARL::Instance> spScript)
{
...
DebuggerClient* DebuggerClientManager::getDebuggerClient(shared_ptr<ARL::Instance> spScript)
{
...
void DebuggerClientManager::updateScriptDocument()
{
	...
	if (m_pCurrentDebuggerClient)
	{
		ARL::RunService* pRunState = m_spDataModel->find<ARL::RunService>();
		if(pRunState && pRunState->getRunState() == ARL::RS_RUNNING)
			pRunState->pause();
...
{
	bool breakOnErrorSet = ANORRLSettings().value(kBreakOnErrorDialogDisabled, false).toBool();
	if (!breakOnErrorSet)
		{
			// Create Dialog Box
			ARL::Scripting::DebuggerManager::singleton();
			std::string labelMessage = std::string("The ANORRL Lua Debugger caught an error and will now take you to the exception in the script.") + 
			...
			{
				ARL::Scripting::DebuggerManager::singleton().setBreakOnErrorMode(ARL::Scripting::DebuggerManager::singleton().getBreakOnErrorMode());
				ANORRLSettings().setValue(kBreakOnErrorModeSetting, (int)ARL::Scripting::DebuggerManager::singleton().getBreakOnErrorMode());
			}
			else
			{
				ARL::Scripting::DebuggerManager::singleton().setBreakOnErrorMode(ARL::Scripting::BreakOnErrorMode_Never);
			...
			{
				ANORRLSettings().setValue(kBreakOnErrorDialogDisabled, true);
			}
		}
}

void DebuggerClientManager::startDebugging(ARL::RunTransition evt)
{
	try
	{
		if (evt.oldState == ARL::RS_STOPPED)
		{
			ARL::Scripting::DebuggerManager::singleton().setScriptAutoResume(false);
			STUDIO_EMIT_SIGNAL debuggingStarted();
		}
		else if (evt.oldState == ARL::RS_PAUSED)
		{
...
void DebuggerClientManager::pauseDebugging(ARL::RunTransition evt)
{
	try
	{
...
void DebuggerClientManager::stopDebugging(ARL::RunTransition evt)
{
	try
	{
		//reset current debugger client
		m_pCurrentDebuggerClient = NULL;
		ARL::Scripting::DebuggerManager::singleton().reset();
		resetAllClients();
...
void DebuggerClientManager::onStepAction()
{
	...
	try
	{
		ARL::DataModel::LegacyLock lock(m_spDataModel, ARL::DataModelJob::Write);
		...
		{
		case STEP_INTO:
			ARL::Scripting::DebuggerManager::singleton().stepInto();
			break;
		case STEP_OUT:
			ARL::Scripting::DebuggerManager::singleton().stepOut();
			break;
		case STEP_OVER:
			ARL::Scripting::DebuggerManager::singleton().stepOver();
			break;
		}
	...
	{
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Exception: Not known");
	}
}

void DebuggerClientManager::onRunTransition(ARL::RunTransition evt)
{
	try
	{
		switch (evt.newState)
...
void DebuggerClientManager::onDebuggerAdded(boost::shared_ptr<ARL::Instance> spInstance)
{
	if (!m_spDataModel)
		return;

	shared_ptr<ARL::Scripting::ScriptDebugger> spDebugger = ARL::Instance::fastSharedDynamicCast<ARL::Scripting::ScriptDebugger>(spInstance);
	if (spDebugger && spDebugger->getScript())
		getOrCreateDebuggerClient(shared_from(spDebugger->getScript()));
}

void DebuggerClientManager::onDebuggerRemoved(boost::shared_ptr<ARL::Instance> spInstance)
{
	shared_ptr<ARL::Scripting::ScriptDebugger> spDebugger = ARL::Instance::fastSharedDynamicCast<ARL::Scripting::ScriptDebugger>(spInstance);
	if (spDebugger && spDebugger->getScript())
	{
...
void DebuggerClientManager::pauseAllClients(bool forceAll)
{
	...
	STUDIO_EMIT_SIGNAL executionDataCleared();
	ARL::Scripting::DebuggerManager::singleton().pause();
}
...
void DebuggerClientManager::resumeAllClients(bool forceAll)
{
	...
	if (FFlag::StudioRemoveDebuggerResumeLock)
	{
		...
		ARL::Scripting::DebuggerManager::singleton().resume();
	}
	else
	{
		ARL::DataModel::LegacyLock lock(m_spDataModel, ARL::DataModelJob::Write);
...
// DebuggerClient
//--------------------------------------------------------------------------------------------
DebuggerClient::DebuggerClient(boost::shared_ptr<ARL::Instance> script)
: m_spScript(script)
...
, m_currentThreadID(0)
{
	...
	m_spDataModel = ARL::shared_from(ARL::Instance::fastDynamicCast<ARL::DataModel>(m_spScript->getRootAncestor()));

	// addDebugger can throw
	try
	{
		//'addDebugger' returns the current debugger if it is already added
		m_spDebugger = ARL::Scripting::DebuggerManager::singleton().addDebugger(m_spScript.get());
	}
	...
	// add breakpoints
	const ARL::Scripting::ScriptDebugger::Breakpoints& breakpoints = m_spDebugger->getBreakpoints();
	...
	// add watches
	const ARL::Scripting::ScriptDebugger::Watches& watches = m_spDebugger->getWatches();
	...
	if (ANORRLScriptDoc* pDoc = ANORRLDocManager::Instance().findOpenScriptDoc(
		LuaSourceBuffer::fromInstance(script)))
	{
		setDocument(pDoc);
		if (ANORRLDocManager::Instance().getCurrentDoc() == pDoc)
			activate();
	}
}
...
void DebuggerClient::setDocument(IANORRLDoc *pDocument)
{
	...
	if (pDocument)
	{
		...
		if (pTextEdit)
		{
			...
			ARL::Scripting::ScriptDebugger::Breakpoints breakpoints = m_spDebugger->getBreakpoints();
			for (ARL::Scripting::ScriptDebugger::Breakpoints::const_iterator iter = breakpoints.begin(); iter != breakpoints.end(); ++iter)
			{
				ARL::Scripting::DebuggerBreakpoint* pBreakpoint = ARL::Instance::fastDynamicCast<ARL::Scripting::DebuggerBreakpoint>(iter->second);
				if(pBreakpoint)
					m_pQDebuggerClient->updateBreakpointState(pBreakpoint->getLine() - 1, ARL::Scripting::DebuggerBreakpoint::prop_Enabled.getValue(pBreakpoint) ? 
				                                                            ARLTextUserData::ENABLED : ARLTextUserData::DISABLED);
			}
...
boost::shared_ptr<ARL::Instance> DebuggerClient::getScript()
{ return m_spScript; }

boost::shared_ptr<ARL::Instance> DebuggerClient::getScript(int frameNo)
{ 
...
{
	boost::shared_ptr<ARL::Instance> spScript = getScript(getCurrentFrame());
	if (!spScript)
		return false;
	...
	if (openDoc)
		ANORRLDocManager::Instance().openDoc(LuaSourceBuffer::fromInstance(spScript));
...
{
	ARLASSERT(pExtDebuggerClient != this); // just want to see when does this happen?
...
ARL::Reflection::Variant DebuggerClient::getWatchValue(boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch)
{
	...
	{
		ARL::DataModel::LegacyLock lock(m_spDataModel, ARL::DataModelJob::Write);
		ARL::Reflection::Variant var = m_spDebugger->getWatchValue(spWatch.get(), getCurrentFrame());		
		return var;
	}
	return ARL::Reflection::Variant();
}
...
void DebuggerClient::addWatch(const QString& watchKey)
{
	...
	for (ARL::Scripting::ScriptDebugger::Watches::const_iterator iter = watches.begin(); iter != watches.end(); ++iter)
	{
		...
		{
			ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Watch expression '%s' is already available", qPrintable(watchKey));
			return;
		}
	}

	ARL::DataModel::LegacyLock lock(m_spDataModel, ARL::DataModelJob::Write);
	boost::shared_ptr<ARL::Scripting::DebuggerWatch> spWatch = m_spDebugger->addWatch(watchKey.toStdString());
...
void DebuggerClient::setCurrentFrame(int frame)
{
	...
	ANORRLDocManager::Instance().openDoc(scriptToOpen);
}
...
void DebuggerClient::syncBreakpointState(int breakpointLine)
{
	...
	ARLTextUserData::eBreakpointState state(ARLTextUserData::NO_BREAKPOINT);
	ARL::Scripting::DebuggerBreakpoint* pBreakpoint = m_spDebugger->findBreakpoint(breakpointLine);
	if (pBreakpoint)
		state = pBreakpoint->isEnabled() ? ARLTextUserData::ENABLED : ARLTextUserData::DISABLED;
...
void DebuggerClient::resetExecution()
{
	...
	boost::shared_ptr<ARL::Instance> spScript = getScript(m_CurrentFrame);
	if (spScript != getScript())
	{
...
void DebuggerClient::syncScriptWithTextEditor(const BreakpointDetails& textEditBreakpoints)
{
	...
	ARL::Scripting::ScriptDebugger::Breakpoints breakpoints = m_spDebugger->getBreakpoints();
	for (ARL::Scripting::ScriptDebugger::Breakpoints::iterator iter = breakpoints.begin(); iter != breakpoints.end(); ++iter)
		(iter->second)->setParent(NULL);

	//recreate all the breakpoints again!
	boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint;
	for (BreakpointDetails::const_iterator iter = textEditBreakpoints.begin(); iter != textEditBreakpoints.end(); ++iter)
	{
		...
		if (spBreakpoint)
		{					
			ARL::Scripting::DebuggerBreakpoint::prop_Enabled.setValue(spBreakpoint.get(), (*iter).isEnabled);
			//ARL::Scripting::DebuggerBreakpoint::prop_Condition.setValue(spBreakpoint.get(), condition); - TODO: add support for condition
		}
	}
}
...
void DebuggerClient::toggleBreakpoint(int breakpointLine)
{
	...
	ARL::ScopedAssign<bool> ignoreBreakpointAddRemove(m_bIgnoreBreakpointAddRemove, true);
	...
	if (m_pQDebuggerClient)
		m_pQDebuggerClient->updateBreakpointState(breakpointLine - 1, isBreakpointAdded ? ARLTextUserData::ENABLED : ARLTextUserData::NO_BREAKPOINT);
}
...
void DebuggerClient::toggleBreakpointState(int breakpointLine)
{
	...
	{
		ARL::DataModel::LegacyLock lock(m_spDataModel, ARL::DataModelJob::Write);
		ARL::Scripting::DebuggerBreakpoint::prop_Enabled.setValue(pBreakpoint, !pBreakpoint->isEnabled());
	}
}
...
{ 
	...
	const ARL::Scripting::ScriptDebugger::PausedThreads& pausedThreads = getPausedThreads();
	ARL::Scripting::ScriptDebugger::PausedThreads::const_iterator iter = pausedThreads.find(threadID);
...
const ARL::Scripting::ScriptDebugger::PausedThreads& DebuggerClient::getPausedThreads()
...
void DebuggerClient::onScriptErrorDetected(int errorLine, std::string errorMessage, ARL::Scripting::ScriptDebugger::Stack stack)
{
	if (errorLine <= 0)
		return;
...
void DebuggerClient::onBreakpointAdded(shared_ptr<ARL::Instance> spInstance)
{
	if (m_bIgnoreBreakpointAddRemove)
		return;

	shared_ptr<ARL::Scripting::DebuggerBreakpoint> spBreakpoint = ARL::Instance::fastSharedDynamicCast<ARL::Scripting::DebuggerBreakpoint>(spInstance);
	...
	if (m_pQDebuggerClient)
	{
		m_pQDebuggerClient->updateBreakpointState(spBreakpoint->getLine() - 1, 
		                                          ARL::Scripting::DebuggerBreakpoint::prop_Enabled.getValue(spBreakpoint.get()) ? 
					                              ARLTextUserData::ENABLED : ARLTextUserData::DISABLED);
	}
}

void DebuggerClient::onBreakpointRemoved(shared_ptr<ARL::Instance> spInstance)
{
	if (m_bIgnoreBreakpointAddRemove)
		return;
...
void DebuggerClient::onWatchAdded(shared_ptr<ARL::Instance> spInstance)
{
	shared_ptr<ARL::Scripting::DebuggerWatch> spWatch = ARL::Instance::fastSharedDynamicCast<ARL::Scripting::DebuggerWatch>(spInstance);
	if (spWatch)
		STUDIO_EMIT_SIGNAL DebuggerClientManager::Instance().watchAdded(spWatch);
}

void DebuggerClient::onWatchRemoved(shared_ptr<ARL::Instance> spInstance)
{
	shared_ptr<ARL::Scripting::DebuggerWatch> spWatch = ARL::Instance::fastSharedDynamicCast<ARL::Scripting::DebuggerWatch>(spInstance);
	if (spWatch)
...
bool DebuggerClient::getValue(const QString& key, ARL::Reflection::Variant& value, int frame)
{
	bool result = false;
...
bool DebuggerClient::isKeyDefined(boost::shared_ptr<const ARL::Reflection::ValueMap> variables, const QString &key, ARL::Reflection::Variant &value)
{
	if(variables && !variables->empty())
	{
		for(ARL::Reflection::ValueMap::const_iterator iter = variables->begin(); iter != variables->end(); ++iter)
		{
...
void QDebuggerClient::updateBreakpointState(int textEditLine, ARLTextUserData::eBreakpointState state)
...
void QDebuggerClient::onInsertBreakpoint()
{
	...
	if (block.isValid())
	{
		...
		updateTextUserData(block.blockNumber(), ARLTextUserData::ENABLED);
	}
}
...
void QDebuggerClient::onDeleteBreakpoint()
{
	...
	{
		m_pDebuggerClient->toggleBreakpoint(block.blockNumber()+1);
		updateTextUserData(block.blockNumber(), ARLTextUserData::NO_BREAKPOINT);
	}
}
...
void QDebuggerClient::setMarker_MT(int textEditLine, QString marker, bool setBlockCurrent)
{
	...
	//set marker
	ARLTextUserData* pUserData = dynamic_cast<ARLTextUserData*>(block.userData());
	if (pUserData)
...
void QDebuggerClient::updateTextUserData(int textEditLine, ARLTextUserData::eBreakpointState state)
{
	QTextBlock block = m_pTextEdit->document()->findBlockByNumber(textEditLine);
	if(block.isValid())
	{
...
void QDebuggerClient::onUpdateContextualMenu(QMenu* pContextualMenu, QPoint pos)
{
	...
	if (block.isValid())
	{
		...
		if(pUserData)
		{
			ARLTextUserData::eBreakpointState state = pUserData->getBreakpointState();

			ANORRLMainWindow& rbxMainWindow = UpdateUIManager::Instance().getMainWindow();
			rbxMainWindow.insertBreakpointAction->setVisible(state == ARLTextUserData::NO_BREAKPOINT);
			rbxMainWindow.deleteBreakpointAction->setVisible(state != ARLTextUserData::NO_BREAKPOINT);
			rbxMainWindow.toggleBreakpointStateAction->setVisible(state != ARLTextUserData::NO_BREAKPOINT);

			//update action text appropriately
			if (state == ARLTextUserData::ENABLED)
			{
			...
			else if (state == ARLTextUserData::DISABLED)
			{
...
DebuggerClient::BreakpointDetails QDebuggerClient::getBreakpointDetails()
{
	...
	while (block.isValid())
	{
		...
		{
			ARLTextUserData::eBreakpointState state = pUserData->getBreakpointState();
			if (state != ARLTextUserData::NO_BREAKPOINT)
				textEditBreakpoints.push_back(DebuggerClient::BreakpointDetail(block.blockNumber() + 1, state == ARLTextUserData::ENABLED ? true: false));
		}
...
void QDebuggerClient::removeBreakpointState()
{
	...
	while (block.isValid())
	{
		ARLTextUserData* pUserData = dynamic_cast<ARLTextUserData*>(block.userData());
		if(pUserData)
			pUserData->setBreakpointState(ARLTextUserData::NO_BREAKPOINT);
