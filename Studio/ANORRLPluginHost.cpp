...
static const char* kPluginFileName = "Plugin.arlm";
...
ANORRLPluginHost::ANORRLPluginHost(QObject* pParent)
    : QObject(pParent),
...
ANORRLPluginHost::~ANORRLPluginHost()
{
}

void ANORRLPluginHost::setNotifier(ARL::IHostNotifier *notifier)
{ 
    m_pNotifier = notifier; 
}

void* ANORRLPluginHost::createToolbar(const std::string& name)
{
...
void* ANORRLPluginHost::createButton(
    void*       tbId, 
...
void ANORRLPluginHost::buttonIconFailedToLoad(void* buttonId)
{
	QImage image;
...
void ANORRLPluginHost::setButtonIconInternal(int actionId, QImage image)
{
	if (!m_ActionMap.contains(actionId))
    {
        // This can happen if it takes a long time to load an icon
...
void ANORRLPluginHost::hideToolbars(const std::vector<void*>& toolbars, bool hide)
{
    ANORRLMainWindow& main_window = UpdateUIManager::Instance().getMainWindow();
...
void ANORRLPluginHost::disableToolbars(const std::vector<void*>& toolbars, bool disable)
{
    ANORRLMainWindow& main_window = UpdateUIManager::Instance().getMainWindow();
...
void ANORRLPluginHost::onShowToolbar(int toolbarID, bool show)
{
...
void ANORRLPluginHost::onShowToolbars(QVariant toolbarsVariant, bool show)
{
...
void ANORRLPluginHost::onEnableToolbar(int toolbarID, bool enable)
{
    if ( !m_ToolBarMap.contains(toolbarID) && !m_RibbonGroupMap.contains(toolbarID) )
	{
		// only output an error if we're showing a toolbar that is supposed to exist
...
void ANORRLPluginHost::deleteToolbars(const std::vector<void*>& toolbars)
{
    ANORRLMainWindow& mainWindow = UpdateUIManager::Instance().getMainWindow();
...
void ANORRLPluginHost::setSetting(int assetId, const std::string& key,
...
void ANORRLPluginHost::getSetting(int assetId, const std::string& key,
...
bool ANORRLPluginHost::getLoggedInUserId(int* userIdOut)
{
	int userId = ANORRLUser::singleton().getUserId();
	if (userId > 0)
	{
		*userIdOut = userId;
		return true;
...
void ANORRLPluginHost::handlePluginAction()
{
    ...
    if ( ANORRLDocManager::Instance().getCurrentDoc() )
    {
        ...
        ANORRLDocManager::Instance().getCurrentDoc()->handlePluginAction(m_pNotifier,(void*)(actionID));
    }
...
void ANORRLPluginHost::onCreateToolbar(int toolbarID,QString name)
{
    ANORRLMainWindow& mainWindow = UpdateUIManager::Instance().getMainWindow();
...
void ANORRLPluginHost::uiAction(std::string uiActionName)
{
    QMetaObject::invokeMethod(
        this,
        "onUIAction",
...
void ANORRLPluginHost::onUIAction(QString uiActionName)
{
...
void ANORRLPluginHost::onCreateButton(int toolbarID,int actionID,QString text,QString tooltip,QString iconName)
{
...
void ANORRLPluginHost::onSetButtonActive(int buttonID, bool active)
{
...
bool ANORRLPluginHost::event(QEvent* evt)
{
...
void ANORRLPluginHost::loadPlugins(ARL::DataModel* dataModel)
{
    if ( !dataModel )
        return;

	// Old style plugins
    loadPluginsFromPath(dataModel,ANORRLPluginHost::builtInPluginPath());
    loadPluginsFromPath(dataModel,ANORRLPluginHost::userPluginPath());
...
void ANORRLPluginHost::loadPluginsFromPath(ARL::DataModel* dataModel,const QString &srcPath)
{
...
void ANORRLPluginHost::loadPluginsInternal(ARL::DataModel* dataModel,const QString& srcPath)
{
    QDir srcDir(srcPath);    
    ...
    {        
        ...
        if ( fileInfo.isDir() )        
            ...
            QApplication::postEvent(this, new ANORRLCustomEventWithArg(PLUGIN_LOAD,boost::bind(&ANORRLPluginHost::doLoadPlugin,dataModel,fileInfo.absoluteFilePath())));
    }
}

void ANORRLPluginHost::doLoadPlugin(ARL::DataModel* dataModel, QString filename)
{
...
void ANORRLPluginHost::findAndLoadModelPlugins(ARL::DataModel* dataModel, const QString& srcPath)
{
...
void ANORRLPluginHost::loadInstalledModelPlugins(ARL::DataModel* dataModel)
{
...
void ANORRLPluginHost::doLoadModelPlugin(ARL::DataModel* dataModel,
...
QString ANORRLPluginHost::builtInPluginPath()
{
    return ANORRLSettings::getBuiltInPluginsFolder();
}

QString ANORRLPluginHost::userPluginPath()
{
	return AuthoringSettings::singleton().pluginsDir.absolutePath();
}

QString ANORRLPluginHost::modelPluginsPath()
{
	return AuthoringSettings::singleton().modelPluginsDir.absolutePath();
}

void ANORRLPluginHost::processInstallPluginFromWebsite(int assetId, int assetVersion,
...
QString ANORRLPluginHost::getPluginMetadataJson()
{
...
void ANORRLPluginHost::setPluginEnabled(int assetId, bool enabled)
{
...
void ANORRLPluginHost::uninstallPlugin(int assetId)
{
...
QDir ANORRLPluginHost::getSpecificPluginFolder(int assetId)
{
	QString assetIdString = QString("%1").arg(assetId);
...
QString ANORRLPluginHost::getPluginMetadataFilePath()
{
...
bool ANORRLPluginHost::readPluginMetadata(shared_ptr<ARL::Reflection::ValueTable>& values)
{
...
void ANORRLPluginHost::writePluginMetadata(const shared_ptr<ARL::Reflection::ValueTable>& values)
{
...
void ANORRLPluginHost::readPluginSettings(int assetId,
...
void ANORRLPluginHost::writePluginSettings(int assetId,
...
void ANORRLPluginHost::init()
{
	...
	if ( mainWindow.isRibbonStyle() )
	{
		ANORRLQuickAccessConfig::singleton().setPluginHostInitialized(true);
...
void ANORRLPluginHost::removeActionsFromMap(QObject* parent)
{
...
void ANORRLPluginHost::handleTerrainRibbonGroupDeletion()
{
...
QString ANORRLPluginHost::getToolbarName(int id) const
{
    ANORRLMainWindow& mainWindow = UpdateUIManager::Instance().getMainWindow();
...
bool ANORRLPluginHost::actionState(const QString &actionID, bool &enableState, bool &checkedState)
{
...
void ANORRLPluginHost::openScriptDoc(shared_ptr<ARL::Instance> instance, int lineNumber)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) ||
		ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
		QMetaObject::invokeMethod(&ANORRLDocManager::Instance(), "openDoc", Qt::QueuedConnection, Q_ARG(shared_ptr<ARL::Instance>, instance), Q_ARG(int, lineNumber));
}

void ANORRLPluginHost::openWikiPage(const std::string& url)
{
	UpdateUIManager::Instance().setDockVisibility(eDW_CONTEXTUAL_HELP, true);

	QMetaObject::invokeMethod(&UpdateUIManager::Instance().getViewWidget<ANORRLContextualHelp>(eDW_CONTEXTUAL_HELP), "onHelpTopicChanged", Q_ARG(QString, QString::fromStdString(url)));
}

void ANORRLPluginHost::exportPlace(std::string filePath, ARL::ExporterSaveType exportType)
{
...
void ANORRLPluginHost::promptForExistingAssetId(const std::string& assetType, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
...
shared_ptr<ARL::Instance> ANORRLPluginHost::csgUnion(shared_ptr<const ARL::Instances> instances, ARL::DataModel* dataModel)
{
...
shared_ptr<const ARL::Instances> ANORRLPluginHost::csgNegate(shared_ptr<const ARL::Instances> instances, ARL::DataModel* dataModel)
{
    if (!FFlag::EnableCSGScripting)
        throw std::runtime_error("Scripted CSG operations are disabled");
...
shared_ptr<const ARL::Instances> ANORRLPluginHost::csgSeparate(shared_ptr<const ARL::Instances> instances, ARL::DataModel* dataModel)
{
    if (!FFlag::EnableCSGScripting)
        throw std::runtime_error("Scripted CSG operations are disabled");
...
void ANORRLPluginHost::handleDragEnterEvent(ARL::DataModel* dataModel, shared_ptr<const ARL::Instances> instances, ARL::Vector2 location)
{
	if (!m_pNotifier)
		return;
