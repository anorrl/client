// This is here as some additional security measures that only exist inside ACC.
#include <random>
#include <windows.h>
...
namespace
{
    ...
    template<int N> __forceinline void accBigFunc()
    {
        _asm
        {
            ARL_NOP6;
    ...
    {
        uintptr_t funcBase = reinterpret_cast<uintptr_t>(&accBigFuncStorage);
    ...
    LONG CALLBACK accReflectionModifiedVeh(EXCEPTION_POINTERS* rec)
    {
        DWORD unused;
        ...
        uintptr_t accBaseAddress = reinterpret_cast<uintptr_t>(GetModuleHandleA(NULL));
            ...
            && (codeAddr - luaBase >= luaSize)) // code is not in .lua
        {
            ...
            analyticsPoints.addPoint("code", ARL::format("0x%08X", codeAddr-accBaseAddress).c_str());
            analyticsPoints.addPoint("data", ARL::format("0x%08X", dataAddr-accBaseAddress).c_str());
            analyticsPoints.report("AccReflectionAv", FInt::US30484p2, true); // add fflag
            if (!FFlag::US30484p1)
            {
...
namespace ARL
{
    ...
    LONG CALLBACK accBigFuncExceptionHandler(EXCEPTION_POINTERS* rec)
    {
        uintptr_t funcBase = reinterpret_cast<uintptr_t>(&accBigFuncStorage);
    ...
    LONG CALLBACK accHwbpExceptionHandler(EXCEPTION_POINTERS* rec)
    {
    ...
    void initAntiMemDump()
    {
        ...
        AddVectoredExceptionHandler(1, &accBigFuncExceptionHandler);

        // If someone was scanning RAM, seeing repeating patterns would be suspicious and detectable.
        DWORD unused;
        VirtualProtect(&accBigFuncStorage, 2*kPageSize, PAGE_EXECUTE_READWRITE, &unused);
        unsigned char* fakeCode = reinterpret_cast<unsigned char*>(GetModuleHandleA(NULL)) + 0x1000;
        memcpy(&accBigFuncStorage, fakeCode, 2*kPageSize);
        VirtualProtect(&accBigFuncStorage, 2*kPageSize, PAGE_EXECUTE_READ, &unused);
    ...
    void initHwbpVeh()
    {
        ...
        AddVectoredExceptionHandler(1, &accHwbpExceptionHandler);
    }
    ...
    void initLuaReadOnly()
    {
        ...
        {
            DWORD unused;
            luaVehHandle = AddVectoredExceptionHandler(0, &accReflectionModifiedVeh);
