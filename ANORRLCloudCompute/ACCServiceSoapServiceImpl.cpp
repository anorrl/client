...
LOGVARIABLE(ACCServiceInit, 1);
LOGVARIABLE(ACCServiceJobs, 1);
LOGVARIABLE(ACCDataModelInit, 1);
...
FASTINTVARIABLE(ACCServiceThreadCount, ARL::TaskScheduler::Threads1)
...
class ACCServiceSettings : public ARL::FastLogJSON
{
public:
	START_DATA_MAP(ACCServiceSettings);
	...
	DECLARE_DATA_INT(HttpUseCurlPercentageACC);
	END_DATA_MAP();
};
...
public:
	...
	void run()
	{
		...
		shared_ptr<const ARL::Reflection::ValueTable> jsonResult(arl::make_shared<const ARL::Reflection::ValueTable>());
...
class ACCServiceDynamicSettings : public ARL::FastLogJSON
{
...
DATA_MAP_IMPL_START(ACCServiceSettings)
...
IMPL_DATA(HttpUseCurlPercentageACC, 0); // do not use CURL by default
DATA_MAP_IMPL_END()
...
std::string ACCServiceSettingsKeyOverwrite;
#endif
...
class CWebService
{
	...
	ACCServiceSettings accSettings;
	ACCServiceDynamicSettings accDynamicSettings;
...
public:
	struct JobItem : boost::noncopyable
	{
		...
		arl::signals::connection notifyAliveConnection;
...
private:
	...
	void LoadClientSettings(ACCServiceSettings& dest);
...
public:
	...
	{
		while (::WaitForSingleObject(doneEvent.m_h, accSettings.GetValueSecurityDataTimer() * 1000) == WAIT_TIMEOUT) 
		{
	...
	{
		while (::WaitForSingleObject(doneEvent.m_h, accSettings.GetValueClientSettingsTimer() * 1000) == WAIT_TIMEOUT) 
		{
			...
			if (clientChanged || thumbnailChanged)
			{
				...
				accDynamicSettings.ReadFromStream(clientSettingsData.c_str());
                if (isThumbnailer)
                {
				    accDynamicSettings.ReadFromStream(thumbnailSettingsData.c_str());
                }
				...
				if (jobs.size() > 0)
				{
					...
					// current we only have 1 datamodel running per accservice, so submit write task just on first datamodel
					shared_ptr<JobItem> job = jobs.begin()->second;
					job->dataModel->submitTask(boost::bind(&ACCServiceDynamicSettings::UpdateSettings, &accDynamicSettings), ARL::DataModelJob::Write);
				}
				else
					accDynamicSettings.UpdateSettings();
			}
		}
	}
	...
	const ARL::Reflection::ValueArray* diag(int type, shared_ptr<ARL::DataModel> dataModel)
	{
		...
		*/
...

		{
			...
			memCounters->push_back(ARL::Diagnostics::Countable<arl::signals::connection>::getCount());
			memCounters->push_back(ARL::Diagnostics::Countable<arl::signals::connection::islot>::getCount());
	...
	shared_ptr<JobItem> createJob(const ns1__Job& job, bool startHeartbeat, shared_ptr<ARL::DataModel>& dataModel)
	{
		...
		if(players)
		{
			LoadClientSettings(accSettings);
            if (accSettings.GetError())
            {
                ARLCRASH(accSettings.GetErrorString().c_str());
            }

			{
				bool useCurl = rand() % 100 < accSettings.GetValueHttpUseCurlPercentageACC();
				FASTLOG1(FLog::ACCDataModelInit, "Using CURL = %d", useCurl);
				ARL::Http::SetUseCurl(useCurl);

                ARL::Http::SetUseStatistics(true);
			}

			FASTLOGS(FLog::ACCDataModelInit, "Creating Data Model, Windows MD5: %s", accSettings.GetValueWindowsMD5());
			FASTLOGS(FLog::ACCDataModelInit, "Creating Data Model, Mac MD5: %s", accSettings.GetValueMacMD5());
			FASTLOGS(FLog::ACCDataModelInit, "Creating Data Model, Windows Player Beta MD5: %s", accSettings.GetValueWindowsPlayerBetaMD5());
			players->setGoldenHashes(accSettings.GetValueWindowsMD5(), accSettings.GetValueMacMD5(), accSettings.GetValueWindowsPlayerBetaMD5());
	...
	void batchJob(const ns1__Job& job, ns1__ScriptExecution* script, std::vector<ns1__LuaValue*>* result, soap* soap)
	{
		...
		try
		{
			...
			FASTLOGS(FLog::ACCServiceJobs, "Opened Batch JobItem %s", id.c_str());
			FASTLOG1(FLog::ACCServiceJobs, "DataModel: %p", dataModel.get());
	...
	void openJob(const ns1__Job& job, ns1__ScriptExecution* script, std::vector<ns1__LuaValue*>* result, soap* soap, bool startHeartbeat)
	{
		...
		try
		{
			...
			try
			{
				...
				FASTLOGS(FLog::ACCServiceJobs, "Opened JobItem %s", id.c_str());
				FASTLOG1(FLog::ACCServiceJobs, "DataModel: %p", pDataModel);
		...
		catch (std::exception& e)
		{
			...
			FASTLOGS(FLog::ACCServiceJobs, "Closing DataModel due to exception: %s", szMsg);
			FASTLOG1(FLog::ACCServiceJobs, "DataModel: %p", dataModel.get());
	...
	void closeJob(const std::string& jobID, const char* errorMessage = NULL)
	{
		...
		// soon as closeJob returns it is safe to force kill the acc process, so make
		...
		if (dataModel)
		{
			FASTLOGS(FLog::ACCServiceJobs, "Closing JobItem %s", jobID.c_str());
			FASTLOG1(FLog::ACCServiceJobs, "DataModel: %p", dataModel.get());
	...
	{
		FASTLOG(FLog::ACCServiceJobs, "Closing all jobs command");
   ...
   dataModelCount(0)
{
	...
	ARL::TaskScheduler::singleton().setThreadCount(ARL::TaskScheduler::ThreadPoolConfig(FInt::ACCServiceThreadCount));
...
{
#ifdef ARL_TEST_BUILD
	if (ACCServiceSettingsKeyOverwrite.length() > 0)
		return ACCServiceSettingsKeyOverwrite;
#endif
	...
	if (settingsKey.length() == 0)
	{
		...
		if (SUCCEEDED(key.Open(HKEY_LOCAL_MACHINE, "Software\\kuro.remover\\ANORRL\\", KEY_READ))) 
		{
			...
			if (SUCCEEDED(key.QueryStringValue("SettingsKey", keyData, &bufLen))) 
			{
				...
				FASTLOGS(FLog::ACCServiceInit, "Read settings key: %s", settingsKey);
			}
		}
...
void CWebService::LoadClientSettings(ACCServiceSettings& dest)
{
...
void CWebService::LoadAppSettings()
{
	...
	FASTLOGS(FLog::ACCServiceInit, "Loading AppSettings.xml, current path: %s", exePath.string());
	...
	if(baseURLNode)
	{
		...
		FASTLOGS(FLog::ACCServiceInit, "Got Base url: %s", baseURL);
		SetBaseURL(baseURL);
	}
...
int ACCServiceSoapService::HelloWorld(_ns1__HelloWorld *ns1__HelloWorld, _ns1__HelloWorldResponse *ns1__HelloWorldResponse)
{
...
int ACCServiceSoapService::Diag(_ns1__Diag *ns1__Diag, _ns1__DiagResponse *ns1__DiagResponse)
{
...
int ACCServiceSoapService::GetStatus(_ns1__GetStatus *ns1__GetStatus, _ns1__GetStatusResponse *ns1__GetStatusResponse)
{
...
int ACCServiceSoapService::OpenJobEx(_ns1__OpenJobEx *ns1__OpenJobEx, _ns1__OpenJobExResponse *ns1__OpenJobExResponse)
{
...
int ACCServiceSoapService::Execute(_ns1__Execute *ns1__Execute, _ns1__ExecuteResponse *ns1__ExecuteResponse)
{
...
int ACCServiceSoapService::ExecuteEx(_ns1__ExecuteEx *ns1__ExecuteEx, _ns1__ExecuteExResponse *ns1__ExecuteExResponse)
{
...
int ACCServiceSoapService::RenewLease(_ns1__RenewLease *ns1__RenewLease, _ns1__RenewLeaseResponse *ns1__RenewLeaseResponse)
{
...
int ACCServiceSoapService::BatchJob(_ns1__BatchJob *ns1__BatchJob, _ns1__BatchJobResponse *ns1__BatchJobResponse)
{
...
int ACCServiceSoapService::BatchJobEx(_ns1__BatchJobEx *ns1__BatchJobEx, _ns1__BatchJobExResponse *ns1__BatchJobExResponse)
{
...
int ACCServiceSoapService::GetExpiration(_ns1__GetExpiration *ns1__GetExpiration, _ns1__GetExpirationResponse *ns1__GetExpirationResponse)
{
...
int ACCServiceSoapService::GetAllJobs(_ns1__GetAllJobs *ns1__GetAllJobs, _ns1__GetAllJobsResponse *ns1__GetAllJobsResponse)
{
...
int ACCServiceSoapService::GetAllJobsEx(_ns1__GetAllJobsEx *ns1__GetAllJobsEx, _ns1__GetAllJobsExResponse *ns1__GetAllJobsExResponse)
{
...
int ACCServiceSoapService::CloseExpiredJobs(_ns1__CloseExpiredJobs *ns1__CloseExpiredJobs, _ns1__CloseExpiredJobsResponse *ns1__CloseExpiredJobsResponse)
{
...
int ACCServiceSoapService::CloseAllJobs(_ns1__CloseAllJobs *ns1__CloseAllJobs, _ns1__CloseAllJobsResponse *ns1__CloseAllJobsResponse)
{
