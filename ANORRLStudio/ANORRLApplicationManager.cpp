...
{
	sharedMemory.setKey("QT_ANORRL_STUDIO");
}

ANORRLApplicationManager& ANORRLApplicationManager::instance()
{
	static ANORRLApplicationManager instance;
	return instance;
}

bool ANORRLApplicationManager::createNewStudioInstance(
	...
	NewInstanceMode mode)
{
	...
	ANORRLCookieJar* cookieJar = dynamic_cast<ANORRLCookieJar*>(ANORRLNetworkAccessManager::Instance().cookieJar());
	if (cookieJar)
	...
	QString anorrlApp = QCoreApplication::applicationFilePath();

	if (mode == NewInstanceMode_Detached)
		return QProcess::startDetached(anorrlApp, args);

	if (!childProcessHandler)
		childProcessHandler = new ChildProcessHandler(&UpdateUIManager::Instance().getMainWindow());
	return childProcessHandler->startChildProcess(anorrlApp, args, mode);
}

int ANORRLApplicationManager::getApplicationCount()
{
	SharedData data;
...
void ANORRLApplicationManager::registerApplication()
{
	sharedMemory.attach();
...
void ANORRLApplicationManager::unregisterApplication()
{
	modifyApplicationCount(-1); // Decrement our count
	sharedMemory.detach();
}

void ANORRLApplicationManager::modifyApplicationCount(int delta)
{
	if (!sharedMemory.lock())
        return;
...
void ANORRLApplicationManager::composeArgList(
...
bool ANORRLApplicationManager::hasChildProcesses()
{
	if (childProcessHandler)
...
void ANORRLApplicationManager::cleanupChildProcesses(int timeout)
{
	if (childProcessHandler)
		childProcessHandler->cleanupChildProcesses(timeout);
}

void ANORRLApplicationManager::startLocalServer()
{
	if (!childProcessHandler)
		childProcessHandler = new ChildProcessHandler(&UpdateUIManager::Instance().getMainWindow());
...
bool ANORRLApplicationManager::hasLocalServer()
{
	if (childProcessHandler)
...
bool ChildProcessHandler::startChildProcess(const QString& anorrlApp, const QStringList &args, NewInstanceMode mode)
{
	QProcess *newProcess = new QProcess();
	newProcess->start(anorrlApp, args);
