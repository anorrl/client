...
void InsertObjectListWidget::onItemInsertRequested(QListWidgetItem* pListWidgetItem)
{
	...
	try
	{
		shared_ptr<ARL::Instance> pObjectToInsert = pInsertListWidgetItem->getInstance()->clone(ARL::EngineCreator);
		if (!pObjectToInsert)
			throw std::runtime_error(ARL::format("InsertObject: Cannot find the object selected."));

		ANORRLIDEDoc* pCurrentDoc = ANORRLDocManager::Instance().getPlayDoc();
...
void InsertObjectListWidget::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
