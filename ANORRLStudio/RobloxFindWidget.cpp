...
void RobloxFindWidget::findAll(const QString& findStr, bool allScripts, eFindFlags flags)
{
	...
	if (RobloxIDEDoc* ideDoc = RobloxDocManager::Instance().getPlayDoc())
		...
		{
			dm->submitTask(boost::bind(&RobloxFindWidget::populateWidget, this, shared_from(dm), findStr, flags, allScripts), ARL::DataModelJob::Read);
			return;
		}
...
void RobloxFindWidget::populateWidget(shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags, bool allScripts)
{
...
void RobloxFindWidget::getNextScript(int index, shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags)
{
	if (m_stopPopulating)
	{
		stopPopulatingItems();
	...
	try
	{
		...
		dm->submitTask(boost::bind(&RobloxFindWidget::searchScriptChain, this, index, sourceText, dm, findStr, flags, hierarchy), ARL::DataModelJob::None);
	}
	catch (const ARL::base_exception&)
	{
		if (index + 1 < m_scriptList.count())
			dm->submitTask(boost::bind(&RobloxFindWidget::getNextScript, this, index + 1, dm, findStr, flags), ARL::DataModelJob::Read);
		else
			stopPopulatingItems();
	}
}

void RobloxFindWidget::searchScriptChain(int index, const QString& sourceText, shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags, const QString& heirarchy)
{
	if (m_stopPopulating)
	{
		stopPopulatingItems();
	...
	if (index + 1 < m_scriptList.count() && !m_stopPopulating)
		dm->submitTask(boost::bind(&RobloxFindWidget::getNextScript, this, index + 1, dm, findStr, flags), ARL::DataModelJob::Read); //Need read lock
	else
		stopPopulatingItems();
}
...
QString RobloxFindWidget::getInstanceHierarchy(shared_ptr<ARL::Instance> instance)
{
	if (!instance)
		return "";
