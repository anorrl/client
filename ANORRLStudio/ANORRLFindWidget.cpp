...
ANORRLFindWidget::ANORRLFindWidget(QWidget *pParent)
: QWidget(pParent)
, m_pListView(NULL)
...
ANORRLFindWidget::~ANORRLFindWidget()
{

}

ANORRLFindWidget& ANORRLFindWidget::singleton()
{
	static ANORRLFindWidget* findWidget = new ANORRLFindWidget();
	return *findWidget;
}

void ANORRLFindWidget::initWidget()
{
...
void ANORRLFindWidget::findAll(const QString& findStr, bool allScripts, eFindFlags flags)
{
	...
	if (ANORRLIDEDoc* ideDoc = ANORRLDocManager::Instance().getPlayDoc())
		if (ARL::DataModel* dm = ideDoc->getDataModel())
		{
			dm->submitTask(boost::bind(&ANORRLFindWidget::populateWidget, this, shared_from(dm), findStr, flags, allScripts), ARL::DataModelJob::Read);
			return;
		}
...
void ANORRLFindWidget::dataModelClosing()
{
	stopPopulatingItems();
	resetItems();
}

bool ANORRLFindWidget::event(QEvent* e)
{
...
static void appendIfScript(shared_ptr<ARL::Instance> instance, QList<LuaSourceBuffer>& scriptList, ARL::CoreGuiService* coreGuiService)
{
	shared_ptr<ARL::Script> script = ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance);
	shared_ptr<ARL::ModuleScript> moduleScript = ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance);
	shared_ptr<ARL::LuaSourceContainer> lsc = ARL::Instance::fastSharedDynamicCast<ARL::LuaSourceContainer>(instance);
...
void ANORRLFindWidget::populateScriptList(bool allScripts)
{
	m_scriptList.clear();

	if (allScripts)
	{
		ANORRLGameExplorer& rge =
		...
		for (std::vector<QString>::const_iterator itr = scriptNames.begin(); itr != scriptNames.end(); ++itr)
		{
			m_scriptList.append(LuaSourceBuffer::fromContentId(
				ARL::ContentId::fromGameAssetName(itr->toStdString())));
		}

		if (ANORRLIDEDoc* ideDoc = ANORRLDocManager::Instance().getPlayDoc())
			if (ARL::DataModel* dm = ideDoc->getDataModel())
				dm->visitDescendants(boost::bind<void>(&appendIfScript, _1, boost::ref(m_scriptList), dm->find<ARL::CoreGuiService>()));
	}
	else
	{
		...
		ANORRLScriptDoc* scriptDoc = dynamic_cast<ANORRLScriptDoc *>(doc);
		if (!scriptDoc)
		{
			ARL::StandardOut::singleton()->print(ARL::MESSAGE_WARNING, "No script currently available.");
			return;
		}
...
void ANORRLFindWidget::addItem(FindData item)
{
	m_pFindListModel->addItem(item);
}

void ANORRLFindWidget::populateWidget(shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags, bool allScripts)
{
...
void ANORRLFindWidget::getNextScript(int index, shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags)
{
	if (m_stopPopulating)
	{
		stopPopulatingItems();
	...
	try
	{
		...
		dm->submitTask(boost::bind(&ANORRLFindWidget::searchScriptChain, this, index, sourceText, dm, findStr, flags, hierarchy), ARL::DataModelJob::None);
	}
	catch (const ARL::base_exception&)
	{
		if (index + 1 < m_scriptList.count())
			dm->submitTask(boost::bind(&ANORRLFindWidget::getNextScript, this, index + 1, dm, findStr, flags), ARL::DataModelJob::Read);
		else
			stopPopulatingItems();
	}
}

void ANORRLFindWidget::searchScriptChain(int index, const QString& sourceText, shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags, const QString& heirarchy)
{
	if (m_stopPopulating)
	{
		stopPopulatingItems();
	...
	if (index + 1 < m_scriptList.count() && !m_stopPopulating)
		dm->submitTask(boost::bind(&ANORRLFindWidget::getNextScript, this, index + 1, dm, findStr, flags), ARL::DataModelJob::Read); //Need read lock
	else
		stopPopulatingItems();
}

QTextCursor ANORRLFindWidget::findNextInScript(const QTextDocument& doc, const QString& findStr, const QTextCursor& cursor, QTextDocument::FindFlags flags, bool regExp)
{
	if (regExp)
...
void ANORRLFindWidget::previousItem()
{
...
void ANORRLFindWidget::nextItem()
{
	if (m_pFindListModel->rowCount() <= 2)
		return;
...
void ANORRLFindWidget::clearItems()
{
...
void ANORRLFindWidget::resetItems()
{
	m_stopPopulating = false;
...
void ANORRLFindWidget::stopPopulatingItems()
{
	if (!m_stopPopulating)
	{
...
void ANORRLFindWidget::openCurrentItem()
{
	openItem(m_pListView->currentIndex());
}

void ANORRLFindWidget::openItem(const QModelIndex & index)
{
	m_pFindListModel->openScript(index.row());
	setFocus(Qt::OtherFocusReason);
}

QString ANORRLFindWidget::getInstanceHierarchy(shared_ptr<ARL::Instance> instance)
{
	if (!instance)
		return "";
	...
	const ARL::Instance* pParent = instance->getParent();
	const ARL::Instance* pChild = instance.get();
...
int FindListModel::openScript(int row)
{
	...
	if (shared_ptr<ARL::LuaSourceContainer> lsc = buffer.toInstance())
	{
	...
	ANORRLScriptDoc* scriptDoc = ANORRLDocManager::Instance().openDoc(buffer);
	if (!scriptDoc)
		return -1;

	ARL::BaldPtr<ScriptTextEditor> editor = scriptDoc->getTextEditor();
	if (!editor)
		return -1;
