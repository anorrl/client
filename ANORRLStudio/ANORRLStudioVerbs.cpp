...
void CopyVerb::doIt(ARL::IDataState*)
{
	...
	pMimeDataForClipboard->setData(sANORRLMimeType,QByteArray(contents.data(), contents.length()));
...
bool PasteVerb::isPasteInfoAvailable() const
{
	...
	if (pClipboard)
	{
		...
		if (pMimeData && pMimeData->hasFormat(sANORRLMimeType))
			return true;
	}
	return false;
}
...
void PasteVerb::createInstancesFromClipboard(shared_ptr<ARL::Instances> itemsToPaste)
{
	...
	QByteArray anorrlData = pMimeData->data(sANORRLMimeType);
	std::istringstream stream(std::string(anorrlData.data(), anorrlData.data() + anorrlData.size()));
...
void InsertModelVerb::insertModel()
{
	...
	QString dlgTitle = m_bInsertInto ? QObject::tr("Open file to Insert") : QObject::tr("Open ANORRL Model");
	QString fileExtn = m_bInsertInto ? QObject::tr("ANORRL Model Files (*.arlm *.arlmx);;Scripts (*.arls *.lua *.txt)") : QObject::tr("ANORRL Model Files (*.arlm *.arlmx)");
...
PublishToANORRLAsVerb::PublishToANORRLAsVerb(ARL::VerbContainer* pVerbContainer, ANORRLMainWindow* mainWnd)
: Verb(pVerbContainer, "PublishToANORRLAsVerb")
, m_pDataModel(dynamic_cast<ARL::DataModel*>(pVerbContainer))
...
PublishToANORRLAsVerb::~PublishToANORRLAsVerb()
{
	if (m_dlg)
        delete m_dlg;
}

bool PublishToANORRLAsVerb::isEnabled() const
{
	return true;
}

void PublishToANORRLAsVerb::initDialog()
{
	static QMutex mutex;
	...
	try
	{
		QString initialUrl = QString("%1/IDE/publishas").arg(ANORRLSettings::getBaseURL());
...
void PublishToANORRLAsVerb::doIt(ARL::IDataState*)
{
	// autosave before we publish just in case
    ANORRLDocManager::Instance().getPlayDoc()->autoSave(true);

	ANORRLMainWindow::sendCounterEvent("QTStudio_IntendPublish");
	...
	{
		QString initialUrl = QString("%1/IDE/Upload.aspx").arg(ANORRLSettings::getBaseURL());
...
QDialog* PublishToANORRLAsVerb::getPublishDialog()
{ 
	return qobject_cast<QDialog*>(m_dlg); 
}

///////////////////////////////////////////////////////////////////////////////////////////////////////
// Bind fn for Publish Selection to ANORRL
...
PublishSelectionToANORRLVerb::PublishSelectionToANORRLVerb(ARL::VerbContainer* pVerbContainer, ANORRLMainWindow* mainWnd)
: Verb(pVerbContainer, "PublishSelectionToANORRLVerb")
, m_pDataModel(dynamic_cast<ARL::DataModel*>(pVerbContainer))
, m_pMainWindow(mainWnd)
, m_dlg(NULL)
{
...
PublishSelectionToANORRLVerb::~PublishSelectionToANORRLVerb()
{
    if (m_dlg)
		delete m_dlg;
}

bool PublishSelectionToANORRLVerb::isEnabled() const
{
	ARL::Selection* pSelection = ARL::ServiceProvider::create<ARL::Selection>(m_pDataModel);
	return (pSelection && (pSelection->size() > 0));
}

void PublishSelectionToANORRLVerb::doIt(ARL::IDataState*)
{
	// autosave before we publish just in case
    ANORRLDocManager::Instance().getPlayDoc()->autoSave(true);

	ANORRLMainWindow::sendCounterEvent("QTStudio_IntendPublishSelection");
	...
	else if (isAnimation)
	{
		...
		initialUrl = QString("%1/UI/Save.aspx?type=Model").arg(ANORRLSettings::getBaseURL());
...
bool CreateNewLinkedSourceVerb::isEnabled() const
{
	...
	return (UpdateUIManager::Instance().getViewWidget<ANORRLGameExplorer>(eDW_GAME_EXPLORER)
		.getCurrentGameId() > 0) && lsc && lsc->getScriptId().isNull() &&
		(ANORRLUser::singleton().getUserId() > 0);
}
	...
	QString newName, bool* success)
{
	*success = false;
	try
	{
		...
		createScriptAssetResponse.setFromJsonFuture(ANORRLGameExplorer::publishScriptAsset(source,
		...
		postUrl = QString(FFlag::GameExplorerUseV2AliasEndpoint ?
			...
			.arg(QString::fromStdString(ContentProvider::getApiBaseUrl(ANORRLSettings::getBaseURL().toStdString())))
			.arg(currentGameId);
...
PublishAsPluginVerb::PublishAsPluginVerb(ARL::VerbContainer* pVerbContainer, ANORRLMainWindow* mainWnd)
...
{
	// autosave before we publish just in case
    ANORRLDocManager::Instance().getPlayDoc()->autoSave(false /*do not force autosave*/);
	...
	QString initialUrl = QString("%1/studio/plugins/publish").arg(ANORRLSettings::getBaseURL());
...
{	ANORRLSettings().setValue(sRibbonStartServerSetting, NameValueStoreManager::singleton().getValue("startServerCB", "checked").toBool()); }

bool ServerPlayersStateInitVerb::isChecked() const
{  return ANORRLSettings().value(sRibbonStartServerSetting, true).toBool(); }
...
{
	QDesktopServices::openUrl(QUrl("https://wiki.anorrl.com/index.php/How_To_Make_Plugins"));
}
...
void PlaySoloVerb::doIt(ARL::IDataState*)
{
	...
	QString fileSaveLocation = QString("%1/visit.arl").arg(ANORRLSettings::getTempLocation());
	...
	// loadfile('https://www.anorrl.com/game/visit.slua')()
	QString script;
	...
	ANORRLApplicationManager::instance().createNewStudioInstance(script, fileSaveLocation, true, true);
}
...
{
	// These should instead come from the Settings infra, once it is ready to go to the temp location
	QString fileSaveLocation = QString("%1/server.arl").arg(ANORRLSettings::getTempLocation());
	QString errorMessage;
	...
	// loadfile('https://www.anorrl.com/game/gameserver.slua')(<placeid>, 53640)

	//local placeId, port, sleeptime, timeout, url, universeId, injectScriptAssetID, libraryRegistrationScriptAssetID = ...

	QString script;
	script = QString(
		"loadfile(\"%1/game/gameserver.slua\")(%2, 53640, nil, nil, \"%1\", %3)\n")
		.arg(::trim_trailing_slashes(ANORRLSettings::getBaseURL().toStdString()).c_str())
		.arg(m_pDataModel->getPlaceID())
		.arg(m_pDataModel->getUniverseId());

	if (ANORRLIDEDoc::isEditMode(m_pDataModel) || ANORRLIDEDoc::getIsCloudEditSession())
		ANORRLApplicationManager::instance().createNewStudioInstance(script, fileSaveLocation, true, false, NewInstanceMode_Server);
}
...
{
    // loadfile('https://www.anorrl.com/game/join.ashx?UserID=0&serverPort=53640')()
	QString script;
	script = QString("loadfile(\"%1/game/join.ashx?UserID=0&serverPort=53640&universeId=%2\")()\n")
		.arg(ANORRLSettings::getBaseURL())
		.arg(m_pDataModel->getUniverseId());
	
	if (ANORRLIDEDoc::getIsCloudEditSession() || (!ARL::Network::Players::clientIsPresent(m_pDataModel) && !ARL::Network::Players::findConstLocalPlayer(m_pDataModel)))
		ANORRLApplicationManager::instance().createNewStudioInstance(script,QString::null,true,true,NewInstanceMode_Player);
}
...
ShutdownClientVerb::ShutdownClientVerb(ARL::VerbContainer* container, IANORRLDoc *pDoc)
...
{
	ANORRLMainWindow *pMainWindow = &UpdateUIManager::Instance().getMainWindow();
	if ( pMainWindow )
	{
		QMetaObject::invokeMethod(
            pMainWindow,
...
ShutdownClientAndSaveVerb::ShutdownClientAndSaveVerb(ARL::VerbContainer* container, IANORRLDoc *pDoc)
...
LeaveGameVerb::LeaveGameVerb(ARL::VerbContainer* container, IANORRLDoc *pDoc)
: Verb(container, "Exit")
, m_pIDEDoc(pDoc)
{
}
...
bool Toggle3DGridVerb::isEnabled() const
{
	...
	return ANORRLIDEDoc::getIsCloudEditSession() || (pLocalPlayer == NULL);
}
...
void ScreenshotVerb::DoPostImage(shared_ptr<ARL::DataModel> spDataModel, const QString &fileName, const QString &seoStr)
{
	...
	try
	{
		QString url = QString("%1/UploadMedia/DoPostImage.ashx?from=client").arg(ANORRLSettings::getBaseURL());
...
{
	QTimer::singleShot(0, ANORRLDocManager::Instance().getPlayDoc(), SLOT(exportSelection()));
}
...
{
	QTimer::singleShot(0, ANORRLDocManager::Instance().getPlayDoc(), SLOT(exportPlace()));
}


PublishToANORRLVerb::PublishToANORRLVerb(ARL::VerbContainer* pVerbContainer, ANORRLMainWindow* pMainWindow)
: Verb(pVerbContainer, "PublishToANORRLVerb")
, m_pDataModel(dynamic_cast<ARL::DataModel*>(pVerbContainer))
, m_pMainWindow(pMainWindow)
, m_bIsPublishInProcess(false)
{
	// Hook up our signal from the datamodel
	m_pDataModel->saveFinishedSignal.connect(boost::bind(&PublishToANORRLVerb::onEventPublishingFinished, this));
}

void PublishToANORRLVerb::onEventPublishingFinished()
{
	m_bIsPublishInProcess = false;
}

void PublishToANORRLVerb::doIt(ARL::IDataState* dataState )
{
	...
	if(!uploadUrl.empty())
	{
		...
		else 
		{
			...
			ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Successfully published - %s", qPrintable(ANORRLDocManager::Instance().getPlayDoc()->displayName()));
		}
	}
	else
...
bool PublishToANORRLVerb::isEnabled() const
{
	return ANORRLIDEDoc::getIsCloudEditSession() ||
		(!m_bIsPublishInProcess && m_pDataModel && ARL::ServiceProvider::find<ARL::Visit>(m_pDataModel) && ARL::DataModel::canSave(m_pDataModel));
}

void PublishToANORRLVerb::save(ARL::ContentId contentID,bool* outError,QString* outErrorTitle,QString* outErrorText)
{
    ARLASSERT(outError);
    ...
    *outErrorText = "Failed to publish place!";
    ...
    try
    {
		...
		ANORRLMainWindow::get(ANORRLDocManager::Instance().getPlayDoc())->getStudioAnalytics()->reportPublishStats(m_pDataModel);
...
{
	return ANORRLDocManager::Instance().getPlayDoc() != NULL;
}
...
void OpenToolBoxWithOptionsVerb::doIt(ARL::IDataState* dataState)
{
	...
	if (!setUrl.isEmpty())
	{
		...
		if (NameValueStoreManager::singleton().getValue("openToolBoxWithOptions", "requiresauthentication").toBool() && !ANORRLUser::singleton().getUserId())
		{
		...
		ANORRLToolBox& anorrlToolBox = UpdateUIManager::Instance().getViewWidget<ANORRLToolBox>(eDW_TOOLBOX);
		anorrlToolBox.loadUrl(QString("%1/%2").arg(ANORRLSettings::getBaseURL()).arg(setUrl));
...
void LaunchHelpForSelectionVerb::doIt(ARL::IDataState* dataState)
{
	...
	if (selection && selection->size() == 1)
	{
		...
		if (!className.isEmpty())
		{
			...
			// update context help
			QMetaObject::invokeMethod(&ANORRLContextualHelpService::singleton(), "onHelpTopicChanged", Q_ARG(QString, className));
		}
	}
}
