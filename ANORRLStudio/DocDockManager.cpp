...
DocDockManager::DocDockManager(ANORRLMainWindow& mainWindow)
    : QObject(&mainWindow),
 ...
 */
void DocDockManager::addDoc(IANORRLDoc& doc)
{
    ARLASSERT(!m_Docs.contains(&doc));
    ARLASSERT(doc.getViewer());

    ARL::BaldPtr<DocDockWidget> dockWidget = new DocDockWidget(m_MainWindow,*this,doc);
 ...
 */
bool DocDockManager::removeDoc(IANORRLDoc& doc)
{
    if ( !m_Docs.contains(&doc) )
        return false;

    ARL::BaldPtr<DocDockWidget> dockWidget = m_Docs[&doc];
    dockWidget->cleanup();
 ...
 * @return false if the doc is not handled by this manager.
 */
bool DocDockManager::renameDoc(IANORRLDoc& doc,const QString& text,const QString& tooltip)
{
    if ( !m_Docs.contains(&doc) )
        return false;
 ...
 * @return false if the doc is not handled by this manager.
 */
bool DocDockManager::setCurrentDoc(IANORRLDoc& doc)
{
    if ( !m_Docs.contains(&doc) )
        return false;
 ...
 */
IANORRLDoc* DocDockManager::getCurrentDoc() const
{
    ARLASSERT(QThread::currentThread() == QCoreApplication::instance()->thread());
 ...
 */
void DocDockManager::startDrag(IANORRLDoc& doc)
{
    ARLASSERT(m_Docs.contains(&doc));
    ARL::BaldPtr<DocDockWidget> dockWidget = m_Docs[&doc];
