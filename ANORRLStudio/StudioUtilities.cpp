...
namespace StudioUtilities
{
    ...
    void convertPhysicalPropertiesIfNeeded(std::vector<boost::shared_ptr<ARL::Instance> > instances, ARL::Workspace* workspace)
	{
		...
		else 
		{
			for (size_t i = 0; i < instances.size(); i++)
			{
				...
				{
					if (ANORRLIDEDoc::displayAskConvertPlaceToNewMaterialsIfInsertNewModel())
					{
	...
	bool checkNetworkAndUserAuthentication(bool openStartPage)
	{
		if (!StudioUtilities::isConnectedToNetwork())
		{
			...
			UpdateUIManager::Instance().showErrorMessage(QObject::tr("Publish to ANORRL"), QObject::tr("Feature not available in offline mode.\nPlease check internet connection and restart ANORRL."));
			return false;
		}
