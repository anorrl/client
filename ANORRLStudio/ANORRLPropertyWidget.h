...
public:
	...
	boost::shared_ptr<ARL::DataModel> getDataModel() { return m_pDataModel; }

	void addInstance(ARL::Instance* pInstance);
	void removeInstance(ARL::Instance* pInstance);
...
protected:
	virtual void initProperty(const ARL::Reflection::ClassDescriptor* pClassDescriptor, 
...
private:
	void visitClassProperties(const ARL::Reflection::ClassDescriptor* pClassDescriptor);
	CategoryItem* getCategoryItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor);
	...
	void onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor);

	void createUpdateJob();
	void removeUpdateJob();

	boost::unordered_map<shared_ptr<ARL::Instance>, ARL::Reflection::Variant>	m_originalValues;
    ...
    std::vector<const ARL::Reflection::PropertyDescriptor*> m_items;
	...
	friend class ANORRLPropertyWidget;
};

class ANORRLPropertyWidget: public QWidget
{
	Q_OBJECT
public:
	ANORRLPropertyWidget(QWidget *pParent);
	virtual ~ANORRLPropertyWidget();
...
private:
	...
	void addInstance(boost::shared_ptr<ARL::Instance> pInstance);
	void removeInstance(boost::shared_ptr<ARL::Instance> pInstance);
	...
	//ARL::readwrite_concurrency_catcher       m_selectedInstancesGuard;	
