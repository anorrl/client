...
CookieMap ANORRLCookieJar::m_cookieMap;
CookieReadStatusMap ANORRLCookieJar::m_cookieReadMap;
...
ANORRLCookieJar::ANORRLCookieJar()
{
...
void ANORRLCookieJar::saveCookiesToDisk()
{
	// Do Not mix the ANORRLStudio Settings with Cookies, create a separate plist file for StudioBrowser
	QSettings settings("ANORRL", "ANORRLStudioBrowser");
...
bool ANORRLCookieJar::setCookiesFromUrl(const NetworkCookieList& cookieList, const QUrl & url) 
{	
    if(!url.isValid())
        return false;
...
NetworkCookieList ANORRLCookieJar::cookiesForUrl(const QUrl & url) const 
{
    if(!url.isValid())
...
void ANORRLCookieJar::loadCookiesFromDisk(const QString &url, bool authenticate) const
{	
		...
		for (int j = 0; j < childKeys.size(); ++j) 
		{
			...
			if (!expDateStr.isEmpty()) 
			{
				...
				if (cookieExpTime > QDateTime::currentDateTimeUtc()) 
				{
					...
					// User is logged in from previous ANORRL Studio session
...
void ANORRLCookieJar::transferCookieToNativeHttpLayer(const QString& domain, const QNetworkCookie& cookie)
{
	ARL::Http::setCookiesForDomain(domain.toStdString(),
...
void ANORRLCookieJar::authenticateHttpLayer(bool isAuthenticated) const
{
...
QString ANORRLCookieJar::getCookieValue(const QString &hostURL, const QString& cookieKey)
{	
	QUrl url(hostURL);
	if (!url.isValid())
		return QString();
