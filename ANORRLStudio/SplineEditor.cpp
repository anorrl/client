    ...
    case QEvent::MouseButtonPress:
        {
            ...
            if( buttons == Qt::LeftButton && m_mode == Free)
            {
                ...
                else if( m_spline->pt.size() < ARL::NumberSequence::kMaxSize ) // we're not on a point, create a new point and drag it
                {
...
{
    show = show && m_spline->pt.size() < ARL::NumberSequence::kMaxSize;
    if( show )
    {
        float x,y;
...
static ARL::NumberSequence::Key convert(const G3D::Vector4& a)
{
    return ARL::NumberSequence::Key(a.x, a.y, a.z);
}

static G3D::Vector4 convert(const ARL::NumberSequence::Key& k, int j )
{
    return G3D::Vector4( k.time, k.value, k.envelope, 0.1f + j );
}

static void convert(SplineDef* dest, const ARL::NumberSequence* src, float ymax)
{
    const std::vector<ARL::NumberSequence::Key>& keys = src->getPoints();
    dest->ymax = ymax;
...
static void convert(ARL::NumberSequence* dest, const SplineDef* src)
{
    std::vector<ARL::NumberSequence::Key> keys;
    keys.reserve( src->pt.size() );
    for( int j=0, e=src->pt.size(); j<e; ++j )
    {
        ARL::NumberSequence::Key key = convert(src->pt[j]);
        if (key.time >= 0.0f && key.time <= 1.0f)
            keys.push_back(key);
    }
    if (!ARL::NumberSequence::validate(keys, false))
        ARLASSERT(ARL::NumberSequence::validate(keys, false)); // if this happens, then the editor is supplying a borked curve and this conversion function is not smart enough to filter it
    *dest = keys;
}

SplineEditorAdapter::SplineEditorAdapter( const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor, boost::shared_ptr<ARL::DataModel> dataModel, ARL::Instance* inst )
    ...
    , m_inhibitPropChanged(false)
{
    ...
    if (pPropertyDescriptor == &ARL::CustomParticleEmitter::prop_transp)
    {
        ymax = 1;
        settingsName = "Transparency";
    }
    else if (pPropertyDescriptor == &ARL::CustomParticleEmitter::prop_size)
    {
        ymax = 10;
        settingsName = "Size";
    }

    ARL::Reflection::Variant variant;
    m_pPropertyDescriptor->getVariant( m_pInst, variant );
    ARL::NumberSequence ns = variant.get<ARL::NumberSequence>();
...
void SplineEditorAdapter::onEdited(bool undo)
{
    ...
    if( undo )
    {
        ARL::ChangeHistoryService::requestWaypoint(m_pPropertyDescriptor->name.c_str(), m_dataModel.get());
    }

    m_inhibitPropChanged = false;
}

void SplineEditorAdapter::onPropChanged(const ARL::Reflection::PropertyDescriptor* pd)
{
...
void SplineEditorAdapter::onAncestryChanged(shared_ptr<ARL::Instance>, shared_ptr<ARL::Instance>)
{
    ARL::DataModel* dm = ARL::DataModel::get(m_pInst);
