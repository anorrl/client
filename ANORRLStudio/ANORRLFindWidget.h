...
class ANORRLFindWidget : public QWidget
{
	Q_OBJECT
public:
	ANORRLFindWidget(QWidget *pParent = 0);
	~ANORRLFindWidget();

	static ANORRLFindWidget& singleton();
...
private:
	...
	void populateWidget(shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags, bool allScripts);
	...
	void getNextScript(int index, shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags);
	void searchScriptChain(int index, const QString& sourceText, shared_ptr<ARL::DataModel> dm, const QString& findStr, eFindFlags flags, const QString& heirarchy);

	QString getInstanceHierarchy(shared_ptr<ARL::Instance> script);
