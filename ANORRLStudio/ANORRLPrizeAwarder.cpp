/**
 * ANORRLPrizeAwarder.cpp
 * Copyright (c) 2013 ROBLOX Corp. All Rights Reserved.
 */

#include "stdafx.h"
#include "ANORRLPrizeAwarder.h"
...
ANORRLPrizeAwarder::ANORRLPrizeAwarder() 
	: m_initialized(false)
{
...
void ANORRLPrizeAwarder::init()
{
	...
	try
	{
		...
		QString prizesAvailable = ARL::ClientAppSettings::singleton().GetValuePrizeAssetIDs();
...
*/
QString ANORRLPrizeAwarder::generateInsecureString(long assetID, int userId)
{
...
void ANORRLPrizeAwarder::awardPrize( const PrizeType type )
{
	try
	{
		// FFlag
		const char* prizeAssetIds = ARL::ClientAppSettings::singleton().GetValuePrizeAssetIDs();
		...
		ARLASSERT(m_prizeAssetDataMap.size());
		...
		int webKitUserId = ANORRLUser::singleton().getUserId();
		if (!webKitUserId)
			return;

		QString	url("%1%2?assetID=%3&token=%4");
		url = url.arg(ANORRLSettings::getBaseURL())
			.arg(ARL::ClientAppSettings::singleton().GetValuePrizeAwarderURL())
			.arg(assetId)
		...
		QObject::connect(networkReply, SIGNAL(finished()), &ANORRLPrizeAwarder::singleton(), SLOT(handlePrizeAwardRequestFinished()));
	}
	catch (std::exception)
	{
	}	
...
*/
void ANORRLPrizeAwarder::handleImageRequestFinished()
{
	ANORRLNetworkReply* networkReply = dynamic_cast<ANORRLNetworkReply*>(sender());
	if (!networkReply)
		return;
...
*/
void ANORRLPrizeAwarder::setPrizeAwarded(PrizeType type, bool isPerUser) const
{
	// Persist to settings so we dont do again
	ANORRLSettings settings;
	int userId = 0;
	if (isPerUser)
		userId = ANORRLUser::singleton().getUserId();
...
*/
bool ANORRLPrizeAwarder::isPrizeAwardedAlready(PrizeType type, bool isPerUser) const
{
	// TODO: Per session?
	ANORRLSettings settings;
	int userId = 0;
...
*/
void ANORRLPrizeAwarder::handlePrizeAwardRequestFinished()
{
	ANORRLNetworkReply* networkReply = dynamic_cast<ANORRLNetworkReply*>(sender());
	if (!networkReply)
		return;
	...
	switch (statusCode)
	{
		case 200: // Success
		{
			...
			ANORRLNetworkReply* imageNetworkReply = ANORRLNetworkAccessManager::Instance().get(networkRequest);
