    ...
    const char*     member )
{
    ...
    for ( ; iter != keys.end() ; ++iter )
    {
        ...
        action->setIcon(QIcon(QtUtilities::getPixmap(":/images/ClassImages.PNG",ANORRLTreeWidgetItem::getImageIndex(instance))));        
...
, m_closeDockWhenDoneWithQuickInsert(false)
{
	...
	m_pCheckBox->setChecked(ANORRLSettings().value(kInsertedItemSelectedSetting, false).toBool());
...
{
	ANORRLSettings().setValue(kInsertedItemSelectedSetting, checked);
}
	...
	QPoint* mousePosition)
{
	try
	{
...

		{
			...
			{
				ANORRLExplorerWidget& explorer = UpdateUIManager::Instance().getViewWidget<ANORRLExplorerWidget>(eDW_OBJECT_EXPLORER);
				ANORRLTreeWidget* pTreeWidget = explorer.getTreeWidget();

				if ( pTreeWidget && pTreeWidget->dataModel() )
					QApplication::postEvent(
					pTreeWidget, 
					new ANORRLCustomEventWithArg(TREE_SCROLL_TO_INSTANCE, 
					boost::bind(
					&ANORRLTreeWidget::scrollToInstance, 
					pTreeWidget, 
					pObjectToInsert.get() ) ) );
		...
		if (!buffer.empty())
			ANORRLDocManager::Instance().openDoc(buffer);
		else
...
ARL::Vector3 InsertObjectWidget::getInsertLocation(boost::shared_ptr<ARL::DataModel> pDataModel, QPoint* insertPosition, bool* isOnPart)
{
	...
	{
		QWidget* viewer = ANORRLDocManager::Instance().getPlayDoc()->getViewer();
		if (viewer)
		{
