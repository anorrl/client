...
typedef boost::unordered_map<int, shared_ptr<const ARL::Instances> > PlayerSelectionMap;
...
private:
	Q_INVOKABLE void headshotThumbnailDataLoaded(int playerId, ARL::HttpFuture future);
	Q_INVOKABLE void headshotThumbnailLoaded(int playerId, ARL::HttpFuture future);
...
class PlayersDataManager: public QObject
{
	Q_OBJECT
public:
	PlayersDataManager(QObject* parent, boost::shared_ptr<ARL::DataModel> dataModel);
	~PlayersDataManager();
	...
	shared_ptr<const ARL::Instances> getLastKnownSelection(int playerId);

	bool isLocalPlayer(boost::shared_ptr<ARL::Network::Player> player);
	boost::shared_ptr<ARL::Network::Player> getPlayer(const QString& playerName, Qt::CaseSensitivity cs = Qt::CaseInsensitive);
...
private:
	...
	void onPlayerAdded(shared_ptr<ARL::Instance> playerInstance);
	void onPlayerRemoving(shared_ptr<ARL::Instance> playerInstance);
	void onCloudEditSelectionChanged(int playerId, shared_ptr<const ARL::Reflection::ValueArray>& newSelection);

	Q_INVOKABLE void onPlayerNameLoaded(ARL::HttpFuture future);
