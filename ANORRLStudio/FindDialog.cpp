...
ANORRLFindWidget::eFindFlags FindAllDialog::getCurrentFlags()
{
	ANORRLFindWidget::eFindFlags flagSetter = ANORRLFindWidget::NO_FLAGS;

	if (!m_pFindOptions->isChecked())
		return flagSetter;

	if (m_pCaseCheckBox->isChecked())
		flagSetter = flagSetter | ANORRLFindWidget::MATCH_CASE;

	if (m_pWholeCheckBox->isChecked())
		flagSetter = flagSetter | ANORRLFindWidget::MATCH_WORD;

	if (m_pRegExCheckBox->isChecked())
		flagSetter = flagSetter | ANORRLFindWidget::REG_EXP;
...
void FindAllDialog::findAll()
{
	...
	ANORRLFindWidget::singleton().findAll(m_pFindEdit->text(), (m_pLookInComboBox->currentIndex() == 0), getCurrentFlags());
	close();
}
...
void FindAllDialog::setFindAllRunning(bool value)
{
	if (value)
	{
		...
		connect(m_pFindAllButton, SIGNAL(clicked()), &ANORRLFindWidget::singleton(), SLOT(stopPopulatingItems()));
	}
	else
	{
		m_pFindAllButton->setText("Find All");
		disconnect(m_pFindAllButton, SIGNAL(clicked()), &ANORRLFindWidget::singleton(), SLOT(stopPopulatingItems()));
		connect(m_pFindAllButton, SIGNAL(clicked()), this, SLOT(findAll()));
	}
}
