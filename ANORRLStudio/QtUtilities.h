...
namespace QtUtilities
{
	...
	class ARLMessageBox : public QMessageBox
	{
	public:
		ARLMessageBox(QWidget* parent = 0);
		ARLMessageBox(QMessageBox::Icon icon, const QString & title, const QString & text, 
			QMessageBox::StandardButtons buttons = NoButton, QWidget *parent = 0, 
			Qt::WindowFlags f = Qt::Dialog | Qt::MSWindowsFixedSizeDialogHint );
	};

	class ARLConfirmationMessageBox : public QDialog
	{
		Q_OBJECT
	private:
		QLabel *label;
		...
		bool clickedYes;
	public:
		ARLConfirmationMessageBox(QString question, QString yesButtonName = QObject::tr("Yes"), QString noButtonName = QObject::tr("No"),  QWidget* parent = 0);
