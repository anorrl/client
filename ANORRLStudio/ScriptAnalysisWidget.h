...
public:
	...
	void setDataModel(boost::shared_ptr<ARL::DataModel> dataModel);

	void updateResults(const LuaSourceBuffer& script, const ARL::ScriptAnalyzer::Result& result, bool showErrors, bool showWarnings);
	void reset();

	void scrollTo(const LuaSourceBuffer& script);
	void updateCategoryItem(QTreeWidgetItem* pItem);

	void instanceRemoving(boost::shared_ptr<ARL::Instance> instance);
...
private:
	...
	void onScriptParentChanged(boost::shared_ptr<ARL::Instance> instance, boost::shared_ptr<ARL::Instance> newParent);
...
public:
	...
	void updateResults(const LuaSourceBuffer& script, const ARL::ScriptAnalyzer::Result& result);
    void setDataModel(boost::shared_ptr<ARL::DataModel> dataModel);
...
private:
    ...
    void updateResultsInstance(const shared_ptr<ARL::Instance>& instance);

	void onInstanceRemoving(boost::shared_ptr<ARL::Instance> instance);
	...
	Q_INVOKABLE void analyzeScriptAssetSource(const QString& scriptAssetName, ARL::HttpFuture future);
	
	void updateResultsScriptAsset(const QString& scriptAssetName, const ARL::ScriptAnalyzer::Result& scriptResults);
	
	Q_INVOKABLE void updateResultsInternal(shared_ptr<ARL::Instance> script, const ARL::ScriptAnalyzer::Result& result);
	Q_INVOKABLE void analyzeAndUpdateResults(shared_ptr<ARL::Instance> script);
