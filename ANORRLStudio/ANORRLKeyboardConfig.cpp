...
ShortcutsModel::ShortcutsModel(ANORRLMainWindow& mainWindow, QObject *parent)
...
ANORRLKeyboardConfig::ANORRLKeyboardConfig()
{
}

ANORRLKeyboardConfig& ANORRLKeyboardConfig::singleton()
{
    static ANORRLKeyboardConfig* anorrlKeyboardConfig = new ANORRLKeyboardConfig();
    return *anorrlKeyboardConfig;
}

void ANORRLKeyboardConfig::storeDefaults(ANORRLMainWindow& mainWindow)
{
...
void ANORRLKeyboardConfig::saveKeyboardConfig(ANORRLMainWindow& mainWindow)
{
    ...
    //
    settings.beginGroup("ANORRL Studio Key Mapping");
    settings.remove("");
    settings.endGroup();
...
void ANORRLKeyboardConfig::loadKeyboardConfig(ANORRLMainWindow& mainWindow)
{
...
ANORRLKeyboardConfigWidget::ANORRLKeyboardConfigWidget(ANORRLMainWindow& MainWindow,
                                                       QWidget* parent)
    : QWidget(parent)
    , m_mainWindow(MainWindow)
	, m_filterEdit(NULL)
...
void ANORRLKeyboardConfigWidget::initialize()
{
    ARLASSERT(!layout());

    ARL::BaldPtr<QLayout> layout = new QVBoxLayout;
...
void ANORRLKeyboardConfigWidget::cancel()
{
    m_model->resetOverrides();
}

void ANORRLKeyboardConfigWidget::accept()
{
    m_model->commitOverrides();
	m_mainWindow.updateShortcutSet();
    ANORRLKeyboardConfig::singleton().saveKeyboardConfig(m_mainWindow);
}

void ANORRLKeyboardConfigWidget::restoreAllDefaults()
{
    ...
    for (; iter != objects.end(); ++iter)
    {
            ...
            !ShortcutExceptions.contains(action->objectName()))
        {
            QMap<QString, QKeySequence>::const_iterator iter =
                ANORRLKeyboardConfig::singleton().defaultShortcuts().find(action->objectName());
            if (iter != ANORRLKeyboardConfig::singleton().defaultShortcuts().end())
            {
                action->setShortcut(iter.value());
            }
        }
...
void ANORRLKeyboardConfigWidget::dataChanged(const QModelIndex&,
                                             const QModelIndex&)
{
    Q_EMIT(dataChanged());
}

void ANORRLKeyboardConfigWidget::setFilter(QString filter)
{
