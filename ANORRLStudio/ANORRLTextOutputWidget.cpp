...
ANORRLTextOutputWidget::ANORRLTextOutputWidget(QWidget* parent)
    : QPlainTextEdit(parent)
{
    ...
    m_TextFormats[ARL::MESSAGE_OUTPUT].setFontWeight(QFont::Normal);

    m_TextFormats[ARL::MESSAGE_INFO].setFontWeight(QFont::Normal);
    m_TextFormats[ARL::MESSAGE_INFO].setForeground(Qt::blue);

    m_TextFormats[ARL::MESSAGE_WARNING].setFontWeight(QFont::Bold);
    m_TextFormats[ARL::MESSAGE_WARNING].setForeground(QColor(255,128,0));

    m_TextFormats[ARL::MESSAGE_ERROR].setFontWeight(QFont::Bold);
    m_TextFormats[ARL::MESSAGE_ERROR].setForeground(Qt::red);

	m_TextFormats[ARL::MESSAGE_SENSITIVE].setFontWeight(QFont::Bold);
	m_TextFormats[ARL::MESSAGE_SENSITIVE].setForeground(Qt::darkMagenta);
    ...
    m_PropertyChangedConnection = AuthoringSettings::singleton().propertyChangedSignal.connect(
        boost::bind(&ANORRLTextOutputWidget::onPropertyChanged,this,_1));
    onPropertyChanged(NULL);

    connect(
        &ANORRL::Instance(),SIGNAL(newOutputMessage(const QString,ARL::MessageType)), 
        this,SLOT(appendOutputText(const QString&,ARL::MessageType)) );
}

ANORRLTextOutputWidget::~ANORRLTextOutputWidget()
{
    m_PropertyChangedConnection.disconnect();
}

QSize ANORRLTextOutputWidget::sizeHint() const
{
	return QSize(width(), 104);
}

void ANORRLTextOutputWidget::contextMenuEvent(QContextMenuEvent * evt)
{
...
bool ANORRLTextOutputWidget::event(QEvent* evt)
{
    bool handled = false;
...
void ANORRLTextOutputWidget::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
 ...
 */
void ANORRLTextOutputWidget::appendOutputText(const QString message,ARL::MessageType type)
{
    ARLASSERT(QThread::currentThread() == qApp->thread());

    QString text = message;

	if ( type != ARL::MESSAGE_OUTPUT )
	{
