...
void InsertServiceDialog::setDataModel(boost::shared_ptr<ARL::DataModel> pDataModel)
{
...
void InsertServiceDialog::recreateWidget()
{
    ...
    InsertObjectWidget::createWidgets(true, m_pInsertObjectListWidget, &ARL::Reflection::ClassDescriptor::rootDescriptor(), m_pDataModel);
...
void InsertServiceDialog::onItemInsertRequested(QListWidgetItem* pListWidgetItem)
{
	...
	try
	{
		...
		boost::shared_ptr<ARL::Instance> pService = ARL::ServiceProvider::create(m_pDataModel.get(), pInsertListWidgetItem->getInstance()->getClassName());
		if (!pService)
		...
		//for undo redo -- 
		std::string action = ARL::format("Insert %s", pService->getName().c_str());
		ARL::ChangeHistoryService::requestWaypoint(action.c_str(), m_pDataModel.get());
	}
	catch(std::bad_alloc&)
	{
		throw;
    ...
    if (!isAvailable())
    {
        ANORRLMainWindow* mainWindow = static_cast<ANORRLMainWindow*>(this->parentWidget());
...
void InsertServiceDialog::redrawDialog()
{
	if (m_pDataModel)
	{
		...
		{		
			...
			if (pCurrentItem)
			{
				ARLASSERT(dynamic_cast<ARL::Service*>(pCurrentItem->getInstance().get()));
				//ARL::StandardOut::singleton()->printf(ARL::MESSAGE_WARNING, "InsertService: current object = %s and visible = %d", pCurrentItem->getInstance()->getClassNameStr().c_str(), visible);
