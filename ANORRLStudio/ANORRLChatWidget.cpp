...
void ChatMessageItem::paint(QPainter* painter, const QStyleOptionViewItem& option)
{
	...
	painter->fillPath(path, pointingRight ? QBrush(QColor("#ce99ff")) : QBrush(QColor("#e3e3e3")));
...
void ChatMessageLineEdit::onTextEdited(const QString& message)
{
	...
	else if (messageText.startsWith("/w ", Qt::CaseInsensitive) || messageText.startsWith("/whisper ", Qt::CaseInsensitive))
	{
		...
		QString playerName;
		boost::shared_ptr<ARL::Network::Player> player = m_pPlayersDataManager->getPlayer(messageText.mid(0, index));
		if (player)
...
// ANORRLChatWidget
//--------------------------------------------------------------------------------------------
ANORRLChatWidget::ANORRLChatWidget(QWidget* parent, boost::shared_ptr<ARL::DataModel> dataModel, boost::shared_ptr<PlayersDataManager> playerDataManager)
: QFrame(parent)
...
, m_pPlayersDataManager(playerDataManager)
{
	...
	ARLASSERT(players);
	m_cPlayerChatMessage = players->chatMessageSignal.connect(boost::bind(&ANORRLChatWidget::onPlayerChatMessage, this, _1));
	...
	ARLASSERT(chatDockWidget && chatDockWidget->toggleViewAction());
	QIcon icon = chatDockWidget->toggleViewAction()->icon();
	m_OriginalChatActionPixmap = icon.pixmap(icon.availableSizes()[0]);
}

void ANORRLChatWidget::onPlayerChatMessage(const ARL::Network::ChatMessage& chatMessage)
{
	...
	{
	case ARL::Network::ChatMessage::CHAT_TYPE_ALL:
		chatType = StudioChat::All;
		break;
	case ARL::Network::ChatMessage::CHAT_TYPE_WHISPER:
		chatType = StudioChat::Whisper;
		break;
	case ARL::Network::ChatMessage::CHAT_TYPE_GAME:
...
void ANORRLChatWidget::addMessage(StudioChat::ChatMessageData messageData)
{
...
void ANORRLChatWidget::onReturnPressed(int chatType, const QString& message, const QString& targetPlayerName)
{
...
void ANORRLChatWidget::sendMessage(const StudioChat::ChatMessageData& messageData, QString& errorMessage)
{
	...
	if (players)
	{
		try
		{
			...
			{
				if (boost::shared_ptr<ARL::Network::Player> player = m_pPlayersDataManager->getPlayer(messageData.playerName))
				{
...
void ANORRLChatWidget::setToggleChatActionPixmap(const QPixmap& pixmap)
{
	QDockWidget* chatDockWidget = UpdateUIManager::Instance().getMainWindow().findChild<QDockWidget*>("chatDockWidget");
	ARLASSERT(chatDockWidget && chatDockWidget->toggleViewAction());
...
void ANORRLChatWidget::updateToggleChatActionPixmap()
{
