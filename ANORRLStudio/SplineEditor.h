...
namespace ARL { class Instance; class NumberSequence; }
namespace ARL { namespace Reflection { class PropertyDescriptor; } }
namespace ARL { class DataModel; }
...
class SplineEditorAdapter : public QObject
{
    Q_OBJECT;
public:
    SplineEditorAdapter( const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor, boost::shared_ptr<ARL::DataModel> dataModel, ARL::Instance* inst );
    ~SplineEditorAdapter();
...
private:
    ...
    shared_ptr<ARL::DataModel> m_dataModel;
...
private Q_SLOTS:
    ...
    void onPropChanged(const ARL::Reflection::PropertyDescriptor* pd); // called when the property changes externally to refresh the stuff
    void onAncestryChanged(shared_ptr<ARL::Instance>, shared_ptr<ARL::Instance>);
};
