...
: m_GroupId(groupId)
{
	...
	QString str = QString("%1/groups/%2").arg(ANORRLSettings::getApiBaseURL()).arg(groupId);
...
bool UserPlaceCreatorsListWidget::canDeleteSelectedCreator()
{
	...
	if (m_PlaceOwnerId > 0 && (ANORRLUser::singleton().getUserId() == m_PlaceOwnerId))
	{
		QList<QTreeWidgetItem*> items = selectedItems();
		if (items.size() == 1)
		{
...
//--------------------------------------------------------------------------------------------
// ANORRLTeamCreateWidget
//--------------------------------------------------------------------------------------------
ANORRLTeamCreateWidget::ANORRLTeamCreateWidget(QWidget* parent)
: QWidget(parent)
...
void ANORRLTeamCreateWidget::setDataModel(boost::shared_ptr<ARL::DataModel> dataModel, boost::shared_ptr<PlayersDataManager> pPlayersDataManager)
{
	...
	if (m_pDataModel)
	{
		...
		m_cPlaceIdChange = m_pDataModel->propertyChangedSignal.connect(boost::bind(&ANORRLTeamCreateWidget::onPlaceIdChanged, this, _1) );
	}

	QMetaObject::invokeMethod(this, "updateCurrentWidgetInStack", Qt::QueuedConnection);
}

PlaceCreatorDetails ANORRLTeamCreateWidget::getPlaceCreatorDetails()
{
...
void ANORRLTeamCreateWidget::updateCurrentWidgetInStack()
{
	m_bLoginButtonClicked = false;
	...
	else if (ANORRLUser::singleton().getUserId() <= 0)
	{		
	...
	else if (ANORRLIDEDoc::getIsCloudEditSession())
	{
...
void ANORRLTeamCreateWidget::addCreatorsListWidget(eTeamType creatorType)
{
...
void ANORRLTeamCreateWidget::onAuthenticationChanged(bool)
{
	// TODO: shall we reset the widget if user changes?

	IANORRLDoc* pDoc = ANORRLDocManager::Instance().getPlayDoc();
	if (FFlag::TeamCreateEnablePublishOnLogin && m_bLoginButtonClicked && pDoc)
	{
		// bring play doc to the front
		ANORRLDocManager::Instance().setCurrentDoc(pDoc);
...
void ANORRLTeamCreateWidget::onLoginButtonClicked()
{
...
void ANORRLTeamCreateWidget::onPublishButtonClicked()
{
	...
	{		
		...
		if (ANORRLUser::singleton().getUserId() <= 0)
		{
			QMetaObject::invokeMethod(this, "updateCurrentWidgetInStack", Qt::QueuedConnection);
			return;
		}
	...
	PublishToANORRLAsVerb* publishToANORRLVerb = dynamic_cast<PublishToANORRLAsVerb*>(m_pDataModel->getVerb("PublishToANORRLAsVerb"));
	if(publishToANORRLVerb)
	{
		publishToANORRLVerb->doIt(m_pDataModel.get());
		QDialog* pDialog = publishToANORRLVerb->getPublishDialog();
		if (pDialog)
		{
...
void ANORRLTeamCreateWidget::onTurnOnButtonClicked()
{
	...
	// publish place
	UpdateUIManager::Instance().getMainWindow().publishToANORRLAction->trigger();

	// turn on cloud edit
	QString postUrl = QString("%1/universes/%2/enablecloudedit").arg(ANORRLSettings::getApiBaseURL()).arg(m_pDataModel->getUniverseId());
...
void ANORRLTeamCreateWidget::onBottomMoreButtonClicked()
{
	QMenu menu(this);
	QAction* cloudEditTurnOffAction = NULL;
	QAction* deleteUserAction = NULL;
	if (ANORRLIDEDoc::getIsCloudEditSession() && canManageCurrentPlace())
	{
	...
	if (resultAction == cloudEditTurnOffAction)
	{
		...
		QString postUrl = QString("%1/universes/%2/disablecloudedit").arg(ANORRLSettings::getApiBaseURL()).arg(m_pDataModel->getUniverseId());
		std::string result = doPost(postUrl.toStdString());
		if (result == "{}")
		{
	...
	else if (resultAction == helpAction)
	{
		if (FFlag::TeamCreateEnableContextHelp)
		{
			...
			QMetaObject::invokeMethod(&ANORRLContextualHelpService::singleton(), "onHelpTopicChanged", Q_ARG(QString, QString("Team_Create")));
		}
		else
		{
...
void ANORRLTeamCreateWidget::onPublishDialogFinished(int status)
{
	disconnect(qobject_cast<QDialog*>(sender()), SIGNAL(finished(int)), this, SLOT(onPublishDialogFinished(int)));
	QMetaObject::invokeMethod(this, "updateCurrentWidgetInStack", Qt::QueuedConnection);
}

void ANORRLTeamCreateWidget::onPlaceIdChanged(const ARL::Reflection::PropertyDescriptor* desc)
{
	if (!desc || desc->name != "PlaceId")
		return;

	QString canManageUrl = QString("%1/users/%2/canmanage/%3").arg(ANORRLSettings::getApiBaseURL()).arg(ANORRLUser::singleton().getUserId()).arg(m_pDataModel->getPlaceID());
	m_PlaceManageStatusQuery = ARL::HttpAsync::get(canManageUrl.toStdString()).then(boost::bind(&ANORRLTeamCreateWidget::updatePlaceManageStatus, this, _1));
}

eTeamType ANORRLTeamCreateWidget::getCreatorType()
{
	if (m_PlaceDetailsQuery.future_ && !m_PlaceDetailsQuery.is_ready())
		m_PlaceDetailsQuery.get();
...
void ANORRLTeamCreateWidget::updatePlaceManageStatus(ARL::HttpFuture future)
{
	...
	int universeId = -1;
	if (m_bCanManageCurrentPlace && !ANORRLIDEDoc::getIsCloudEditSession() && canLaunchCloudEdit(universeId))
	{
	...
	QString url = m_bCanManageCurrentPlace ? QString("%1/places/%2/settings").arg(ANORRLSettings::getBaseURL()).arg(placeId)
		                                   : QString("%1/Marketplace/ProductInfo?assetId=%2").arg(ANORRLSettings::getApiBaseURL()).arg(placeId);
	m_PlaceDetailsQuery = ARL::HttpAsync::get(url.toStdString()).then(boost::bind(&ANORRLTeamCreateWidget::updatePlaceDetails, this, _1));	
}

void ANORRLTeamCreateWidget::updatePlaceDetails(ARL::HttpFuture future)
{
...
bool ANORRLTeamCreateWidget::canManageCurrentPlace()
{
...
void ANORRLTeamCreateWidget::enableFriendsInviteEdit(bool state)
{
...
void ANORRLTeamCreateWidget::loadFriendsList()
{
	if (m_pFriendsInfoLoader)
		return;

	m_pFriendsInfoLoader = new FriendsInfoLoader(this, ANORRLUser::singleton().getUserId());
	connect(m_pFriendsInfoLoader, SIGNAL(finishedLoading()), this, SLOT(updatePlayerFriendsInfo()));
}

void ANORRLTeamCreateWidget::updatePlayerFriendsInfo()
{
	if (!canManageCurrentPlace())
		return;
...
void ANORRLTeamCreateWidget::onFriendsInviteEditReturnPressed()
{
	...
	prop.setFromJsonFuture(ARL::HttpAsync::get(QString("%1/users/get-by-username?username=%2").arg(ANORRLSettings::getApiBaseURL()).arg(playerName).toStdString()));
...
void ANORRLTeamCreateWidget::onCompleterPopupMenuItemClicked(const QModelIndex& clickedIndex)
{
...
void ANORRLTeamCreateWidget::onRemoveCloudEditorRequested(int playerId)
{
...
void ANORRLTeamCreateWidget::onRemoveCloudEditConfirm()
{
	QApplication::setOverrideCursor(Qt::BusyCursor);
...
void ANORRLTeamCreateWidget::onRemoveCloudEditCancel()
{
	userRemoveButton->setProperty("userIdToDelete", QVariant());
	QMetaObject::invokeMethod(this, "updateCurrentWidgetInStack", Qt::QueuedConnection);
}

bool ANORRLTeamCreateWidget::addCloudEditor(int playerId)
{
	...
	{		
		QString postUrl = QString("%1/universes/%2/enablecloudedit").arg(ANORRLSettings::getApiBaseURL()).arg(universeId);
	...
	if (canAddEditor)
	{
		QString postUrl = QString("%1/universes/%2/addcloudeditor?userId=%3").arg(ANORRLSettings::getApiBaseURL()).arg(universeId).arg(playerId);
		std::string result = doPost(postUrl.toStdString());	
		return result == "{}";
	}
...
bool ANORRLTeamCreateWidget::removeCloudEditor(int playerId)
{
	int universeId = m_pDataModel->getUniverseId();
	...
	QString postUrl = QString("%1/universes/%2/removecloudeditor?userId=%3").arg(ANORRLSettings::getApiBaseURL()).arg(universeId).arg(playerId);
	std::string result = doPost(postUrl.toStdString());
	return result == "{}";
}

bool ANORRLTeamCreateWidget::canLaunchCloudEdit(int &universeId)
{
	if (FFlag::DoNotCheckCanCloudEditWhenNotLoggedIn && ANORRLUser::singleton().getUserId() <= 0)
	{
		return false;
	}
	...
	if (universeId <= 0)
	{
		...
		QString getUniverseUrl = QString("%1//universes/get-universe-containing-place?placeId=%2").arg(ANORRLSettings::getApiBaseURL()).arg(placeId);
	...
	if (universeId > 0)
	{
		...
		QString cloudEditEnabledUrl = QString("%1/universes/%2/cloudeditenabled").arg(ANORRLSettings::getApiBaseURL()).arg(universeId);
...
void ANORRLTeamCreateWidget::resetDataModel()
{	setDataModel(shared_ptr<ARL::DataModel>(), shared_ptr<PlayersDataManager>()); }

std::string ANORRLTeamCreateWidget::doPost(const std::string& postUrl)
{
	std::string result;
	try
	{
...
{
	QString str = QString("%1/users/%2/friends?page=%3").arg(ANORRLSettings::getApiBaseURL()).arg(m_PlayerId).arg(pageToLoad);
...
{
	QString str = QString("%1/universes/%2/listcloudeditors?page=%3").arg(ANORRLSettings::getApiBaseURL()).arg(m_UniverseId).arg(pageToLoad);
