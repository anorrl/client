...
void ScriptSyntaxHighlighter::setLexState(ARLLuaLexState lexState, int currentPos)
{
	...
	switch(m_lexState)
	{
        case ARL_LUA_COMMENT:
        case ARL_LUA_COMMENTLINE:
        ...
        case ARL_LUA_OPERATOR:
            ...
            break;
        case ARL_LUA_STRING:
        case ARL_LUA_STRINGEOL:
        case ARL_LUA_CHARACTER:
        case ARL_LUA_LITERALSTRING:
...
bool ScriptSyntaxHighlighter::checkApplyFoldState(const QString &keyword)
{
	...
	ARLTextUserData* pUserData = dynamic_cast<ARLTextUserData*>(currentBlockUserData());
	if(foldState != 0)
	{
		if(!pUserData)
			pUserData = new ARLTextUserData();
...
void ScriptSyntaxHighlighter::highlightBlock(const QString &text)
{
	...
	if(previousBlockState() > -1)
		m_lexState = ARLLuaLexState(previousBlockState());

	if (m_lexState == ARL_LUA_LITERALSTRING || m_lexState == ARL_LUA_COMMENT) 
	{
		QTextBlock previousBlock = currentBlock().previous();
		if(previousBlock.isValid())
		{
			ARLTextUserData* pPreviousUserData = dynamic_cast<ARLTextUserData*>(previousBlock.userData());
	...
	for(int ii=0; ii < text.size(); ii++)
	{
		...
		else if (m_lexState == ARL_LUA_IDENTIFIER) 
		{
			if (!IsAWordChar(text[ii].toAscii()) || (text[ii] == QChar('.')))
			{
				...
				{
					m_lexState = ARL_LUA_WORD;
					hasFold |= checkApplyFoldState(keyword);
				}
				setLexState(ARL_LUA_DEFAULT, ii);				
			}
		} 
		else if (m_lexState == ARL_LUA_COMMENTLINE || m_lexState == ARL_LUA_PREPROCESSOR) 
		{
			if (ii == text.size()-1) 
				setLexState(ARL_LUA_DEFAULT, ++ii);
		} 
		else if (m_lexState == ARL_LUA_STRING) 
		{
			if (text[ii] == QChar('\\'))
			{
				if(ii+1 < text.size())
			...
			{
				m_lexState = ARL_LUA_STRINGEOL;
				setLexState(ARL_LUA_DEFAULT, ++ii);				
			}
		} 
		else if (m_lexState == ARL_LUA_CHARACTER) 
		{
			if (text[ii] == QChar('\\')) 
			{
				if(ii+1 < text.size())
		...
		else if (m_lexState == ARL_LUA_LITERALSTRING || m_lexState == ARL_LUA_COMMENT) 
		{
			if (text[ii] == QChar('[')) 
			{
		...
		if (m_lexState == ARL_LUA_DEFAULT) 
		{
			...
			{
				m_lexState = ARL_LUA_STRING;
			} 
			else if (text[ii] == QChar('\'')) 
			{
				m_lexState = ARL_LUA_CHARACTER;
			} 
			else if (text[ii] == QChar('['))
			{
				...
				else 
				{
					m_nestLevel = 1;
					hasFold |= checkApplyFoldState("[==[");
					setLexState(ARL_LUA_LITERALSTRING, ii);
					ii += m_sepCount;
				}
			} 
			else if (text[ii] == QChar('-') && (ii+1 < text.size()) && text[ii+1] == QChar('-'))
			{
				setLexState(ARL_LUA_COMMENTLINE, ii);
	...
	if (m_lexState == ARL_LUA_OPERATOR)
	{
		...
		setLexState(ARL_LUA_DEFAULT, finalIndex);
	}
	else if (m_lexState == ARL_LUA_NUMBER || m_lexState == ARL_LUA_COMMENTLINE || m_lexState == ARL_LUA_PREPROCESSOR)		
	{
		setLexState(ARL_LUA_DEFAULT, finalIndex);			
	}
	else if(m_lexState == ARL_LUA_STRING || m_lexState == ARL_LUA_CHARACTER)
	{
        if (finalIndex > 0)
        {
            if(text[finalIndex-1] == QChar('\\'))
...
void ScriptSyntaxHighlighter::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
