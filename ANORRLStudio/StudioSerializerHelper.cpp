    ...
    bool            publishAssets)
{
	...
	if ( !outputFilename.endsWith(".arl", Qt::CaseInsensitive) && !outputFilename.endsWith(".arlx", Qt::CaseInsensitive) )
		outputFilename.append(".arl");
    ...
    QString*        outErrorMessage )
{
    ...
    *outErrorMessage = "";
    ...
    QTemporaryFile tempFile(QDir::tempPath() + "/ARL-XXXXXX.arl");
    if ( !tempFile.open() )
    {
        *outErrorMessage = tr(
    ...
    QString*        outErrorMessage )
{
    ...
    *outErrorMessage = "";
    ...
    // create a temp file on disk, open to force creation
    QTemporaryFile tempFile(QDir::tempPath() + "/ARL-XXXXXX.bin.arl");
    if ( !tempFile.open() )
    {
        *outErrorMessage = tr(
            "Failed to open temporary file while saving.\n\n"
