...
ScriptTextEditor::ScriptTextEditor(
    ANORRLScriptDoc*  pScriptDoc,
    ANORRLMainWindow* pMainWindow)
...
void ScriptTextEditor::focusInEvent(QFocusEvent* evt)
{
    ...
    ANORRLDocManager::Instance().setCurrentDoc(m_pScriptDoc);
...
void ScriptTextEditor::keyPressEvent(QKeyEvent* e)
{
    ...
    else if ( key == Qt::Key_Return || key == Qt::Key_Backspace )
    {
        ...
        ARLASSERT(!currentTextCursor.isNull());

        QTextBlock startBlock = currentTextCursor.block();
        ARLASSERT(startBlock.isValid());
        ARLASSERT(startBlock.userData());

        ARLTextUserData& userData = static_cast<ARLTextUserData&>(*startBlock.userData());
...
void ScriptTextEditor::autoIndent()
{
    ...
    {
        ARLTextUserData* pUserData = dynamic_cast<ARLTextUserData*>(previousBlock.userData());
        if (pUserData)
        {
            ...
            if ((foldState < 0) || isElse)
            {
                if (isElse)
                {
                    ...
                    while (block.isValid())
                    {
                        const ARLTextUserData* userData = dynamic_cast<ARLTextUserData*>(block.userData());
                        if (userData)
                        {
...
void ScriptTextEditor::setAnalysisResult(const ARL::ScriptAnalyzer::Result& analysisResults)
{
	...
	QList<QTextEdit::ExtraSelection> extraSelections;
	ARL::ScriptAnalyzer::Location loc;
	int startPos = -1, endPos = -1;
	bool atEof = false;

	if (analysisResults.error)
	{
		ARL::ScriptAnalyzer::Error error = analysisResults.error.get();
		loc = error.location;
	...
	{
		ARL::ScriptAnalyzer::Warning warning = analysisResults.warnings[i];
...
void ScriptTextEditor::commentSelection()
{
    ...
    while ( tmpBlock.isValid() && tmpBlock.blockNumber() <= endBlock.blockNumber() )
    {
        ...
        ARLTextUserData* userData = dynamic_cast<ARLTextUserData*>(tmpBlock.userData());
...
void ScriptTextEditor::updateSelection(bool* isBlockUnfolded)
{
    ...
    ARLTextUserData* userData = dynamic_cast<ARLTextUserData*>(startBlock.userData());
...
void ScriptTextEditor::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
...
int ScriptTextEditor::getDocFoldState(int blockNumber)
{
    ...
    {
        ARLTextUserData* pUserData = dynamic_cast<ARLTextUserData*>(iterBlock.userData());
        if (pUserData)
...
void ScriptTextEditor::selectText(const ARL::ScriptAnalyzer::Location& location, bool trySettingBlocksVisible)
{
...
{
	for(ARL::Reflection::ClassDescriptor::ClassDescriptors::const_iterator iter = ARL::Reflection::ClassDescriptor::all_begin();
		iter!=ARL::Reflection::ClassDescriptor::all_end();++iter)
	{
		std::string className = ((*iter)->name).toString();
		updateWikiLookup((*iter)->name.c_str(), "API:Class/" + className);
		
		for(ARL::Reflection::MemberDescriptorContainer<ARL::Reflection::PropertyDescriptor>::Collection::const_iterator propIter = (*iter)->begin<ARL::Reflection::PropertyDescriptor>();
			propIter!=(*iter)->end<ARL::Reflection::PropertyDescriptor>();++propIter)
		...
		for(ARL::Reflection::MemberDescriptorContainer<ARL::Reflection::FunctionDescriptor>::Collection::const_iterator funcIter = (*iter)->begin<ARL::Reflection::FunctionDescriptor>();
			funcIter!=(*iter)->end<ARL::Reflection::FunctionDescriptor>();++funcIter)
		...
		for(ARL::Reflection::MemberDescriptorContainer<ARL::Reflection::YieldFunctionDescriptor>::Collection::const_iterator yfuncIter = (*iter)->begin<ARL::Reflection::YieldFunctionDescriptor>();
			yfuncIter!=(*iter)->end<ARL::Reflection::YieldFunctionDescriptor>();++yfuncIter)
		...
		for(ARL::Reflection::MemberDescriptorContainer<ARL::Reflection::EventDescriptor>::Collection::const_iterator eventIter = (*iter)->begin<ARL::Reflection::EventDescriptor>();
			eventIter!=(*iter)->end<ARL::Reflection::EventDescriptor>();++eventIter)
		...
		for(ARL::Reflection::MemberDescriptorContainer<ARL::Reflection::CallbackDescriptor>::Collection::const_iterator callbackIter = (*iter)->begin<ARL::Reflection::CallbackDescriptor>();
			callbackIter!=(*iter)->end<ARL::Reflection::CallbackDescriptor>();++callbackIter)
	...
	for(std::vector< const ARL::Reflection::EnumDescriptor* >::const_iterator enumIter = ARL::Reflection::EnumDescriptor::enumsBegin();
		enumIter!=ARL::Reflection::EnumDescriptor::enumsEnd();++enumIter)
	{
		std::string enumName = (*enumIter)->name.toString();
		updateWikiLookup((*enumIter)->name.c_str(), "API:Enum/" + enumName);

		for(std::vector< const ARL::Reflection::EnumDescriptor::Item* >::const_iterator itemItor = (*enumIter)->begin();
...
void ScriptTextEditor::populateValuesFromLocation(const ARL::ScriptAnalyzer::Location& location, int& startPos, int& endPos, bool& atEof)
{
...
int ScriptTextEditor::getStartPosition(const ARL::ScriptAnalyzer::Location& location)
{
...
int ScriptTextEditor::getEndPosition(const ARL::ScriptAnalyzer::Location& location)
{
	QTextBlock endBlock = document()->findBlockByNumber(location.end.line);
	if (endBlock.isValid() && endBlock.isVisible())
...
void ScriptTextEditor::clearFoundItems()
{
	...
	QApplication::postEvent(this, new ANORRLCustomEvent(EXTRASELECTION_UPDATE));
}
...
void ScriptTextEditor::updateFindSelection()
{
	...
	ARL::Time timeCheck = ARL::Time::now<ARL::Time::Fast>();
	...
	while ((ARL::Time::now<ARL::Time::Fast>() - timeCheck).msec() < 10)
	{
	...
	timeCheck = ARL::Time::now<ARL::Time::Fast>();
...
ScriptTraversalThread::ScriptTraversalThread(ScriptTextEditor *parent, ANORRLScriptDoc *document)
	:QThread(parent),
...
{
	shared_ptr<ARL::DataModel> dm = m_pDocument->getDataModel();

	void* resultPtr = NULL;

	{
		ARL::DataModel::LegacyLock l(dm.get(), ARL::DataModelJob::Write);

		std::string code = m_pScriptEditor->toPlainText().toStdString();
		shared_ptr<ARL::Instance> scriptInstance = m_pDocument->getCurrentScript().toInstance();
		// Linked script source won't have an instance, so use a fake one to appease the analyzer
		if (!scriptInstance)
		{
			scriptInstance = ARL::Creatable<ARL::Instance>::create<ARL::Script>();
		}

		ARL::ScriptAnalyzer::Result result = ARL::ScriptAnalyzer::analyze(dm.get(), scriptInstance, code);
		QMetaObject::invokeMethod(m_pScriptEditor, "setAnalysisResult", Qt::QueuedConnection, Q_ARG(ARL::ScriptAnalyzer::Result, result));
	}
}
...
void FindThread::doWork()
{
	...
	ARL::DataModel::LegacyLock lock(dm.get(), ARL::DataModelJob::Write);
	...
	while (m_searchStartBlockLine != m_searchLastBlockLine)
	{
		...
		if (hasFoundItem && (ARL::Time::now<ARL::Time::Fast>() - timeCheck).msec() > 20)
		{
			timeCheck = ARL::Time::now<ARL::Time::Fast>();
			hasFoundItem = false;
			QApplication::postEvent(m_pScriptEditor, new ANORRLCustomEvent(EXTRASELECTION_UPDATE));
		}
	}
    ...
    bool       bUp)
{
    ...
    {
        ARLTextUserData* pUserData = dynamic_cast<ARLTextUserData*>(blockFound.userData());
        if (pUserData)
