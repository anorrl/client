...
#include "ANORRLTabWidget.h"
#include "ANORRLIDEDoc.h"

static const int ValidAttachRegionHeight = 256;

DocTabManager::DocTabManager(ANORRLMainWindow& mainWindow)
    : QObject(&mainWindow),
      m_RubberBand(QRubberBand::Rectangle),
      m_MainWindow(mainWindow),
      m_MouseDown(false)
{
    m_TabWidget = new ANORRLTabWidget(&m_MainWindow);
    ...
    ARL::BaldPtr<QWidget> cornerWidget = new QWidget(m_TabWidget);
    ARL::BaldPtr<QLayout> layout = new QHBoxLayout(cornerWidget);
...
{
    ARLASSERT(m_Docs.isEmpty());
}

/**
 * Adds a new tab for the doc.
 */
void DocTabManager::addDoc(IANORRLDoc& doc, int insertAtIndex)
{
    ARLASSERT(m_Docs.indexOf(&doc) == -1);
    ARLASSERT(doc.getViewer());

    ARL::BaldPtr<QWidget> widget = doc.getViewer();
    ARLASSERT(widget);
	...
	ARLASSERT((index != -1) && (index < m_Docs.size()));
 ...
 */
bool DocTabManager::removeDoc(IANORRLDoc& doc)
{
 ...
 * @return false if the doc is not handled by this manager.
 */
bool DocTabManager::renameDoc(IANORRLDoc& doc,const QString& text,const QString& tooltip,const QIcon& icon)
{
    const int index = m_Docs.indexOf(&doc);
    if ( index == -1 )
        return false;
 ...
 * @return false if the doc is not handled by this manager.
 */
bool DocTabManager::setCurrentDoc(IANORRLDoc& doc)
{
    const int index = m_Docs.indexOf(&doc);
    if ( index == -1 )
        return false;
 ...
 */
IANORRLDoc* DocTabManager::getCurrentDoc() const
{
...
void DocTabManager::onCurrentChanged(int index)
{
    if ( index == -1 )
    {
        ANORRLDocManager::Instance().setCurrentDoc(NULL);
    }
    else if ( !m_FakeTab->isVisible() && index < m_Docs.size())
    {
        ARL::BaldPtr<IANORRLDoc> doc = m_Docs[index];
...
{   
    ...
    if (doc->docType() == IANORRLDoc::IDE)
    {
        ANORRLIDEDoc* ideDoc = static_cast<ANORRLIDEDoc*>(doc.get());
...
{
    ARL::BaldPtr<IANORRLDoc> doc = getCurrentDoc();
    if ( doc )
        ANORRLDocManager::Instance().detachDoc(*doc,false);
}
...
void DocTabManager::onMouseMoveEvent(QMouseEvent* event)
{
    ...
    {
        ANORRLDocManager::Instance().detachDoc(*m_Docs[m_DragTabIndex],true);
        m_MouseDown = false;
    }
}
...
void DocTabManager::setDockHoverOverPos(const QPoint& globalPos)
{
    ...
    else
    {
        ...
        if ( m_FakeTab->isVisible() )
        {
			ARLASSERT(m_TabWidget->indexOf(m_FakeTab) != -1);
 ...
 */
bool DocTabManager::attemptAttach(IANORRLDoc& doc,const QPoint& globalPos)
{
    m_RubberBand.setVisible(false);
	int fakeTabIndex = -1;
    ...
    if ( isValidHoverOver(globalPos) )
    {
        ANORRLDocManager::Instance().attachDoc(doc, fakeTabIndex);
        return true;
    }
    else
    {
...
void DocTabManager::onFocusChanged(QWidget* oldWidget,QWidget* newWidget)
{
    ...
    {
        ARL::BaldPtr<IANORRLDoc> doc = m_Docs[m_TabWidget->currentIndex()];
        ANORRLDocManager::Instance().setCurrentDoc(doc);
    }
}
