...
static const QString QuickAccessConfigKey         = "ANORRL Quick Access Config";
...
void QuickAccessActionsListWidget::initialize()
{
	setUpdatesEnabled(false);
	clear();
	ANORRLQuickAccessConfig::QuickAccessActionCollection& quickAccessBarActions = ANORRLQuickAccessConfig::singleton().getQuickAccessBarActions();
...
ActionItem* QuickAccessActionsListWidget::createItem(QAction* action)
{
	...
	if (ANORRLQuickAccessConfig::singleton().availableInQuickAccessBar(action))
		return new QuickAccessActionItem(action, ANORRLQuickAccessConfig::singleton().visibleInQuickAccessBar(action));
...
{
	ANORRLQuickAccessConfig::QuickAccessActionCollection quickAccessCollection;
	int numChild = 	topLevelItemCount(), currentChild = 0;
	ActionItem* pCurrentItem = NULL;
	...
	{		
		...
		{
			quickAccessCollection.append(ANORRLQuickAccessConfig::ActionData(pCurrentItem->getAction(), 
	...
	ANORRLQuickAccessConfig::singleton().recreateQuickAccessBar(quickAccessCollection);
}
...
: ANORRLSavingStateDialog<QDialog>(parent, "QuickAccessConfigDialog/Geometry")
, m_dataChanged(false)
{
	...
	connect(&ANORRLQuickAccessConfig::singleton(), SIGNAL(defaultsRestored()), this, SLOT(onDefaultsRestored()));
...
bool QuickAccessConfigDialog::eventFilter(QObject* watched, QEvent* evt)
{
	if (evt->type() == QEvent::ActionAdded)
	{
		...
		if (ANORRLQuickAccessConfig::singleton().availableInQuickAccessBar(action))
		{
			QAction* actionToAdd = ANORRLQuickAccessConfig::singleton().getProxyAction(action);
			if (!actionToAdd)
				actionToAdd = action;
	...
	else if (evt->type() == QEvent::ActionRemoved)
	{
		...
		if (ANORRLQuickAccessConfig::singleton().availableInQuickAccessBar(action))
		{
			QAction* isProxyAction = ANORRLQuickAccessConfig::singleton().getProxyAction(action);
			if (!isProxyAction)
			{
			...
			{
				ANORRLQuickAccessConfig::singleton().resetProxyAction(action);
			}
		}
	}
...
void QuickAccessConfigDialog::onAddButtonClicked()
{
	...
	QAction* actionToAdd = ANORRLQuickAccessConfig::singleton().mapProxyAction(pActionItem->getAction(), true);
	if (!actionToAdd)
...
void QuickAccessConfigDialog::restoreDefaults(QAbstractButton* button)
{
	if (m_dialogButtonBox->button(QDialogButtonBox::RestoreDefaults) == button)
		ANORRLQuickAccessConfig::singleton().restoreDefaults();
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ANORRLQuickAccessConfig
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
ANORRLQuickAccessConfig& ANORRLQuickAccessConfig::singleton()
{
	static ANORRLQuickAccessConfig* anorrlQuickAccessConfig = new ANORRLQuickAccessConfig();
	return *anorrlQuickAccessConfig;
}

ANORRLQuickAccessConfig::ANORRLQuickAccessConfig()
...
void ANORRLQuickAccessConfig::updateQuickAccessMenu(QMenu* pMenu)
{
	pMenu->addSeparator()->setText(tr("Advanced Customization"));
	pMenu->addAction(m_pMainWindow->customizeQuickAccessAction);
}

void ANORRLQuickAccessConfig::updateActionVisiblity(QAction* action)
{
...
void ANORRLQuickAccessConfig::onExtensionMenuShow()
{
...
void ANORRLQuickAccessConfig::onExtensionMenuHide()
{
	QMenu* menu = qobject_cast<QMenu*>(sender());
	if (menu)
	{
...
void ANORRLQuickAccessConfig::storeDefaults(const QuickAccessActionCollection& defaultQuickAccessBarActions) 
{ m_defaultQuickAccessBarActions = defaultQuickAccessBarActions;  }

void ANORRLQuickAccessConfig::restoreDefaults()
{ 
	recreateQuickAccessBar(m_defaultQuickAccessBarActions);
	Q_EMIT defaultsRestored();
}

void ANORRLQuickAccessConfig::recreateQuickAccessBar(const QuickAccessActionCollection& quickAccessBarActions)
{
...
bool ANORRLQuickAccessConfig::loadQuickAccessConfig()
{
	QSettings settings;
...
void ANORRLQuickAccessConfig::saveQuickAccessConfig()
{
...
void ANORRLQuickAccessConfig::addToQuickAccessBar(QAction* actionToAdd, bool visibility)
{
...
void ANORRLQuickAccessConfig::removeFromQuickAccessBar(QAction* actionToRemove)
{
...
QAction* ANORRLQuickAccessConfig::mapProxyAction(QAction* action, bool force)
{
...
QAction* ANORRLQuickAccessConfig::resetProxyAction(QAction* action)
{
	QuickAccessBarProxyAction* pProxyAction = dynamic_cast<QuickAccessBarProxyAction*>(getProxyAction(action));
	if (!pProxyAction)
		return NULL;
...
bool ANORRLQuickAccessConfig::updateProxyAction(QAction* action)
{
	QuickAccessActionCollection::iterator iter = m_currentQuickAccessBarActions.begin();
	while (iter != m_currentQuickAccessBarActions.end())
	{
...
bool ANORRLQuickAccessConfig::visibleInQuickAccessBar(QAction* action)
{
...
bool ANORRLQuickAccessConfig::availableInQuickAccessBar(QAction* action)
{
	QAction* actionToLookFor = getProxyAction(action);
	if (!actionToLookFor)
		actionToLookFor = action;
...
QAction* ANORRLQuickAccessConfig::getProxyAction(QAction* action)
{
 ...
 bool ANORRLQuickAccessConfig::ActionData::isProxyAction() const
