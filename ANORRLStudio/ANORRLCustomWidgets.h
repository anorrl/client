...
class ANORRLCustomEvent: public QEvent
{
public:
	ANORRLCustomEvent(int eventType) 
	: QEvent(QEvent::Type(eventType)) 
	{}
};

class ANORRLCustomEventWithArg: public QEvent
{
public:
	ANORRLCustomEventWithArg(int eventType, boost::function<void()> eventArg) 
	: QEvent(QEvent::Type(eventType)) 
	, m_pEventArg(NULL)
	{
		//make a copy
...
public:
	...
	QString getImageUrl(const QString& lastValue, const QPoint& location, boost::shared_ptr<ARL::DataModel> dm);
...
class PartSelectionTool : public ARL::Named<ARL::MouseCommand, sPartSelectionTool>
{
public:
...
private:
	...
	/*override*/ void onMouseHover(const shared_ptr<ARL::InputObject>& inputObject);
	/*override*/ shared_ptr<ARL::MouseCommand> onMouseDown(const shared_ptr<ARL::InputObject>& inputObject);
	/*override*/ shared_ptr<ARL::MouseCommand> onMouseUp(const shared_ptr<ARL::InputObject>& inputObject);
	/*override*/ void render3dAdorn(ARL::Adorn* adorn);

	boost::shared_ptr<ARL::PartInstance> m_pHighlightPart;
...
class PrimaryPartSelectionEditor: public QWidget, public InstanceSelectionHandler
{
	Q_OBJECT
public:
	PrimaryPartSelectionEditor(QWidget* parent, PropertyItem* pItem, const QString& labelText, boost::shared_ptr<ARL::ModelInstance> instance);
...
private:
	...
	/*override*/ void onInstanceSelected(boost::shared_ptr<ARL::Instance> instance);
	/*override*/ void onInstanceHovered(boost::shared_ptr<ARL::Instance> instance);

	void onPartSelected(ARL::PartInstance* pPart);
	void onPartHoveredOver(ARL::PartInstance* pPart, bool& isValid);

	bool setPrimaryPart(ARL::PartInstance* pPart);
	...
	boost::weak_ptr<ARL::PartInstance>         m_pTempPrimaryPart;
