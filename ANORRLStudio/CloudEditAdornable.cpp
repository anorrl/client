...
void CloudEditAdornable::renderCamera(Color3 playerColor, shared_ptr<ARL::Network::Player> player, const Camera* localCamera, Adorn* adorn)
{
...
void CloudEditAdornable::attach(shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> playersDataManager)
{
	...
	if (Network::Players* players = m_pDataModel->find<Network::Players>())
	{
		for (auto& playerInstance : *(players->getPlayers()))
		{
			if (shared_ptr<Network::Player> p = Instance::fastSharedDynamicCast<Network::Player>(playerInstance))
			{
				...
				Color3 playerColor = ARL::BrickColor(m_pPlayersDataManager->getPlayerColor(p->getUserID())).color3();

				renderCamera(playerColor, p, m_pDataModel->getWorkspace()->getConstCamera(), adorn);
				shared_ptr<const ARL::Instances> selectedInstances = m_pPlayersDataManager->getLastKnownSelection(p->getUserID());
