...
class ANORRLPropertyWidget;
class ANORRLPluginHost;
class ANORRLSettingsDialog;
class ANORRLDiagnosticsView;
class ANORRLScriptReview;
class ANORRLTaskScheduler;
class ANORRLWebDoc;
class InsertObjectWidget;
class InsertServiceDialog;
class ANORRLToolBox;
class ANORRLTextOutputWidget;
...
class ANORRLMainWindow: public ANORRLRibbonMainWindow
{
	Q_OBJECT
...
public:
	...
	ANORRLMainWindow(const QMap<QString, QString> argMap);
    virtual ~ANORRLMainWindow();
    ...
    ANORRLPluginHost& pluginHost() const                { return *m_pPluginHost; }
	...
	static ANORRLMainWindow* get(QObject* context);
    ...
    bool requestDocClose(IANORRLDoc& doc, bool closeIfLastDoc = true);
    ...
    Q_INVOKABLE bool requestDocClose(IANORRLDoc* doc) { return requestDocClose(*doc); }

    bool handleFileOpen(
        const QString&          fileName,
        IANORRLDoc::ARLDocType  type,
	...
	ANORRLWebDoc* getConfigureWebDoc();
	void closeConfigureWebDoc();
	void closePublishGameWindow();

    ANORRLTextOutputWidget* getOutputWidget()   { return m_pTextOutput; }

    ANORRLPluginHost* getPluginHost() const 	{ return m_pPluginHost; }
	void setPluginHost(ANORRLPluginHost* val) 	{ m_pPluginHost = val; }
...
public Q_SLOTS:
    ...
    bool fileSave(IANORRLDoc* pDoc = NULL);
...
private Q_SLOTS:
	...
	void fileOpen();
	void fileClose();
	bool fileSaveAs(IANORRLDoc *pDoc=NULL);
...
private:
	...
	void checkInternetConnectionSendCounter(ARL::Http& anorrlRequest);
    ...
    ARL::BaldPtr<ANORRLSettingsDialog>          m_pSettingsDialog;
    ARL::BaldPtr<ShortcutHelpDialog>            m_pShortcutHelpDialog;
    ARL::BaldPtr<ANORRLInputConfigDialog>       m_pInputConfigDialog;
    ARL::BaldPtr<QSplashScreen>                 m_splashScreen;

    ANORRLPluginHost*                m_pPluginHost;
	boost::scoped_ptr<ANORRLWebDoc>  m_managePluginsDoc;
	boost::scoped_ptr<ANORRLWebDoc>  m_configureGameDoc;
