...
#include "stdafx.h"
#include "Bootstrapper2013Client.h"
...
static const TCHAR* BootstrapperFileName    = _T("ANORRL2013PlayerLauncher.exe");
...
static const TCHAR* StartRobloxAppMutex     = _T("arl.lambda.cam/startRoblox2013App");
static const TCHAR* LauncherFileName        = _T("RobloxProxy.dll");
static const TCHAR* LauncherFileName64      = _T("RobloxProxy64.dll");
static const TCHAR* FriendlyName            = _T("ANORRL2013");
	...
	:Bootstrapper(hInstance)
{
	...
	_protocolHandlerScheme = get2013PlayerProtocolScheme(BaseHost());

	//Plugin depends on RobloxReg value as well, 
	//so if you ever change this guy make sure that plugins code is updates as well
	_regSubPath = _T("ANORRL2013");
	_regPath = _T("SOFTWARE\\") + _regSubPath;
	_versionFileName = _T("ANORRL2013Version.txt");
	_versionGuidName = _T(VERSIONGUIDNAME2013PLAYER);
...
{
	return get2013PlayerInstallKey();
}
...
HRESULT BootstrapperClient::SheduleTaskWinVista()
{
	...
	regInfo->put_Description(L"This tasks helps ANORRL 2013 game be updated.");
...
void BootstrapperClient::DeployComponents(bool isUpdating, bool commitData)
{
	...
	files.push_back(std::pair<std::wstring, std::wstring>(_T("ANORRL2013App.zip"), _T("")));
...
void BootstrapperClient::DoInstallApp()
{
	...
	RegisterUninstall(_T(PLAYER2013LINKNAME_CUR));

    setStage(6);
	try		
	{
        LOG_ENTRY("createShortcuts()");
		CreateShortcuts(!isUpdating, _T(PLAYER2013LINKNAME_CUR), BootstrapperFileName, _T("-browser"));
	}
...
void BootstrapperClient::DoUninstallApp(CRegKey &hk)
{
	...
	deleteCurVersionKeys(logger, isPerUser(), get2013PlayerCode().c_str());

	LOG_ENTRY("BootstrapperClient::DoUninstallApp - deleteDesktopShortcut");
	deleteDesktopShortcut(logger, _T(PLAYER2013LINKNAME_CUR));

	LOG_ENTRY("BootstrapperClient::DoUninstallApp - deleteProgramsShortcut");
	deleteProgramsShortcut(logger, isPerUser(), _T(PLAYER2013LINKNAME_CUR));
