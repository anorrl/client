...
using namespace std ;
typedef list<NP_ANORRL_INSTANCE *> NppANORRLInstance;
...
private:
	...
	NppANORRLInstance queue; 
	NppANORRLInstance::iterator i;
	NppANORRLInstance::iterator index;
	int code;
...
public:
	...
	void add(NP_ANORRL_INSTANCE *item) {
		lock();
	...
	ILauncher * find(NPObject *obj) {
		ILauncher *item = NULL;
		NP_ANORRL_INSTANCE  * tmp = NULL;
		lock();
		if( queue.size()>0 ){
			for (i =  queue.begin(); i != queue.end(); ++i){
				tmp = (NP_ANORRL_INSTANCE *)(*i);
				if( tmp->so == obj ){
					item = tmp->IANORRLLauncher;
					break;
				}
			}
		}
	...
	NP_ANORRL_INSTANCE * find(NPP  activeInstance){
		NP_ANORRL_INSTANCE *item = NULL, * tmp = NULL;
		lock();
		if( queue.size()>0 ){
			for (i =  queue.begin(); i != queue.end(); ++i){
				tmp = (NP_ANORRL_INSTANCE *)(*i);
	...
	NP_ANORRL_INSTANCE * findAndRemove(NPP  activeInstance){
		NP_ANORRL_INSTANCE *item = NULL, * tmp = NULL;
		lock();
		if( queue.size()>0 ){
			for (i =  queue.begin(); i != queue.end(); ++i){
