...
#include "stdafx.h"
#include "BootstrapperQT2013Studio.h"
...
static const TCHAR* StartEvent                      = _T("arl.lambda.cam/robloxQT2013StudioStartedEvent");
...
{
    LOG_ENTRY("BootstrapperQT2013Studio::BootstrapperQT2013Studio");

	_versionFileName = _T("BootstrapperQT2013StudioVersion.txt");
	_versionGuidName = _T(VERSIONGUIDNAME2013STUDIO);

	_protocolHandlerScheme = getQT2013StudioProtocolScheme(BaseHost());
}
...
{
	return getQT2013StudioInstallKey();
}
...
{
	std::wstring event = format_string(_T("QT2013Studio%s"), eventName);
...
void BootstrapperQTStudio::DeployComponents(bool isUpdating, bool commitData)
{
	...
	files.push_back(std::pair<std::wstring, std::wstring>(_T("ANORRL2013Studio.zip"), _T("")));
...
void BootstrapperQTStudio::registerFileTypes()
{
    // register file extensions
	{
		...
		CRegKey roblox_place = CreateKey(rbxl, _T("ANORRL.Place"), NULL);
...
void BootstrapperQTStudio::DoInstallApp()
{
	...
	RegisterUninstall(_T(STUDIO2013QTLINKNAME_CUR));
	...
	try
	{
		...
		CreateShortcuts(!isUpdating || hasLegacyShortcut, _T(STUDIO2013QTLINKNAME_CUR), BootstrapperQTStudioFileName, _T("-ide"));
	}
	catch (std::exception &)
	{
...
void BootstrapperQTStudio::DoUninstallApp(CRegKey &hk)
{
	...
	deleteCurVersionKeys(logger, isPerUser(), getQT2013StudioCode().c_str());
    ...
    createRobloxShortcut(logger,isPerUser(),_T(STUDIO2013QTLINKNAME_CUR),exePath.c_str(),_T("-ide"),false,false);
	createRobloxShortcut(logger,isPerUser(),_T(STUDIO2013QTLINKNAME_CUR),exePath.c_str(),_T("-ide"),true,false);
    ...
    CRegKey installKey = CreateKey(hk, getQT2013StudioRegistryPath().c_str(), progPath.c_str());
